/*\
title: $:/plugins/tiddlywiki/codemirror-6/plugins/lang-sass/lang-sass.js
...
class s {
	constructor(e, t, O, s, i, r, a, o, n, l = 0, h) {
		this.p = e, this.stack = t, this.state = O, this.reducePos = s, this.pos = i, this.score = r, this.buffer = a, this.bufferBase = o, this.curContext = n, this.lookAhead = l, this.parent = h
	}
	toString() {
	...
	static start(e, t, O = 0) {
		...
		return new s(e, [], t, O, O, 0, [], 0, r ? new i(r, r.start) : null, 0, null)
	}
	get context() {
	...
	reduce(e) {
		...
		let O = e >> 19,
			...
			a = i.dynamicPrecedence(s);
		if(a && (this.score += a), 0 == O) return this.pushState(i.getGoto(this.state, s, !0), this.reducePos), s < i.minRepeatTerm && this.storeNode(s, this.reducePos, this.reducePos, r ? 8 : 4, !0), void this.reduceContext(s, this.reducePos);
	...
	shift(e, t, O, s) {
		if(131072 & e) this.pushState(65535 & e, this.pos);
			...
			this.pos = s, !r.stateFlag(i, 1) && (s > O || t <= r.maxNode) && (this.reducePos = s), this.pushState(i, Math.min(O, this.reducePos)), this.shiftContext(t, O), t <= r.maxNode && this.buffer.push(t, O, s, 4)
		}
	}
	apply(e, t, O, s) {
...
function $(e, t, O, s, i, r) {
	...
	e: for(; 0 != (o & e[a]);) {
		let O = e[a + 1];
		for(let s = a + 3; s < O; s += 2)
			if((e[s + 1] & o) > 0) {
				let O = e[s];
				if(n.allows(O) && (-1 == t.token.value || t.token.value == O || p(O, t.token.value, i, r))) {
					t.acceptToken(O);
					break
				}
			} let s = t.next,
...
class g {
	...
	nodeAt(t) {
		...
		for(;;) {
			...
			let r = s.children[i],
				a = this.start[O] + s.positions[i];
			if(a > t) return this.nextStart = a, null;
			if(r instanceof e.Tree) {
				if(a == t) {
					if(a < this.safeFrom) return null;
					let t = a + r.length;
					if(t <= this.safeTo) {
						let O = r.prop(e.NodeProp.lookAhead);
						if(!O || t + O < this.fragment.to) return r
					}
				}
				this.index[O]++, a + r.length >= Math.max(this.safeFrom, t) && (this.trees.push(r), this.start.push(a), this.index.push(0))
			} else this.index[O]++, this.nextStart = a + r.length
		}
	}
}
class k {
	...
	getActions(e) {
		...
		for(let s = 0; s < i.length; s++) {
			...
			let n = i[s],
				l = this.tokens[s];
			if((!O || n.fallback) && ((n.contextual || l.start != e.pos || l.mask != r || l.context != a) && (this.updateCachedToken(l, n, e), l.mask = r, l.context = a), l.lookAhead > l.end + 25 && (o = Math.max(l.lookAhead, o)), 0 != l.value)) {
				let s = t;
	...
	addActions(e, t, O, s) {
		...
		for(let e = 0; e < 2; e++)
			for(let o = r.stateSlot(i, e ? 2 : 1);; o += 3) {
				if(65535 == a[o]) {
					if(1 != a[o + 1]) {
						0 == s && 2 == a[o + 1] && (s = this.putAction(x(a, o + 2), t, O, s));
						break
					}
					o = x(a, o + 2)
				}
				a[o] == t && (s = this.putAction(x(a, o + 1), t, O, s))
			}
		return s
	}
}
class R {
	constructor(e, t, O, i) {
		...
		this.stacks = [s.start(this, e.top[0], r)], this.fragments = O.length && this.stream.end - r > 4 * e.bufferLength ? new g(O, e.nodeSet) : null
	}
	get parsedPos() {
	...
	advance() {
		...
		for(let r = 0; r < O.length; r++) {
			let a = O[r];
			for(;;) {
				if(this.tokens.mainToken = null, a.pos > s) i.push(a);
				else {
					if(this.advanceStack(a, i, O)) continue;
					{
						e || (e = [], t = []), e.push(a);
	...
	advanceStack(t, O, s) {
		...
		if(this.fragments) {
			...
			for(let o = this.fragments.nodeAt(i); o;) {
				...
				if(i > -1 && o.length && (!O || (o.prop(e.NodeProp.contextHash) || 0) == s)) return t.useNode(o, i), f && console.log(a + this.stackID(t) + ` (via reuse of ${r.getName(o.type.id)})`), !0;
		...
		if(o > 0) return t.reduce(o), f && console.log(a + this.stackID(t) + ` (via always-reduce ${r.getName(65535&o)})`), !0;
		...
		for(let e = 0; e < n.length;) {
			...
			if(Q.apply(o, l, d ? d.start : Q.pos, h), f && console.log(a + this.stackID(Q) + ` (via ${65536&o?`reduce of ${r.getName(65535&o)}`:"shift"} for ${r.getName(l)} @ ${i}${Q==t?"":", split"})`), c) return !0;
	...
	runRecovery(e, t, O) {
		...
		for(let r = 0; r < e.length; r++) {
			...
			if(a.deadEnd) {
				if(i) continue;
				if(i = !0, a.restart(), f && console.log(l + this.stackID(a) + " (restarted)"), this.advanceFully(a, O)) continue
			}
			let h = a.split(),
				c = l;
			...
			for(let e of a.recoverByInsert(o)) f && console.log(l + this.stackID(e) + " (via recover-insert)"), this.advanceFully(e, O);
			this.stream.end > a.pos ? (n == a.pos && (n++, o = 0), a.recoverByDelete(o, n), f && console.log(l + this.stackID(a) + ` (via recover-delete ${this.parser.getName(o)})`), P(a, O)) : (!s || s.score < h.score) && (s = h)
		}
		return s
	}
	stackToTree(t) {
...
class b extends e.Parser {
	constructor(t) {
			...
			for(let O of t.nodeProps) {
				...
				for(let e = 1; e < O.length;) {
					...
					if(s >= 0) r(s, t, O[e++]);
						...
						let i = O[e + -s];
						for(let a = -s; a > 0; a--) r(O[e++], t, i);
						e++
					}
				}
			}
		this.nodeSet = new e.NodeSet(O.map((O, r) => e.NodeType.define({
			name: r >= this.minRepeatTerm ? void 0 : O,
			id: r,
			props: i[r],
			top: s.indexOf(r) > -1,
			error: 0 == r,
		...
		this.specializers = this.specializerSpecs.map(z), this.states = o(t.states, Uint32Array), this.data = o(t.stateData), this.goto = o(t.goto), this.maxTerm = t.maxTerm, this.tokenizers = t.tokenizers.map(e => "number" == typeof e ? new c(a, e) : e), this.topRules = t.topRules, this.dialects = t.dialects || {}, this.dynamicPrecedences = t.dynamicPrecedences || null, this.tokenPrecTable = t.tokenPrec, this.termNames = t.termNames || null, this.maxNode = this.nodeSet.types.length - 1, this.dialect = this.parseDialect(), this.top = this.topRules[Object.keys(this.topRules)[0]]
	}
	createParse(e, t, O) {
	...
	hasAction(e, t) {
		let O = this.data;
		for(let s = 0; s < 2; s++)
			for(let i, r = this.stateSlot(e, s ? 2 : 1);; r += 3) {
				if(65535 == (i = O[r])) {
					if(1 != O[r + 1]) {
						if(2 == O[r + 1]) return x(O, r + 2);
						break
					}
					i = O[r = x(O, r + 2)]
				}
				if(i == t || 0 == i) return x(O, r + 1)
			}
		return 0
	}
	stateSlot(e, t) {
	...
	configure(e) {
		...
		return e.tokenizers && (t.tokenizers = this.tokenizers.map(t => {
			...
			if(!i) return O;
			let r = Object.assign(Object.assign({}, O), {
				external: i.to
			});
			return t.specializers[s] = z(r), r
...
const j = new d((e, t) => {
	...
	C = new d((e, t) => {
		for(let O = !1, s = 0, i = 0;; i++) {
			...
			if(!(Y(r) || 45 == r || 95 == r || O && T(r))) {
				if(35 == r && 123 == e.peek(1)) {
					e.acceptToken(5, 2);
					break
				}
				O && e.acceptToken(2 == s && t.canShift(4) ? 4 : t.canShift(173) ? 173 : 40 == r ? 171 : 172);
				break
			}!O && (45 != r || i > 0) && (O = !0), s === i && 45 == r && s++, e.advance()
		}
	}),
	_ = new d(e => {
		if(125 == e.next) {
...
const V = new N(null, 0),
	...
	K = b.deserialize({
		...
		tokenizers: [U, W, _, E, C, j, q, Z, 0, 1, 2, 3, 4],
		topRules: {
			StyleSheet: [0, 10],
			Styles: [1, 145]
		},
...
const te = (e, t, O) => (s, i) => {
		for(let r = !1, a = 0, o = 0;; o++) {
			...
			if(M(n) || 45 == n || 95 == n || r && H(n)) !r && (45 != n || o > 0) && (r = !0), a === o && 45 == n && a++, s.advance();
			else {
				if(92 != n || 10 == s.peek(1)) {
					r && s.acceptToken(2 == a && i.canShift(2) ? t : 40 == n ? O : e);
					break
				}
...
b.deserialize({
	...
	tokenizers: [ie, re, Oe, se, 1, 2, 3, 4, new Q("m~RRYZ[z{a~~g~aO#v~~dP!P!Qg~lO#w~~", 28, 129)],
	topRules: {
		StyleSheet: [0, 6],
		Styles: [1, 105]
	},
...
function Pe(t, O, s) {
	if(O.to - O.from > 4096) {
		...
		if(o.firstChild())
			do {
				for(let e of Pe(t, o.node, s)) a.has(e.label) || (a.add(e.label), r.push(e))
			} while(o.nextSibling());
		return ge.set(O, r), r
	} {
		let e = [],
		...
		return O.cursor().iterate(O => {
			var r;
			if(s(O) && O.matchContext(ke) && ":" == (null === (r = O.node.nextSibling) || void 0 === r ? void 0 : r.name)) {
...
const Xe = e => t => {
		let {
			state: s,
			pos: i
		} = t, r = O.syntaxTree(s).resolveInner(i, -1), a = r.type.isError && r.from == r.to - 1 && "-" == s.doc.sliceString(r.from, r.to);
