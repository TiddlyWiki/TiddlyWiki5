"use strict";
var e = require("$:/plugins/tiddlywiki/codemirror-6/lib/lezer-common.js"),
	t = require("$:/plugins/tiddlywiki/codemirror-6/lib/lezer-highlight.js"),
	O = require("$:/plugins/tiddlywiki/codemirror-6/lib/codemirror-language.js");
class s {
	constructor(e, t, O, s, i, a, r, o, n, l = 0, h) {
		this.p = e, this.stack = t, this.state = O, this.reducePos = s, this.pos = i, this.score = a, this.buffer = r, this.bufferBase = o, this.curContext = n, this.lookAhead = l, this.parent = h
	}
	toString() {
	...
	static start(e, t, O = 0) {
		...
		return new s(e, [], t, O, O, 0, [], 0, a ? new i(a, a.start) : null, 0, null)
	}
	get context() {
	...
	reduce(e) {
		...
		let O = e >> 19,
			...
			a = this.reducePos < this.pos - 25 && this.setLookAhead(this.pos),
			r = i.dynamicPrecedence(s);
		if(r && (this.score += r), 0 == O) return this.pushState(i.getGoto(this.state, s, !0), this.reducePos), s < i.minRepeatTerm && this.storeNode(s, this.reducePos, this.reducePos, a ? 8 : 4, !0), void this.reduceContext(s, this.reducePos);
		let o = this.stack.length - 3 * (O - 1) - (262144 & e ? 6 : 0),
			n = o ? this.stack[o - 2] : this.p.ranges[0].from,
			l = this.reducePos - n;
		l >= 2e3 && !(null === (t = this.p.parser.nodeSet.types[s]) || void 0 === t ? void 0 : t.isAnonymous) && (n == this.p.lastBigReductionStart ? (this.p.bigReductionCount++, this.p.lastBigReductionSize = l) : this.p.lastBigReductionSize < l && (this.p.bigReductionCount = 1, this.p.lastBigReductionStart = n, this.p.lastBigReductionSize = l));
		let h = o ? this.stack[o - 1] : 0,
			c = this.bufferBase + this.buffer.length - h;
		if(s < i.minRepeatTerm || 131072 & e) {
			let e = i.stateFlag(this.state, 1) ? this.pos : this.reducePos;
			this.storeNode(s, n, e, c + 4, !0)
		}
		if(262144 & e) this.state = this.stack[o];
		else {
			let e = this.stack[o - 3];
			this.state = i.getGoto(e, s, !0)
		}
		for(; this.stack.length > o;) this.stack.pop();
		this.reduceContext(s, n)
	}
	storeNode(e, t, O, s = 4, i = !1) {
		if(0 == e && (!this.stack.length || this.stack[this.stack.length - 1] < this.buffer.length + this.bufferBase)) {
			let e = this,
				s = this.buffer.length;
			if(0 == s && e.parent && (s = e.bufferBase - e.parent.bufferBase, e = e.parent), s > 0 && 0 == e.buffer[s - 4] && e.buffer[s - 1] > -1) {
				if(t == O) return;
				if(e.buffer[s - 2] >= t) return void(e.buffer[s - 2] = O)
			}
		}
		if(i && this.pos != O) {
			...
			if(i > 0 && (0 != this.buffer[i - 4] || this.buffer[i - 1] < 0)) {
				let e = !1;
				for(let t = i; t > 0 && this.buffer[t - 2] > O; t -= 4)
					if(this.buffer[t - 1] >= 0) {
						e = !0;
						break
					} if(e)
					for(; i > 0 && this.buffer[i - 2] > O;) this.buffer[i] = this.buffer[i - 4], this.buffer[i + 1] = this.buffer[i - 3], this.buffer[i + 2] = this.buffer[i - 2], this.buffer[i + 3] = this.buffer[i - 1], i -= 4, s > 4 && (s -= 4)
			}
			this.buffer[i] = e, this.buffer[i + 1] = t, this.buffer[i + 2] = O, this.buffer[i + 3] = s
		} else this.buffer.push(e, t, O, s)
	}
	shift(e, t, O, s) {
		if(131072 & e) this.pushState(65535 & e, this.pos);
		else if(262144 & e) this.pos = s, this.shiftContext(t, O), t <= this.p.parser.maxNode && this.buffer.push(t, O, s, 4);
		else {
			let i = e,
				{
					parser: a
				} = this.p;
			this.pos = s, !a.stateFlag(i, 1) && (s > O || t <= a.maxNode) && (this.reducePos = s), this.pushState(i, Math.min(O, this.reducePos)), this.shiftContext(t, O), t <= a.maxNode && this.buffer.push(t, O, s, 4)
		}
	}
	apply(e, t, O, s) {
		65536 & e ? this.reduce(e) : this.shift(e, t, O, s)
	}
	useNode(e, t) {
		let O = this.p.reused.length - 1;
		(O < 0 || this.p.reused[O] != e) && (this.p.reused.push(e), O++);
		let s = this.pos;
		this.reducePos = this.pos = s + e.length, this.pushState(t, s), this.buffer.push(O, s, this.reducePos, -1), this.curContext && this.updateContext(this.curContext.tracker.reuse(this.curContext.context, e, this, this.p.stream.reset(this.pos - e.length)))
	}
	split() {
		let e = this,
			t = e.buffer.length;
		...
		return new s(this.p, this.stack.slice(), this.state, this.reducePos, this.pos, this.score, O, i, this.curContext, this.lookAhead, e)
	}
	recoverByDelete(e, t) {
	...
	recoverByInsert(e) {
		...
		if(t.length > 8 || this.stack.length >= 120) {
			let O = [];
			for(let s, i = 0; i < t.length; i += 2)(s = t[i + 1]) != this.state && this.p.parser.hasAction(s, e) && O.push(t[i], s);
			if(this.stack.length < 120)
				for(let e = 0; O.length < 8 && e < t.length; e += 2) {
					let s = t[e + 1];
					O.some((e, t) => 1 & t && e == s) || O.push(t[e], s)
				}
			t = O
		}
		let O = [];
		for(let e = 0; e < t.length && O.length < 4; e += 2) {
			...
			let i = this.split();
			i.pushState(s, this.pos), i.storeNode(0, i.pos, i.pos, 4, !0), i.shiftContext(t[e], this.pos), i.reducePos = this.pos, i.score -= 200, O.push(i)
		}
		return O
	}
	forceReduce() {
	...
	findForcedReduction() {
		let {
			...
			if(!t.includes(s)) return t.push(s), e.allActions(s, t => {
				if(393216 & t);
				else if(65536 & t) {
					let O = (t >> 19) - i;
					if(O > 1) {
						let s = 65535 & t,
							i = this.stack.length - 3 * O;
						if(i >= 0 && e.getGoto(this.stack[i], s, !1) >= 0) return O << 19 | 65536 | s
					}
				} else {
					let e = O(t, i + 1);
					if(null != e) return e
	...
	updateContext(e) {
		if(e != this.curContext.context) {
			let t = new i(this.curContext.tracker, e);
...
class r {
	constructor(e, t, O) {
		this.stack = e, this.pos = t, this.index = O, this.buffer = e.buffer, 0 == this.index && this.maybeNext()
	}
...
class h {
	...
	peek(e) {
		let t, O, s = this.chunkOff + e;
		if(s >= 0 && s < this.chunk.length) t = this.pos + e, O = this.chunk.charCodeAt(s);
		else {
			let s = this.resolveOffset(e, 1);
			if(null == s) return -1;
			if(t = s, t >= this.chunk2Pos && t < this.chunk2Pos + this.chunk2.length) O = this.chunk2.charCodeAt(t - this.chunk2Pos);
			else {
				let e = this.rangeIndex,
					s = this.range;
				for(; s.to <= t;) s = this.ranges[++e];
				this.chunk2 = this.input.chunk(this.chunk2Pos = t), t + this.chunk2.length > s.to && (this.chunk2 = this.chunk2.slice(0, s.to - t)), O = this.chunk2.charCodeAt(0)
			}
		}
	...
	read(e, t) {
		...
		for(let s of this.ranges) {
			if(s.from >= t) break;
			s.to > e && (O += this.input.read(Math.max(s.from, e), Math.min(s.to, t)))
		}
		return O
	}
}
class c {
	...
	token(e, t) {
		...
		$(this.data, e, t, this.id, O.data, O.tokenPrecTable)
	}
}
c.prototype.contextual = c.prototype.fallback = c.prototype.extend = !1;
class Q {
	constructor(e, t, O) {
		this.precTable = t, this.elseToken = O, this.data = "string" == typeof e ? o(e) : e
	}
	token(e, t) {
		let O = e.pos,
			s = 0;
		for(;;) {
			...
			if($(this.data, e, t, 0, this.data, this.precTable), e.token.value > -1) break;
			if(null == this.elseToken) return;
			if(O || s++, null == i) break;
			e.reset(i, e.token)
		}
		s && (e.reset(O, e.token), e.acceptToken(this.elseToken, s))
	}
}
Q.prototype.contextual = c.prototype.fallback = c.prototype.extend = !1;
class d {
	constructor(e, t = {}) {
...
function $(e, t, O, s, i, a) {
	...
	e: for(; 0 != (o & e[r]);) {
		let O = e[r + 1];
		for(let s = r + 3; s < O; s += 2)
			if((e[s + 1] & o) > 0) {
				let O = e[s];
				if(n.allows(O) && (-1 == t.token.value || t.token.value == O || p(O, t.token.value, i, a))) {
					t.acceptToken(O);
					break
				}
			} let s = t.next,
			l = 0,
			h = e[r + 2];
		if(!(t.next < 0 && h > l && 65535 == e[O + 3 * h - 3])) {
			for(; l < h;) {
				let i = l + h >> 1,
					a = O + i + (i << 1),
					o = e[a],
...
function p(e, t, O, s) {
	let i = u(O, s, t);
	return i < 0 || u(O, s, e) < i
}
const f = "undefined" != typeof process && process.env && /\bparse\b/.test(process.env.LOG);
let m = null;

function S(t, O, s) {
	let i = t.cursor(e.IterMode.IncludeAnonymous);
	for(i.moveTo(O);;)
		if(!(s < 0 ? i.childBefore(O) : i.childAfter(O)))
			for(;;) {
				if((s < 0 ? i.to < O : i.from > O) && !i.type.isError) return s < 0 ? Math.max(0, Math.min(i.to - 1, O - 25)) : Math.min(t.length, Math.max(i.from + 1, O + 25));
...
class g {
	...
	nextFragment() {
		...
		if(e) {
			for(this.safeFrom = e.openStart ? S(e.tree, e.from + e.offset, 1) - e.offset : e.from, this.safeTo = e.openEnd ? S(e.tree, e.to + e.offset, -1) - e.offset : e.to; this.trees.length;) this.trees.pop(), this.start.pop(), this.index.pop();
	...
	nodeAt(t) {
		...
		for(;;) {
			...
			let a = s.children[i],
				r = this.start[O] + s.positions[i];
			if(r > t) return this.nextStart = r, null;
			if(a instanceof e.Tree) {
				if(r == t) {
					if(r < this.safeFrom) return null;
					let t = r + a.length;
					if(t <= this.safeTo) {
						let O = a.prop(e.NodeProp.lookAhead);
						if(!O || t + O < this.fragment.to) return a
					}
				}
				this.index[O]++, r + a.length >= Math.max(this.safeFrom, t) && (this.trees.push(a), this.start.push(r), this.index.push(0))
			} else this.index[O]++, this.nextStart = r + a.length
		}
	}
}
class k {
	constructor(e, t) {
		this.stream = t, this.tokens = [], this.mainToken = null, this.actions = [], this.tokens = e.tokenizers.map(e => new n)
	}
	getActions(e) {
		let t = 0,
			O = null,
			...
			a = s.stateSlot(e.state, 3),
			r = e.curContext ? e.curContext.hash : 0,
			o = 0;
		for(let s = 0; s < i.length; s++) {
			if(!(1 << s & a)) continue;
			let n = i[s],
				l = this.tokens[s];
			if((!O || n.fallback) && ((n.contextual || l.start != e.pos || l.mask != a || l.context != r) && (this.updateCachedToken(l, n, e), l.mask = a, l.context = r), l.lookAhead > l.end + 25 && (o = Math.max(l.lookAhead, o)), 0 != l.value)) {
				let s = t;
				if(l.extended > -1 && (t = this.addActions(e, l.extended, l.end, t)), t = this.addActions(e, l.value, l.end, t), !n.extend && (O = l, t > s)) break
			}
		}
		for(; this.actions.length > t;) this.actions.pop();
		return o && e.setLookAhead(o), O || e.pos != this.stream.end || (O = new n, O.value = e.p.parser.eofTerm, O.start = O.end = e.pos, t = this.addActions(e, O.value, O.end, t)), this.mainToken = O, this.actions
	}
	getMainToken(e) {
		...
		return t.start = O, t.end = Math.min(O + 1, s.stream.end), t.value = O == s.stream.end ? s.parser.eofTerm : 0, t
	}
	updateCachedToken(e, t, O) {
		let s = this.stream.clipPos(O.pos);
		if(t.token(this.stream.reset(s, e), O), e.value > -1) {
			let {
				parser: t
			} = O.p;
			for(let s = 0; s < t.specialized.length; s++)
				if(t.specialized[s] == e.value) {
					let i = t.specializers[s](this.stream.read(e.start, e.end), O);
	...
	putAction(e, t, O, s) {
		for(let t = 0; t < s; t += 3)
			if(this.actions[t] == e) return s;
		return this.actions[s++] = e, this.actions[s++] = t, this.actions[s++] = O, s
	}
	addActions(e, t, O, s) {
		let {
			state: i
		...
		for(let e = 0; e < 2; e++)
			for(let o = a.stateSlot(i, e ? 2 : 1);; o += 3) {
				if(65535 == r[o]) {
					if(1 != r[o + 1]) {
						0 == s && 2 == r[o + 1] && (s = this.putAction(x(r, o + 2), t, O, s));
						break
					}
					o = x(r, o + 2)
				}
				r[o] == t && (s = this.putAction(x(r, o + 1), t, O, s))
			}
		return s
	}
}
class R {
	constructor(e, t, O, i) {
		this.parser = e, this.input = t, this.ranges = i, this.recovering = 0, this.nextStackID = 9812, this.minStackPos = 0, this.reused = [], this.stoppedAt = null, this.lastBigReductionStart = -1, this.lastBigReductionSize = 0, this.bigReductionCount = 0, this.stream = new h(t, i), this.tokens = new k(e, this.stream), this.topTerm = e.top[1];
		let {
			from: a
		} = i[0];
		this.stacks = [s.start(this, e.top[0], a)], this.fragments = O.length && this.stream.end - a > 4 * e.bufferLength ? new g(O, e.nodeSet) : null
	}
	get parsedPos() {
		return this.minStackPos
	}
	advance() {
		let e, t, O = this.stacks,
			s = this.minStackPos,
			i = this.stacks = [];
		...
		for(let a = 0; a < O.length; a++) {
			...
			for(;;) {
				if(this.tokens.mainToken = null, r.pos > s) i.push(r);
					...
					if(this.advanceStack(r, i, O)) continue;
					{
						e || (e = [], t = []), e.push(r);
						let O = this.tokens.getMainToken(r);
		...
		if(!i.length) {
			...
			if(t) return f && console.log("Finish with " + this.stackID(t)), this.stackToTree(t);
			if(this.parser.strict) throw f && e && console.log("Stuck with token " + (this.tokens.mainToken ? this.parser.getName(this.tokens.mainToken.value) : "none")), new SyntaxError("No parse at " + s);
			this.recovering || (this.recovering = 5)
		}
		if(this.recovering && e) {
			let O = null != this.stoppedAt && e[0].pos > this.stoppedAt ? e[0] : this.runRecovery(e, t, i);
			if(O) return f && console.log("Force-finish " + this.stackID(O)), this.stackToTree(O.forceAll())
		}
		if(this.recovering) {
			let e = 1 == this.recovering ? 1 : 3 * this.recovering;
			if(i.length > e)
				for(i.sort((e, t) => t.score - e.score); i.length > e;) i.pop();
			i.some(e => e.reducePos > s) && this.recovering--
		} else if(i.length > 1) {
			...
			i.length > 12 && (i.sort((e, t) => t.score - e.score), i.splice(12, i.length - 12))
		}
	...
	advanceStack(t, O, s) {
		...
		if(this.fragments) {
			let O = t.curContext && t.curContext.tracker.strict,
				s = O ? t.curContext.hash : 0;
			for(let o = this.fragments.nodeAt(i); o;) {
				let i = this.parser.nodeSet.types[o.type.id] == o.type ? a.getGoto(t.state, o.type.id) : -1;
				if(i > -1 && o.length && (!O || (o.prop(e.NodeProp.contextHash) || 0) == s)) return t.useNode(o, i), f && console.log(r + this.stackID(t) + ` (via reuse of ${a.getName(o.type.id)})`), !0;
				if(!(o instanceof e.Tree) || 0 == o.children.length || o.positions[0] > 0) break;
				let n = o.children[0];
				if(!(n instanceof e.Tree && 0 == o.positions[0])) break;
				o = n
			}
		}
		let o = a.stateSlot(t.state, 4);
		if(o > 0) return t.reduce(o), f && console.log(r + this.stackID(t) + ` (via always-reduce ${a.getName(65535&o)})`), !0;
		if(t.stack.length >= 8400)
			for(; t.stack.length > 6e3 && t.forceReduce(););
		let n = this.tokens.getActions(t);
		for(let e = 0; e < n.length;) {
			...
			if(Q.apply(o, l, d ? d.start : Q.pos, h), f && console.log(r + this.stackID(Q) + ` (via ${65536&o?`reduce of ${a.getName(65535&o)}`:"shift"} for ${a.getName(l)} @ ${i}${Q==t?"":", split"})`), c) return !0;
			Q.pos > i ? O.push(Q) : s.push(Q)
		}
		return !1
	}
	advanceFully(e, t) {
	...
	runRecovery(e, t, O) {
		...
		for(let a = 0; a < e.length; a++) {
			...
			if(r.deadEnd) {
				if(i) continue;
				if(i = !0, r.restart(), f && console.log(l + this.stackID(r) + " (restarted)"), this.advanceFully(r, O)) continue
			}
			let h = r.split(),
				c = l;
			for(let e = 0; e < 10 && h.forceReduce(); e++) {
				if(f && console.log(c + this.stackID(h) + " (via force-reduce)"), this.advanceFully(h, O)) break;
				f && (c = this.stackID(h) + " -> ")
			}
			for(let e of r.recoverByInsert(o)) f && console.log(l + this.stackID(e) + " (via recover-insert)"), this.advanceFully(e, O);
			this.stream.end > r.pos ? (n == r.pos && (n++, o = 0), r.recoverByDelete(o, n), f && console.log(l + this.stackID(r) + ` (via recover-delete ${this.parser.getName(o)})`), P(r, O)) : (!s || s.score < h.score) && (s = h)
		}
		return s
	}
	stackToTree(t) {
		return t.close(), e.Tree.build({
			buffer: r.create(t),
	...
	stackID(e) {
		let t = (m || (m = new WeakMap)).get(e);
		return t || m.set(e, t = String.fromCodePoint(this.nextStackID++)), t + e
	}
}
...
function P(e, t) {
	for(let O = 0; O < t.length; O++) {
		let s = t[O];
		if(s.pos == e.pos && s.sameState(e)) return void(t[O].score < e.score && (t[O] = e))
	}
	t.push(e)
}
class X {
...
const y = e => e;
class b extends e.Parser {
	constructor(t) {
		if(super(), this.wrappers = [], 14 != t.version) throw new RangeError(`Parser version (${t.version}) doesn't match runtime version (14)`);
		let O = t.nodeNames.split(" ");
		this.minRepeatTerm = O.length;
		for(let e = 0; e < t.repeatNodeCount; e++) O.push("");
		let s = Object.keys(t.topRules).map(e => t.topRules[e][1]),
			i = [];
		for(let e = 0; e < O.length; e++) i.push([]);

		function a(e, t, O) {
			i[e].push([t, t.deserialize(String(O))])
		}
		if(t.nodeProps)
			for(let O of t.nodeProps) {
				let t = O[0];
				"string" == typeof t && (t = e.NodeProp[t]);
				for(let e = 1; e < O.length;) {
					let s = O[e++];
					if(s >= 0) a(s, t, O[e++]);
					else {
						let i = O[e + -s];
						for(let r = -s; r > 0; r--) a(O[e++], t, i);
						e++
					}
				}
			}
		this.nodeSet = new e.NodeSet(O.map((O, a) => e.NodeType.define({
		...
		}))), t.propSources && (this.nodeSet = this.nodeSet.extend(...t.propSources)), this.strict = !1, this.bufferLength = e.DefaultBufferLength;
		let r = o(t.tokenData);
		...
		this.specializers = this.specializerSpecs.map(z), this.states = o(t.states, Uint32Array), this.data = o(t.stateData), this.goto = o(t.goto), this.maxTerm = t.maxTerm, this.tokenizers = t.tokenizers.map(e => "number" == typeof e ? new c(r, e) : e), this.topRules = t.topRules, this.dialects = t.dialects || {}, this.dynamicPrecedences = t.dynamicPrecedences || null, this.tokenPrecTable = t.tokenPrec, this.termNames = t.termNames || null, this.maxNode = this.nodeSet.types.length - 1, this.dialect = this.parseDialect(), this.top = this.topRules[Object.keys(this.topRules)[0]]
	}
	createParse(e, t, O) {
		let s = new R(this, e, t, O);
		for(let i of this.wrappers) s = i(s, e, t, O);
		return s
	}
	getGoto(e, t, O = !1) {
		let s = this.goto;
	...
	hasAction(e, t) {
		let O = this.data;
		for(let s = 0; s < 2; s++)
			for(let i, a = this.stateSlot(e, s ? 2 : 1);; a += 3) {
				if(65535 == (i = O[a])) {
					if(1 != O[a + 1]) {
						if(2 == O[a + 1]) return x(O, a + 2);
						break
					}
					i = O[a = x(O, a + 2)]
				}
				if(i == t || 0 == i) return x(O, a + 1)
			}
		return 0
	}
	stateSlot(e, t) {
	...
	allActions(e, t) {
		let O = this.stateSlot(e, 4),
			s = O ? t(O) : void 0;
		for(let O = this.stateSlot(e, 1); null == s; O += 3) {
	...
	configure(e) {
		let t = Object.assign(Object.create(b.prototype), this);
		...
		return e.tokenizers && (t.tokenizers = this.tokenizers.map(t => {
			...
			return O ? O.to : t
		})), e.specializers && (t.specializers = this.specializers.slice(), t.specializerSpecs = this.specializerSpecs.map((O, s) => {
			let i = e.specializers.find(e => e.from == O.external);
			if(!i) return O;
			let a = Object.assign(Object.assign({}, O), {
				external: i.to
			});
			return t.specializers[s] = z(a), a
	...
	parseDialect(e) {
		...
		for(let e = 0; e < t.length; e++)
			if(!O[e])
				for(let O, i = this.dialects[t[e]]; 65535 != (O = this.data[i++]);)(s || (s = new Uint8Array(this.maxTerm + 1)))[O] = 1;
		return new X(e, O, s)
	}
...
function z(e) {
	if(e.external) {
		let t = e.extend ? 1 : 0;
		return (O, s) => e.external(O, s) << 1 | t
	}
	return e.get
}
const w = [9, 10, 11, 12, 13, 32, 133, 160, 5760, 8192, 8193, 8194, 8195, 8196, 8197, 8198, 8199, 8200, 8201, 8202, 8232, 8233, 8239, 8287, 12288],
	v = 10;
...
function G(e) {
	let t;
	return 47 == e.next && (47 == (t = e.peek(1)) || 42 == t)
}
const j = new d((e, t) => {
		if(t.dialectEnabled(0)) {
			let O;
			if(e.next < 0 && t.canShift(176)) e.acceptToken(176);
			else if(((O = e.peek(-1)) == v || O < 0) && t.canShift(175)) {
				let t = 0;
				for(; e.next != v && w.includes(e.next);) e.advance(), t++;
				e.next == v || G(e) ? e.acceptToken(175, -t) : t && e.acceptToken(177)
			} else if(e.next == v) e.acceptToken(174, 1);
			else if(w.includes(e.next)) {
				for(e.advance(); e.next != v && w.includes(e.next);) e.advance();
				e.acceptToken(177)
			}
		} else {
			let t = 0;
			for(; w.includes(e.next);) e.advance(), t++;
			t && e.acceptToken(177)
		}
	}, {
		contextual: !0
	...
	q = new d((e, t) => {
		if(G(e)) {
			if(e.advance(), t.dialectEnabled(0)) {
				let t = -1;
				for(let O = 1;; O++) {
					let s = e.peek(-O - 1);
					if(s == v || s < 0) {
				...
				if(t > -1) {
					...
					for(e.advance(); e.next >= 0;)
						if(e.next == v) {
							e.advance();
							let O = 0;
							for(; e.next != v && w.includes(e.next);) O++, e.advance();
							if(O < t) {
								s = -O - 1;
								break
							}
						} else {
							if(O && 42 == e.next && 47 == e.peek(1)) {
								s = 2;
								break
							}
							e.advance()
						} return void e.acceptToken(O ? 7 : 6, s)
				}
			}
			if(47 == e.next) {
				for(; e.next != v && e.next >= 0;) e.advance();
				e.acceptToken(6)
			} else {
				for(e.advance(); e.next >= 0;) {
					let {
	...
	Z = new d((e, t) => {
		43 != e.next && 61 != e.next || !t.dialectEnabled(0) || e.acceptToken(61 == e.next ? 8 : 9, 1)
	}),
	U = new d((e, t) => {
		if(!t.dialectEnabled(0)) return;
		let O = t.context.depth;
		if(e.next < 0 && O) e.acceptToken(169);
		else if(e.peek(-1) == v) {
			let t = 0;
			for(; e.next != v && w.includes(e.next);) e.advance(), t++;
			t == O || e.next == v || G(e) || (t < O ? e.acceptToken(169, -t) : e.acceptToken(168))
		}
	}),
	_ = new d((e, t) => {
		for(let O = !1, s = 0, i = 0;; i++) {
			let {
				next: a
			} = e;
			if(!(Y(a) || 45 == a || 95 == a || O && T(a))) {
				if(35 == a && 123 == e.peek(1)) {
					e.acceptToken(5, 2);
					break
				}
				O && e.acceptToken(2 == s && t.canShift(4) ? 4 : t.canShift(173) ? 173 : 40 == a ? 171 : 172);
				break
			}!O && (45 != a || i > 0) && (O = !0), s === i && 45 == a && s++, e.advance()
		}
	}),
	C = new d(e => {
		if(125 == e.next) {
			for(e.advance(); Y(e.next) || 45 == e.next || 95 == e.next || T(e.next);) e.advance();
			35 == e.next && 123 == e.peek(1) ? e.acceptToken(2, 2) : e.acceptToken(1)
		}
	}),
	W = new d(e => {
		if(w.includes(e.peek(-1))) {
			let {
				next: t
			} = e;
			(Y(t) || 95 == t || 35 == t || 46 == t || 91 == t || 58 == t && Y(e.peek(1)) || 45 == t || 38 == t || 42 == t) && e.acceptToken(170)
		}
	}),
	E = new d(e => {
		if(!w.includes(e.peek(-1))) {
			...
			if(37 == t && (e.advance(), e.acceptToken(3)), Y(t)) {
				do {
					e.advance()
				} while(Y(e.next) || T(e.next));
				e.acceptToken(3)
			}
		}
	});
...
const V = new N(null, 0),
	I = new class {
		constructor(e) {
			this.start = e.start, this.shift = e.shift || y, this.reduce = e.reduce || y, this.reuse = e.reuse || y, this.hash = e.hash || (() => 0), this.strict = !1 !== e.strict
		}
	}({
		start: V,
		shift: (e, t, O, s) => 168 == t ? new N(e, O.pos - s.pos) : 169 == t ? e.parent : e,
		hash: e => e.hash
	}),
	F = t.styleTags({
		"AtKeyword import charset namespace keyframes media supports include mixin use forward extend at-root": t.tags.definitionKeyword,
		"Keyword selector": t.tags.keyword,
		ControlKeyword: t.tags.controlKeyword,
		...
		"ClassName Suffix": t.tags.className,
		...
		SassVariableName: t.tags.special(t.tags.variableName),
		Callee: t.tags.operatorKeyword,
		Unit: t.tags.unit,
		"UniversalSelector NestingSelector IndentedMixin IndentedInclude": t.tags.definitionOperator,
		MatchOp: t.tags.compareOperator,
		"ChildOp SiblingOp, LogicOp": t.tags.logicOperator,
		BinOp: t.tags.arithmeticOperator,
		"Important Global Default": t.tags.modifier,
		...
		"InterpolationStart InterpolationContinue InterpolationEnd": t.tags.meta,
		': "..."': t.tags.punctuation,
	...
	K = b.deserialize({
		...
		tokenizers: [U, W, C, E, _, j, q, Z, 0, 1, 2, 3, 4],
		topRules: {
			StyleSheet: [0, 10],
			Styles: [1, 145]
		},
	...
	J = [9, 10, 11, 12, 13, 32, 133, 160, 5760, 8192, 8193, 8194, 8195, 8196, 8197, 8198, 8199, 8200, 8201, 8202, 8232, 8233, 8239, 8287, 12288];
...
function ee(e) {
	return H(e) || e >= 97 && e <= 102 || e >= 65 && e <= 70
}
const te = (e, t, O) => (s, i) => {
		for(let a = !1, r = 0, o = 0;; o++) {
			let {
				next: n
			} = s;
			if(M(n) || 45 == n || 95 == n || a && H(n)) !a && (45 != n || o > 0) && (a = !0), r === o && 45 == n && r++, s.advance();
			else {
				if(92 != n || 10 == s.peek(1)) {
					a && s.acceptToken(2 == r && i.canShift(2) ? t : 40 == n ? O : e);
					break
				}
				if(s.advance(), ee(s.next)) {
					do {
						s.advance()
					} while(ee(s.next));
					32 == s.next && s.advance()
	...
	ie = new d(e => {
		if(J.includes(e.peek(-1))) {
			...
			(M(t) || 95 == t || 35 == t || 46 == t || 42 == t || 91 == t || 58 == t && M(e.peek(1)) || 45 == t || 38 == t) && e.acceptToken(122)
		}
	}),
	ae = new d(e => {
		if(!J.includes(e.peek(-1))) {
			...
			if(37 == t && (e.advance(), e.acceptToken(1)), M(t)) {
				do {
					e.advance()
				} while(M(e.next) || H(e.next));
				e.acceptToken(1)
			}
		}
	}),
	re = t.styleTags({
		"AtKeyword import charset namespace keyframes media supports": t.tags.definitionKeyword,
		...
		"MatchOp CompareOp": t.tags.compareOperator,
		"ChildOp SiblingOp, LogicOp": t.tags.logicOperator,
		BinOp: t.tags.arithmeticOperator,
...
b.deserialize({
	...
	tokenizers: [ie, ae, Oe, se, 1, 2, 3, 4, new Q("m~RRYZ[z{a~~g~aO#v~~dP!P!Qg~lO#w~~", 28, 129)],
	topRules: {
		StyleSheet: [0, 6],
		Styles: [1, 105]
	},
...
function de() {
	if(!Qe && "object" == typeof document && document.body) {
		let {
			style: e
		} = document.body, t = [], O = new Set;
		for(let s in e) "cssText" != s && "cssFloat" != s && "string" == typeof e[s] && (/[A-Z]/.test(s) && (s = s.replace(/[A-Z]/g, e => "-" + e.toLowerCase())), O.has(s) || (t.push(s), O.add(s)));
		Qe = t.sort().map(e => ({
			type: "property",
			label: e,
			apply: e + ": "
...
function Pe(t, O, s) {
	if(O.to - O.from > 4096) {
		...
		let a = [],
			r = new Set,
			o = O.cursor(e.IterMode.IncludeAnonymous);
		if(o.firstChild())
			do {
				for(let e of Pe(t, o.node, s)) r.has(e.label) || (r.add(e.label), a.push(e))
			} while(o.nextSibling());
		return ge.set(O, a), a
	} {
		let e = [],
		...
		return O.cursor().iterate(O => {
			var a;
			if(s(O) && O.matchContext(ke) && ":" == (null === (a = O.node.nextSibling) || void 0 === a ? void 0 : a.name)) {
				let s = t.sliceString(O.from, O.to);
				i.has(s) || (i.add(s), e.push({
					label: s,
					type: "variable"
				}))
			}
...
const Xe = e => t => {
		let {
			state: s,
			pos: i
		} = t, a = O.syntaxTree(s).resolveInner(i, -1), r = a.type.isError && a.from == a.to - 1 && "-" == s.doc.sliceString(a.from, a.to);
		if("PropertyName" == a.name || (r || "TagName" == a.name) && /^(Block|Styles)$/.test(a.resolve(a.to).name)) return {
			from: a.from,
			options: de(),
			validFor: me
		};
		if("ValueName" == a.name) return {
			from: a.from,
			options: ue,
			validFor: me
		};
		if("PseudoClassName" == a.name) return {
			from: a.from,
			options: $e,
			validFor: me
		};
		if(e(a) || (t.explicit || r) && function(e, t) {
				var O;
				if(("(" == e.name || e.type.isError) && (e = e.parent || e), "ArgList" != e.name) return !1;
				let s = null === (O = e.parent) || void 0 === O ? void 0 : O.firstChild;
				return "Callee" == (null == s ? void 0 : s.name) && "var" == t.sliceString(s.from, s.to)
			}(a, s.doc)) return {
			from: e(a) || r ? a.from : i,
			options: Pe(s.doc, Re(a), e),
			validFor: Se
		};
		if("TagName" == a.name) {
			for(let {
		...
		let o = a.resolve(i),
			n = o.childBefore(i);
		return n && ":" == n.name && "PseudoClassSelector" == o.name ? {
			from: i,
			options: $e,
			validFor: me
		} : n && ":" == n.name && "Declaration" == o.name || "ArgList" == o.name ? {
			from: i,
			options: ue,
			validFor: me
	...
	ye = O.LRLanguage.define({
		...
		parser: K.configure({
			props: [O.foldNodeProp.add({
				...
				Comment: (e, t) => ({
					from: e.from + 2,
					to: "*/" == t.sliceDoc(e.to - 2, e.to) ? e.to - 2 : e.to
				})
			}), O.indentNodeProp.add({
				Declaration: O.continuedIndent()
			})]
		}),
		languageData: {
			commentTokens: {
	...
	be = ye.configure({
		...
		props: [O.indentNodeProp.add({
			"Block RuleSet": e => e.baseIndent + e.unit
		}), O.foldNodeProp.add({
			Block: e => ({
				from: e.from,
				to: e.to
	...
	xe = Xe(e => "VariableName" == e.name || "SassVariableName" == e.name);
exports.sass = function(e) {
	return new O.LanguageSupport((null == e ? void 0 : e.indented) ? be : ye, ye.data.of({
		autocomplete: xe
	}))
}, exports.sassCompletionSource = xe, exports.sassLanguage = ye;
