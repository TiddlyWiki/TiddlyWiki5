"use strict";
var t = require("$:/plugins/tiddlywiki/codemirror-6/lib/codemirror-language.js"),
...
function k(t, e, i) {
	let r = t.cursor(s.IterMode.IncludeAnonymous);
	for(r.moveTo(e);;)
		if(!(i < 0 ? r.childBefore(e) : r.childAfter(e)))
			for(;;) {
				if((i < 0 ? r.to < e : r.from > e) && !r.type.isError) return i < 0 ? Math.max(0, Math.min(r.to - 1, e - 25)) : Math.min(t.length, Math.max(r.from + 1, e + 25));
...
class m {
	...
	nodeAt(t) {
		...
		for(;;) {
			...
			let h = i.children[r],
				n = this.start[e] + i.positions[r];
			if(n > t) return this.nextStart = n, null;
			if(h instanceof s.Tree) {
				if(n == t) {
					if(n < this.safeFrom) return null;
					let t = n + h.length;
					if(t <= this.safeTo) {
						let e = h.prop(s.NodeProp.lookAhead);
	...
	addActions(t, e, s, i) {
		...
		for(let t = 0; t < 2; t++)
			for(let o = h.stateSlot(r, t ? 2 : 1);; o += 3) {
				if(65535 == n[o]) {
					if(1 != n[o + 1]) {
						0 == i && 2 == n[o + 1] && (i = this.putAction(T(n, o + 2), e, s, i));
						break
					}
					o = T(n, o + 2)
				}
				n[o] == e && (i = this.putAction(T(n, o + 1), e, s, i))
			}
		return i
	}
}
class x {
	constructor(t, e, s, r) {
		this.parser = t, this.input = e, this.ranges = r, this.recovering = 0, this.nextStackID = 9812, this.minStackPos = 0, this.reused = [], this.stoppedAt = null, this.lastBigReductionStart = -1, this.lastBigReductionSize = 0, this.bigReductionCount = 0, this.stream = new c(e, r), this.tokens = new b(t, this.stream), this.topTerm = t.top[1];
		let {
			from: h
		} = r[0];
	...
	advanceStack(t, e, i) {
		...
		if(this.fragments) {
			...
			for(let o = this.fragments.nodeAt(r); o;) {
				let r = this.parser.nodeSet.types[o.type.id] == o.type ? h.getGoto(t.state, o.type.id) : -1;
				if(r > -1 && o.length && (!e || (o.prop(s.NodeProp.contextHash) || 0) == i)) return t.useNode(o, r), d && console.log(n + this.stackID(t) + ` (via reuse of ${h.getName(o.type.id)})`), !0;
				if(!(o instanceof s.Tree) || 0 == o.children.length || o.positions[0] > 0) break;
				let a = o.children[0];
				if(!(a instanceof s.Tree && 0 == o.positions[0])) break;
				o = a
			}
		}
		let o = h.stateSlot(t.state, 4);
		if(o > 0) return t.reduce(o), d && console.log(n + this.stackID(t) + ` (via always-reduce ${h.getName(65535&o)})`), !0;
		...
		for(let s = 0; s < a.length;) {
			...
			if(f.apply(o, l, p ? p.start : f.pos, c), d && console.log(n + this.stackID(f) + ` (via ${65536&o?`reduce of ${h.getName(65535&o)}`:"shift"} for ${h.getName(l)} @ ${r}${f==t?"":", split"})`), u) return !0;
			f.pos > r ? e.push(f) : i.push(f)
		}
		return !1
	}
	advanceFully(t, e) {
	...
	runRecovery(t, e, s) {
		...
		for(let h = 0; h < t.length; h++) {
			...
			this.stream.end > n.pos ? (a == n.pos && (a++, o = 0), n.recoverByDelete(o, a), d && console.log(l + this.stackID(n) + ` (via recover-delete ${this.parser.getName(o)})`), S(n, s)) : (!i || i.score < c.score) && (i = c)
		}
		return i
	}
	stackToTree(t) {
		return t.close(), s.Tree.build({
			buffer: n.create(t),
	...
	constructor(t) {
		...
		if(t.nodeProps)
			for(let e of t.nodeProps) {
				let t = e[0];
				"string" == typeof t && (t = s.NodeProp[t]);
				for(let s = 1; s < e.length;) {
					let i = e[s++];
					if(i >= 0) h(i, t, e[s++]);
					else {
						let r = e[s + -i];
						for(let n = -i; n > 0; n--) h(e[s++], t, r);
						s++
					}
				}
			}
		this.nodeSet = new s.NodeSet(e.map((e, h) => s.NodeType.define({
		...
		}))), t.propSources && (this.nodeSet = this.nodeSet.extend(...t.propSources)), this.strict = !1, this.bufferLength = s.DefaultBufferLength;
		let n = o(t.tokenData);
	...
	hasAction(t, e) {
		...
		for(let i = 0; i < 2; i++)
			for(let r, h = this.stateSlot(t, i ? 2 : 1);; h += 3) {
				if(65535 == (r = s[h])) {
					if(1 != s[h + 1]) {
						if(2 == s[h + 1]) return T(s, h + 2);
						break
					}
					r = s[h = T(s, h + 2)]
				}
				if(r == e || 0 == r) return T(s, h + 1)
			}
		return 0
	}
	stateSlot(t, e) {
...
const R = {
	...
	y = t.LRLanguage.define({
		name: "wast",
		parser: C.configure({
			props: [t.indentNodeProp.add({
				App: t.delimitedIndent({
					closing: ")",
					align: !1
				})
			}), t.foldNodeProp.add({
				...
				Keyword: e.tags.keyword,
				Type: e.tags.typeName,
...
exports.wast = function() {
	return new t.LanguageSupport(y)
}, exports.wastLanguage = y;
