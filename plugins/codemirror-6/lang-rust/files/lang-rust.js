...
class i {
	constructor(O, e, t, i, s, Q, n, r, a, P = 0, $) {
		this.p = O, this.stack = e, this.state = t, this.reducePos = i, this.pos = s, this.score = Q, this.buffer = n, this.bufferBase = r, this.curContext = a, this.lookAhead = P, this.parent = $
	}
	toString() {
		return `[${this.stack.filter((O,e)=>e%3==0).concat(this.state)}]@${this.pos}${this.score?"!"+this.score:""}`
	}
	static start(O, e, t = 0) {
		let Q = O.parser.context;
		return new i(O, [], e, t, t, 0, [], 0, Q ? new s(Q, Q.start) : null, 0, null)
	}
	get context() {
	...
	reduce(O) {
		...
		let t = O >> 19,
			...
			Q = this.reducePos < this.pos - 25 && this.setLookAhead(this.pos),
			n = s.dynamicPrecedence(i);
		if(n && (this.score += n), 0 == t) return this.pushState(s.getGoto(this.state, i, !0), this.reducePos), i < s.minRepeatTerm && this.storeNode(i, this.reducePos, this.reducePos, Q ? 8 : 4, !0), void this.reduceContext(i, this.reducePos);
		let r = this.stack.length - 3 * (t - 1) - (262144 & O ? 6 : 0),
			a = r ? this.stack[r - 2] : this.p.ranges[0].from,
			P = this.reducePos - a;
		P >= 2e3 && !(null === (e = this.p.parser.nodeSet.types[i]) || void 0 === e ? void 0 : e.isAnonymous) && (a == this.p.lastBigReductionStart ? (this.p.bigReductionCount++, this.p.lastBigReductionSize = P) : this.p.lastBigReductionSize < P && (this.p.bigReductionCount = 1, this.p.lastBigReductionStart = a, this.p.lastBigReductionSize = P));
		let $ = r ? this.stack[r - 1] : 0,
			o = this.bufferBase + this.buffer.length - $;
		if(i < s.minRepeatTerm || 131072 & O) {
			let O = s.stateFlag(this.state, 1) ? this.pos : this.reducePos;
			this.storeNode(i, a, O, o + 4, !0)
		}
		if(262144 & O) this.state = this.stack[r];
		else {
			let O = this.stack[r - 3];
			this.state = s.getGoto(O, i, !0)
		}
		for(; this.stack.length > r;) this.stack.pop();
		this.reduceContext(i, a)
	}
	storeNode(O, e, t, i = 4, s = !1) {
		...
		if(s && this.pos != t) {
			...
			if(s > 0 && (0 != this.buffer[s - 4] || this.buffer[s - 1] < 0)) {
				let O = !1;
				for(let e = s; e > 0 && this.buffer[e - 2] > t; e -= 4)
					if(this.buffer[e - 1] >= 0) {
						O = !0;
						break
					} if(O)
					for(; s > 0 && this.buffer[s - 2] > t;) this.buffer[s] = this.buffer[s - 4], this.buffer[s + 1] = this.buffer[s - 3], this.buffer[s + 2] = this.buffer[s - 2], this.buffer[s + 3] = this.buffer[s - 1], s -= 4, i > 4 && (i -= 4)
			}
			this.buffer[s] = O, this.buffer[s + 1] = e, this.buffer[s + 2] = t, this.buffer[s + 3] = i
	...
	shift(O, e, t, i) {
		if(131072 & O) this.pushState(65535 & O, this.pos);
			...
			this.pos = i, !Q.stateFlag(s, 1) && (i > t || e <= Q.maxNode) && (this.reducePos = i), this.pushState(s, Math.min(t, this.reducePos)), this.shiftContext(e, t), e <= Q.maxNode && this.buffer.push(e, t, i, 4)
		}
	}
	apply(O, e, t, i) {
	...
	split() {
		...
		return new i(this.p, this.stack.slice(), this.state, this.reducePos, this.pos, this.score, t, s, this.curContext, this.lookAhead, O)
	}
	recoverByDelete(O, e) {
	...
	recoverByInsert(O) {
		...
		if(e.length > 8 || this.stack.length >= 120) {
			let t = [];
			for(let i, s = 0; s < e.length; s += 2)(i = e[s + 1]) != this.state && this.p.parser.hasAction(i, O) && t.push(e[s], i);
		...
		for(let O = 0; O < e.length && t.length < 4; O += 2) {
			...
			let s = this.split();
			s.pushState(i, this.pos), s.storeNode(0, s.pos, s.pos, 4, !0), s.shiftContext(e[O], this.pos), s.reducePos = this.pos, s.score -= 200, t.push(s)
		}
		return t
	}
	forceReduce() {
			...
			if(!e.includes(i)) return e.push(i), O.allActions(i, e => {
				if(393216 & e);
					...
					if(t > 1) {
						let i = 65535 & e,
							s = this.stack.length - 3 * t;
						if(s >= 0 && O.getGoto(this.stack[s], i, !1) >= 0) return t << 19 | 65536 | i
					}
				} else {
					let O = t(e, s + 1);
					if(null != O) return O
	...
	updateContext(O) {
		if(O != this.curContext.context) {
			let e = new s(this.curContext.tracker, O);
...
class n {
	...
	static create(O, e = O.bufferBase + O.buffer.length) {
		return new n(O, e, e - O.bufferBase)
	}
	maybeNext() {
...
function r(O, e = Uint16Array) {
	if("string" != typeof O) return O;
	let t = null;
	for(let i = 0, s = 0; i < O.length;) {
		let Q = 0;
		for(;;) {
...
class $ {
	constructor(O, e) {
		this.input = O, this.ranges = e, this.chunk = "", this.chunkOff = 0, this.chunk2 = "", this.chunk2Pos = 0, this.next = -1, this.token = P, this.rangeIndex = 0, this.pos = this.chunkPos = e[0].from, this.range = e[0], this.end = e[e.length - 1].to, this.readNext()
	}
	resolveOffset(O, e) {
		let t = this.range,
			i = this.rangeIndex,
	...
	reset(O, e) {
		if(e ? (this.token = e, e.start = O, e.lookAhead = O + 1, e.value = e.extended = -1) : this.token = P, this.pos != O) {
...
class o {
	...
	token(O, e) {
		...
		! function(O, e, t, i, s, Q) {
			...
			O: for(; 0 != (r & O[n]);) {
				let t = O[n + 1];
				for(let i = n + 3; i < t; i += 2)
					if((O[i + 1] & r) > 0) {
						let t = O[i];
						if(a.allows(t) && (-1 == e.token.value || e.token.value == t || l(t, e.token.value, s, Q))) {
							e.acceptToken(t);
							break
						}
					} let i = e.next,
		...
		}(this.data, O, e, this.id, t.data, t.tokenPrecTable)
	}
}
o.prototype.contextual = o.prototype.fallback = o.prototype.extend = !1, o.prototype.fallback = o.prototype.extend = !1;
class X {
	constructor(O, e = {}) {
...
function l(O, e, t, i) {
	let s = h(t, i, e);
	return s < 0 || h(t, i, O) < s
}
const S = "undefined" != typeof process && process.env && /\bparse\b/.test(process.env.LOG);
let c = null;

function f(e, t, i) {
	let s = e.cursor(O.IterMode.IncludeAnonymous);
	for(s.moveTo(t);;)
		if(!(i < 0 ? s.childBefore(t) : s.childAfter(t)))
			for(;;) {
				if((i < 0 ? s.to < t : s.from > t) && !s.type.isError) return i < 0 ? Math.max(0, Math.min(s.to - 1, t - 25)) : Math.min(e.length, Math.max(s.from + 1, t + 25));
				if(i < 0 ? s.prevSibling() : s.nextSibling()) break;
				if(!s.parent()) return i < 0 ? 0 : e.length
			}
}
class p {
	constructor(O, e) {
	...
	nodeAt(e) {
		...
		for(;;) {
			...
			let Q = i.children[s],
				n = this.start[t] + i.positions[s];
			if(n > e) return this.nextStart = n, null;
			if(Q instanceof O.Tree) {
				if(n == e) {
					if(n < this.safeFrom) return null;
					let e = n + Q.length;
					if(e <= this.safeTo) {
						let t = Q.prop(O.NodeProp.lookAhead);
						if(!t || e + t < this.fragment.to) return Q
					}
				}
				this.index[t]++, n + Q.length >= Math.max(this.safeFrom, e) && (this.trees.push(Q), this.start.push(n), this.index.push(0))
			} else this.index[t]++, this.nextStart = n + Q.length
		}
	}
}
class u {
	constructor(O, e) {
		this.stream = e, this.tokens = [], this.mainToken = null, this.actions = [], this.tokens = O.tokenizers.map(O => new a)
	}
	getActions(O) {
		let e = 0,
			t = null,
			...
			Q = i.stateSlot(O.state, 3),
			n = O.curContext ? O.curContext.hash : 0,
			r = 0;
		for(let i = 0; i < s.length; i++) {
			if(!(1 << i & Q)) continue;
			let a = s[i],
				P = this.tokens[i];
			if((!t || a.fallback) && ((a.contextual || P.start != O.pos || P.mask != Q || P.context != n) && (this.updateCachedToken(P, a, O), P.mask = Q, P.context = n), P.lookAhead > P.end + 25 && (r = Math.max(P.lookAhead, r)), 0 != P.value)) {
				let i = e;
				if(P.extended > -1 && (e = this.addActions(O, P.extended, P.end, e)), e = this.addActions(O, P.value, P.end, e), !a.extend && (t = P, e > i)) break
			}
		}
		for(; this.actions.length > e;) this.actions.pop();
		return r && O.setLookAhead(r), t || O.pos != this.stream.end || (t = new a, t.value = O.p.parser.eofTerm, t.start = t.end = O.pos, e = this.addActions(O, t.value, t.end, e)), this.mainToken = t, this.actions
	}
	getMainToken(O) {
		...
		if(e.token(this.stream.reset(i, O), t), O.value > -1) {
			...
			for(let i = 0; i < e.specialized.length; i++)
				if(e.specialized[i] == O.value) {
					let s = e.specializers[i](this.stream.read(O.start, O.end), t);
					if(s >= 0 && t.p.parser.dialect.allows(s >> 1)) {
						1 & s ? O.extended = s >> 1 : O.value = s >> 1;
						break
					}
				}
	...
	addActions(O, e, t, i) {
		...
		for(let O = 0; O < 2; O++)
			for(let r = Q.stateSlot(s, O ? 2 : 1);; r += 3) {
				if(65535 == n[r]) {
					if(1 != n[r + 1]) {
						0 == i && 2 == n[r + 1] && (i = this.putAction(Y(n, r + 2), e, t, i));
						break
					}
					r = Y(n, r + 2)
				}
				n[r] == e && (i = this.putAction(Y(n, r + 1), e, t, i))
			}
		return i
	}
}
class g {
	constructor(O, e, t, s) {
		this.parser = O, this.input = e, this.ranges = s, this.recovering = 0, this.nextStackID = 9812, this.minStackPos = 0, this.reused = [], this.stoppedAt = null, this.lastBigReductionStart = -1, this.lastBigReductionSize = 0, this.bigReductionCount = 0, this.stream = new $(e, s), this.tokens = new u(O, this.stream), this.topTerm = O.top[1];
		let {
			from: Q
		} = s[0];
		this.stacks = [i.start(this, O.top[0], Q)], this.fragments = t.length && this.stream.end - Q > 4 * O.bufferLength ? new p(t, O.nodeSet) : null
	}
	get parsedPos() {
	...
	advance() {
		...
		for(let Q = 0; Q < t.length; Q++) {
			let n = t[Q];
			for(;;) {
				if(this.tokens.mainToken = null, n.pos > i) s.push(n);
				else {
		...
		if(!s.length) {
			...
			if(e) return S && console.log("Finish with " + this.stackID(e)), this.stackToTree(e);
			if(this.parser.strict) throw S && O && console.log("Stuck with token " + (this.tokens.mainToken ? this.parser.getName(this.tokens.mainToken.value) : "none")), new SyntaxError("No parse at " + i);
			this.recovering || (this.recovering = 5)
		}
		if(this.recovering && O) {
			let t = null != this.stoppedAt && O[0].pos > this.stoppedAt ? O[0] : this.runRecovery(O, e, s);
			if(t) return S && console.log("Force-finish " + this.stackID(t)), this.stackToTree(t.forceAll())
		}
		if(this.recovering) {
			let O = 1 == this.recovering ? 1 : 3 * this.recovering;
			if(s.length > O)
				for(s.sort((O, e) => e.score - O.score); s.length > O;) s.pop();
			s.some(O => O.reducePos > i) && this.recovering--
		} else if(s.length > 1) {
			O: for(let O = 0; O < s.length - 1; O++) {
				let e = s[O];
				for(let t = O + 1; t < s.length; t++) {
					let i = s[t];
			...
			s.length > 12 && (s.sort((O, e) => e.score - O.score), s.splice(12, s.length - 12))
		}
	...
	advanceStack(e, t, i) {
		...
		if(this.fragments) {
			...
			for(let r = this.fragments.nodeAt(s); r;) {
				let s = this.parser.nodeSet.types[r.type.id] == r.type ? Q.getGoto(e.state, r.type.id) : -1;
				if(s > -1 && r.length && (!t || (r.prop(O.NodeProp.contextHash) || 0) == i)) return e.useNode(r, s), S && console.log(n + this.stackID(e) + ` (via reuse of ${Q.getName(r.type.id)})`), !0;
				if(!(r instanceof O.Tree) || 0 == r.children.length || r.positions[0] > 0) break;
				let a = r.children[0];
				if(!(a instanceof O.Tree && 0 == r.positions[0])) break;
				r = a
			}
		}
		let r = Q.stateSlot(e.state, 4);
		if(r > 0) return e.reduce(r), S && console.log(n + this.stackID(e) + ` (via always-reduce ${Q.getName(65535&r)})`), !0;
		...
		for(let O = 0; O < a.length;) {
			...
			if(X.apply(r, P, h ? h.start : X.pos, $), S && console.log(n + this.stackID(X) + ` (via ${65536&r?`reduce of ${Q.getName(65535&r)}`:"shift"} for ${Q.getName(P)} @ ${s}${X==e?"":", split"})`), o) return !0;
			X.pos > s ? t.push(X) : i.push(X)
		}
		return !1
	}
	advanceFully(O, e) {
	...
	runRecovery(O, e, t) {
		...
		for(let Q = 0; Q < O.length; Q++) {
			...
			if(n.deadEnd) {
				if(s) continue;
				if(s = !0, n.restart(), S && console.log(P + this.stackID(n) + " (restarted)"), this.advanceFully(n, t)) continue
			}
			let $ = n.split(),
				o = P;
			for(let O = 0; O < 10 && $.forceReduce(); O++) {
				if(S && console.log(o + this.stackID($) + " (via force-reduce)"), this.advanceFully($, t)) break;
				S && (o = this.stackID($) + " -> ")
			}
			for(let O of n.recoverByInsert(r)) S && console.log(P + this.stackID(O) + " (via recover-insert)"), this.advanceFully(O, t);
			this.stream.end > n.pos ? (a == n.pos && (a++, r = 0), n.recoverByDelete(r, a), S && console.log(P + this.stackID(n) + ` (via recover-delete ${this.parser.getName(r)})`), d(n, t)) : (!i || i.score < $.score) && (i = $)
		}
		return i
	}
	stackToTree(e) {
	...
	stackID(O) {
		let e = (c || (c = new WeakMap)).get(O);
		return e || c.set(O, e = String.fromCodePoint(this.nextStackID++)), e + O
	}
}
...
class z extends O.Parser {
	constructor(e) {
		...
		let i = Object.keys(e.topRules).map(O => e.topRules[O][1]),
			s = [];
		for(let O = 0; O < t.length; O++) s.push([]);

		function Q(O, e, t) {
			s[O].push([e, e.deserialize(String(t))])
		}
		if(e.nodeProps)
			for(let t of e.nodeProps) {
				...
				for(let O = 1; O < t.length;) {
					...
					if(i >= 0) Q(i, e, t[O++]);
						...
						let s = t[O + -i];
						for(let n = -i; n > 0; n--) Q(t[O++], e, s);
						O++
					}
				}
			}
		this.nodeSet = new O.NodeSet(t.map((t, Q) => O.NodeType.define({
			name: Q >= this.minRepeatTerm ? void 0 : t,
			id: Q,
			props: s[Q],
			top: i.indexOf(Q) > -1,
			error: 0 == Q,
			skipped: e.skippedNodes && e.skippedNodes.indexOf(Q) > -1
		...
		this.specializers = this.specializerSpecs.map(b), this.states = r(e.states, Uint32Array), this.data = r(e.stateData), this.goto = r(e.goto), this.maxTerm = e.maxTerm, this.tokenizers = e.tokenizers.map(O => "number" == typeof O ? new o(n, O) : O), this.topRules = e.topRules, this.dialects = e.dialects || {}, this.dynamicPrecedences = e.dynamicPrecedences || null, this.tokenPrecTable = e.tokenPrec, this.termNames = e.termNames || null, this.maxNode = this.nodeSet.types.length - 1, this.dialect = this.parseDialect(), this.top = this.topRules[Object.keys(this.topRules)[0]]
	}
	createParse(O, e, t) {
		let i = new g(this, O, e, t);
		for(let s of this.wrappers) i = s(i, O, e, t);
		return i
	}
	getGoto(O, e, t = !1) {
		let i = this.goto;
	...
	hasAction(O, e) {
		let t = this.data;
		for(let i = 0; i < 2; i++)
			for(let s, Q = this.stateSlot(O, i ? 2 : 1);; Q += 3) {
				if(65535 == (s = t[Q])) {
					if(1 != t[Q + 1]) {
						if(2 == t[Q + 1]) return Y(t, Q + 2);
						break
					}
					s = t[Q = Y(t, Q + 2)]
				}
				if(s == e || 0 == s) return Y(t, Q + 1)
			}
		return 0
	}
	stateSlot(O, e) {
	...
	configure(O) {
		let e = Object.assign(Object.create(z.prototype), this);
		...
		return O.tokenizers && (e.tokenizers = this.tokenizers.map(e => {
			...
			let s = O.specializers.find(O => O.from == t.external);
			if(!s) return t;
			let Q = Object.assign(Object.assign({}, t), {
				external: s.to
			});
			return e.specializers[i] = b(Q), Q
	...
	parseDialect(O) {
		...
		for(let O = 0; O < e.length; O++)
			if(!t[O])
				for(let t, s = this.dialects[e[O]]; 65535 != (t = this.data[s++]);)(i || (i = new Uint8Array(this.maxTerm + 1)))[t] = 1;
		return new W(O, t, i)
	}
...
const _ = new X((O, e) => {
		if(Z(O.next)) {
			...
			if(102 == O.next) {
				let t = O.peek(1);
				if(!(51 == t && 50 == O.peek(2) || 54 == t && 52 == O.peek(2))) return;
				O.advance(3), e = !0
			}
			e && O.acceptToken(5)
	...
	R = e.styleTags({
		"const macro_rules struct union enum type fn impl trait let static": e.tags.definitionKeyword,
		"mod use crate": e.tags.moduleKeyword,
		"pub unsafe async mut extern default move": e.tags.modifier,
		"for if else loop while match continue break return await": e.tags.controlKeyword,
		"as in ref": e.tags.operatorKeyword,
		"where _ crate super dyn": e.tags.keyword,
		self: e.tags.self,
		String: e.tags.string,
		Char: e.tags.character,
		RawString: e.tags.special(e.tags.string),
		Boolean: e.tags.bool,
		Identifier: e.tags.variableName,
		"CallExpression/Identifier": e.tags.function(e.tags.variableName),
		BoundIdentifier: e.tags.definition(e.tags.variableName),
		"FunctionItem/BoundIdentifier": e.tags.function(e.tags.definition(e.tags.variableName)),
		LoopLabel: e.tags.labelName,
		FieldIdentifier: e.tags.propertyName,
		"CallExpression/FieldExpression/FieldIdentifier": e.tags.function(e.tags.propertyName),
		Lifetime: e.tags.special(e.tags.variableName),
		ScopeIdentifier: e.tags.namespace,
		TypeIdentifier: e.tags.typeName,
		"MacroInvocation/Identifier MacroInvocation/ScopedIdentifier/Identifier": e.tags.macroName,
		"MacroInvocation/TypeIdentifier MacroInvocation/ScopedIdentifier/TypeIdentifier": e.tags.macroName,
		'"!"': e.tags.macroName,
		UpdateOp: e.tags.updateOperator,
		LineComment: e.tags.lineComment,
		BlockComment: e.tags.blockComment,
		Integer: e.tags.integer,
		Float: e.tags.float,
		...
		"=": e.tags.definitionOperator,
		".. ... => ->": e.tags.punctuation,
		"( )": e.tags.paren,
		"[ ]": e.tags.squareBracket,
		"{ }": e.tags.brace,
		". DerefOp": e.tags.derefOperator,
		"&": e.tags.operator,
		", ; ::": e.tags.separator,
		"Attribute/...": e.tags.meta
	}),
	x = {
		__proto__: null,
		self: 28,
	...
	v = t.LRLanguage.define({
		name: "rust",
		parser: T.configure({
			props: [t.indentNodeProp.add({
				IfExpression: t.continuedIndent({
					except: /^\s*({|else\b)/
				}),
				"String BlockComment": () => null,
				AttributeItem: O => O.continue(),
				"Statement MatchArm": t.continuedIndent()
			}), t.foldNodeProp.add(O => /(Block|edTokens|List)$/.test(O.name) ? t.foldInside : "BlockComment" == O.name ? O => ({
				from: O.from + 2,
				to: O.to - 2
			}) : void 0)]
		}),
...
exports.rust = function() {
	return new t.LanguageSupport(v)
}, exports.rustLanguage = v;
