...
class i {
	constructor(t, e, s, i, r, n, o, h, a, O = 0, l) {
		this.p = t, this.stack = e, this.state = s, this.reducePos = i, this.pos = r, this.score = n, this.buffer = o, this.bufferBase = h, this.curContext = a, this.lookAhead = O, this.parent = l
	}
	toString() {
	...
	static start(t, e, s = 0) {
		...
		return new i(t, [], e, s, s, 0, [], 0, n ? new r(n, n.start) : null, 0, null)
	}
	get context() {
	...
	reduce(t) {
		...
		if(o && (this.score += o), 0 == s) return this.pushState(r.getGoto(this.state, i, !0), this.reducePos), i < r.minRepeatTerm && this.storeNode(i, this.reducePos, this.reducePos, n ? 8 : 4, !0), void this.reduceContext(i, this.reducePos);
		let h = this.stack.length - 3 * (s - 1) - (262144 & t ? 6 : 0),
			a = h ? this.stack[h - 2] : this.p.ranges[0].from,
			O = this.reducePos - a;
		O >= 2e3 && !(null === (e = this.p.parser.nodeSet.types[i]) || void 0 === e ? void 0 : e.isAnonymous) && (a == this.p.lastBigReductionStart ? (this.p.bigReductionCount++, this.p.lastBigReductionSize = O) : this.p.lastBigReductionSize < O && (this.p.bigReductionCount = 1, this.p.lastBigReductionStart = a, this.p.lastBigReductionSize = O));
		let l = h ? this.stack[h - 1] : 0,
	...
	shift(t, e, s, i) {
		if(131072 & t) this.pushState(65535 & t, this.pos);
			...
			this.pos = i, !n.stateFlag(r, 1) && (i > s || e <= n.maxNode) && (this.reducePos = i), this.pushState(r, Math.min(s, this.reducePos)), this.shiftContext(e, s), e <= n.maxNode && this.buffer.push(e, s, i, 4)
		}
	}
	apply(t, e, s, i) {
	...
	split() {
		...
		return new i(this.p, this.stack.slice(), this.state, this.reducePos, this.pos, this.score, s, r, this.curContext, this.lookAhead, t)
	}
	recoverByDelete(t, e) {
		...
		! function(t, e, s, i, r, n) {
			...
			t: for(; 0 != (h & t[o]);) {
				...
				for(let i = o + 3; i < s; i += 2)
					if((t[i + 1] & h) > 0) {
						let s = t[i];
						if(a.allows(s) && (-1 == e.token.value || e.token.value == s || u(s, e.token.value, r, n))) {
							e.acceptToken(s);
							break
						}
					} let i = e.next,
					O = 0,
					l = t[o + 2];
				if(!(e.next < 0 && l > O && 65535 == t[s + 3 * l - 3])) {
					for(; O < l;) {
						let r = O + l >> 1,
							n = s + r + (r << 1),
							h = t[n],
		...
		for(;;) {
			...
			if(n instanceof t.Tree) {
				if(o == e) {
					...
					let e = o + n.length;
					if(e <= this.safeTo) {
						let s = n.prop(t.NodeProp.lookAhead);
						if(!s || e + s < this.fragment.to) return n
					}
				}
				this.index[s]++, o + n.length >= Math.max(this.safeFrom, e) && (this.trees.push(n), this.start.push(o), this.index.push(0))
			} else this.index[s]++, this.nextStart = o + n.length
		}
	}
}
class P {
	constructor(t, e) {
		this.stream = e, this.tokens = [], this.mainToken = null, this.actions = [], this.tokens = t.tokenizers.map(t => new a)
	}
	getActions(t) {
		let e = 0,
			s = null,
		...
		for(let i = 0; i < r.length; i++) {
			...
			let a = r[i],
				O = this.tokens[i];
			if((!s || a.fallback) && ((a.contextual || O.start != t.pos || O.mask != n || O.context != o) && (this.updateCachedToken(O, a, t), O.mask = n, O.context = o), O.lookAhead > O.end + 25 && (h = Math.max(O.lookAhead, h)), 0 != O.value)) {
				let i = e;
				if(O.extended > -1 && (e = this.addActions(t, O.extended, O.end, e)), e = this.addActions(t, O.value, O.end, e), !a.extend && (s = O, e > i)) break
			}
		}
		for(; this.actions.length > e;) this.actions.pop();
		return h && t.setLookAhead(h), s || t.pos != this.stream.end || (s = new a, s.value = t.p.parser.eofTerm, s.start = s.end = t.pos, e = this.addActions(t, s.value, s.end, e)), this.mainToken = s, this.actions
	}
	getMainToken(t) {
	...
	addActions(t, e, s, i) {
		...
		for(let t = 0; t < 2; t++)
			for(let h = n.stateSlot(r, t ? 2 : 1);; h += 3) {
				if(65535 == o[h]) {
					if(1 != o[h + 1]) {
						0 == i && 2 == o[h + 1] && (i = this.putAction($(o, h + 2), e, s, i));
						break
					}
					h = $(o, h + 2)
				}
				o[h] == e && (i = this.putAction($(o, h + 1), e, s, i))
			}
		return i
	}
}
class Q {
	constructor(t, e, s, r) {
		this.parser = t, this.input = e, this.ranges = r, this.recovering = 0, this.nextStackID = 9812, this.minStackPos = 0, this.reused = [], this.stoppedAt = null, this.lastBigReductionStart = -1, this.lastBigReductionSize = 0, this.bigReductionCount = 0, this.stream = new l(e, r), this.tokens = new P(t, this.stream), this.topTerm = t.top[1];
		let {
			from: n
		} = r[0];
		this.stacks = [i.start(this, t.top[0], n)], this.fragments = s.length && this.stream.end - n > 4 * t.bufferLength ? new k(s, t.nodeSet) : null
	}
	get parsedPos() {
	...
	advance() {
		...
		if(!r.length) {
			...
			if(e) return p && console.log("Finish with " + this.stackID(e)), this.stackToTree(e);
			if(this.parser.strict) throw p && t && console.log("Stuck with token " + (this.tokens.mainToken ? this.parser.getName(this.tokens.mainToken.value) : "none")), new SyntaxError("No parse at " + i);
			this.recovering || (this.recovering = 5)
		}
		if(this.recovering && t) {
			let s = null != this.stoppedAt && t[0].pos > this.stoppedAt ? t[0] : this.runRecovery(t, e, r);
			if(s) return p && console.log("Force-finish " + this.stackID(s)), this.stackToTree(s.forceAll())
		}
		if(this.recovering) {
	...
	advanceStack(e, s, i) {
		...
		if(this.fragments) {
			...
			for(let h = this.fragments.nodeAt(r); h;) {
				let r = this.parser.nodeSet.types[h.type.id] == h.type ? n.getGoto(e.state, h.type.id) : -1;
				if(r > -1 && h.length && (!s || (h.prop(t.NodeProp.contextHash) || 0) == i)) return e.useNode(h, r), p && console.log(o + this.stackID(e) + ` (via reuse of ${n.getName(h.type.id)})`), !0;
				if(!(h instanceof t.Tree) || 0 == h.children.length || h.positions[0] > 0) break;
				let a = h.children[0];
				if(!(a instanceof t.Tree && 0 == h.positions[0])) break;
				h = a
			}
		}
		let h = n.stateSlot(e.state, 4);
		if(h > 0) return e.reduce(h), p && console.log(o + this.stackID(e) + ` (via always-reduce ${n.getName(65535&h)})`), !0;
		...
		for(let t = 0; t < a.length;) {
			...
			if(f.apply(h, O, u ? u.start : f.pos, l), p && console.log(o + this.stackID(f) + ` (via ${65536&h?`reduce of ${n.getName(65535&h)}`:"shift"} for ${n.getName(O)} @ ${r}${f==e?"":", split"})`), c) return !0;
			f.pos > r ? s.push(f) : i.push(f)
		}
		return !1
	}
	advanceFully(t, e) {
	...
	runRecovery(t, e, s) {
		...
		for(let n = 0; n < t.length; n++) {
			...
			if(o.deadEnd) {
				if(r) continue;
				if(r = !0, o.restart(), p && console.log(O + this.stackID(o) + " (restarted)"), this.advanceFully(o, s)) continue
			}
			let l = o.split(),
				c = O;
			for(let t = 0; t < 10 && l.forceReduce(); t++) {
				if(p && console.log(c + this.stackID(l) + " (via force-reduce)"), this.advanceFully(l, s)) break;
				p && (c = this.stackID(l) + " -> ")
			}
			for(let t of o.recoverByInsert(h)) p && console.log(O + this.stackID(t) + " (via recover-insert)"), this.advanceFully(t, s);
			this.stream.end > o.pos ? (a == o.pos && (a++, h = 0), o.recoverByDelete(h, a), p && console.log(O + this.stackID(o) + ` (via recover-delete ${this.parser.getName(h)})`), m(o, s)) : (!i || i.score < l.score) && (i = l)
		}
		return i
	}
	stackToTree(e) {
	...
	stackID(t) {
		...
		return e || d.set(t, e = String.fromCodePoint(this.nextStackID++)), e + t
	}
}
...
class S extends t.Parser {
	constructor(e) {
			...
			for(let s of e.nodeProps) {
				...
				for(let t = 1; t < s.length;) {
					...
					if(i >= 0) n(i, e, s[t++]);
						...
						let r = s[t + -i];
						for(let o = -i; o > 0; o--) n(s[t++], e, r);
						t++
					}
				}
			}
		this.nodeSet = new t.NodeSet(s.map((s, n) => t.NodeType.define({
		...
		this.specializers = this.specializerSpecs.map(b), this.states = h(e.states, Uint32Array), this.data = h(e.stateData), this.goto = h(e.goto), this.maxTerm = e.maxTerm, this.tokenizers = e.tokenizers.map(t => "number" == typeof t ? new c(o, t) : t), this.topRules = e.topRules, this.dialects = e.dialects || {}, this.dynamicPrecedences = e.dynamicPrecedences || null, this.tokenPrecTable = e.tokenPrec, this.termNames = e.termNames || null, this.maxNode = this.nodeSet.types.length - 1, this.dialect = this.parseDialect(), this.top = this.topRules[Object.keys(this.topRules)[0]]
	}
	createParse(t, e, s) {
	...
	hasAction(t, e) {
		let s = this.data;
		for(let i = 0; i < 2; i++)
			for(let r, n = this.stateSlot(t, i ? 2 : 1);; n += 3) {
				if(65535 == (r = s[n])) {
					if(1 != s[n + 1]) {
						if(2 == s[n + 1]) return $(s, n + 2);
						break
					}
					r = s[n = $(s, n + 2)]
				}
				if(r == e || 0 == r) return $(s, n + 1)
			}
		return 0
	}
	stateSlot(t, e) {
	...
	configure(t) {
		let e = Object.assign(Object.create(S.prototype), this);
...
const R = e.styleTags({
		...
		AnyChar: e.tags.character,
		Literal: e.tags.string,
		"tokens from grammar as empty prop extend specialize AtName": e.tags.keyword,
		"@top @left @right @cut @external": e.tags.modifier,
		"@precedence @tokens @context @dialects @skip @detectDelim @conflict": e.tags.definitionKeyword,
		"@extend @specialize": e.tags.operatorKeyword,
		"CharSet InvertedCharSet": e.tags.regexp,
		CharClass: e.tags.atom,
		RuleName: e.tags.variableName,
		"RuleDeclaration/RuleName InlineRule/RuleName TokensBody/RuleName": e.tags.definition(e.tags.variableName),
		PrecedenceName: e.tags.labelName,
		Name: e.tags.name,
		"( )": e.tags.paren,
	...
	v = s.LRLanguage.define({
		name: "lezer",
		parser: z.configure({
			props: [s.foldNodeProp.add({
				"Body TokensBody SkipBody PrecedenceBody": s.foldInside
			})]
		}),
		languageData: {
			commentTokens: {
...
exports.lezer = function() {
	return new s.LanguageSupport(v)
}, exports.lezerLanguage = v;
