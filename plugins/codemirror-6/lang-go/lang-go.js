/*\
title: $:/plugins/tiddlywiki/codemirror-6/plugins/lang-go/lang-go.js
...
function _(t, e) {
	...
	return e.cursor(O.IterMode.IncludeAnonymous).iterate(O => {
		if(n) n = !1;
			...
			for(let e of _(t, O.node)) i.push(e);
			return !1
		}
	}), v.set(e, i), i
}
const E = /^[\w$\xa1-\uffff][\w$\d\xa1-\uffff]*$/,
	...
	D = O => {
		...
		let i = [];
		for(let e = t; e; e = e.parent) Z.has(e.name) && (i = i.concat(_(O.state.doc, e)));
		return {
			options: i,
			from: s ? t.from : O.pos,
			validFor: E
...
const B = "interface struct chan map package go return break continue goto fallthrough else defer range true false nil".split(" ").map(N);
exports.go = function() {
	let O = U.concat(B);
