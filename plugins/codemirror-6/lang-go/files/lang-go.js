...
class i {
	constructor(O, t, e, s, i, n, r, a, o, Q = 0, h) {
		this.p = O, this.stack = t, this.state = e, this.reducePos = s, this.pos = i, this.score = n, this.buffer = r, this.bufferBase = a, this.curContext = o, this.lookAhead = Q, this.parent = h
	}
	toString() {
		return `[${this.stack.filter((O,t)=>t%3==0).concat(this.state)}]@${this.pos}${this.score?"!"+this.score:""}`
	}
	static start(O, t, e = 0) {
		let s = O.parser.context;
		return new i(O, [], t, e, e, 0, [], 0, s ? new n(s, s.start) : null, 0, null)
	}
	get context() {
	...
	reduce(O) {
		...
		if(r && (this.score += r), 0 == e) return this.pushState(i.getGoto(this.state, s, !0), this.reducePos), s < i.minRepeatTerm && this.storeNode(s, this.reducePos, this.reducePos, n ? 8 : 4, !0), void this.reduceContext(s, this.reducePos);
		let a = this.stack.length - 3 * (e - 1) - (262144 & O ? 6 : 0),
			o = a ? this.stack[a - 2] : this.p.ranges[0].from,
			Q = this.reducePos - o;
		Q >= 2e3 && !(null === (t = this.p.parser.nodeSet.types[s]) || void 0 === t ? void 0 : t.isAnonymous) && (o == this.p.lastBigReductionStart ? (this.p.bigReductionCount++, this.p.lastBigReductionSize = Q) : this.p.lastBigReductionSize < Q && (this.p.bigReductionCount = 1, this.p.lastBigReductionStart = o, this.p.lastBigReductionSize = Q));
		let h = a ? this.stack[a - 1] : 0,
			l = this.bufferBase + this.buffer.length - h;
		if(s < i.minRepeatTerm || 131072 & O) {
			let O = i.stateFlag(this.state, 1) ? this.pos : this.reducePos;
			this.storeNode(s, o, O, l + 4, !0)
		}
		if(262144 & O) this.state = this.stack[a];
		else {
			let O = this.stack[a - 3];
			this.state = i.getGoto(O, s, !0)
		}
		for(; this.stack.length > a;) this.stack.pop();
		this.reduceContext(s, o)
	}
	storeNode(O, t, e, s = 4, i = !1) {
		if(0 == O && (!this.stack.length || this.stack[this.stack.length - 1] < this.buffer.length + this.bufferBase)) {
			let O = this,
				s = this.buffer.length;
			if(0 == s && O.parent && (s = O.bufferBase - O.parent.bufferBase, O = O.parent), s > 0 && 0 == O.buffer[s - 4] && O.buffer[s - 1] > -1) {
				if(t == e) return;
				if(O.buffer[s - 2] >= t) return void(O.buffer[s - 2] = e)
			}
		}
		if(i && this.pos != e) {
			...
			if(i > 0 && (0 != this.buffer[i - 4] || this.buffer[i - 1] < 0)) {
				...
				for(let t = i; t > 0 && this.buffer[t - 2] > e; t -= 4)
					...
					for(; i > 0 && this.buffer[i - 2] > e;) this.buffer[i] = this.buffer[i - 4], this.buffer[i + 1] = this.buffer[i - 3], this.buffer[i + 2] = this.buffer[i - 2], this.buffer[i + 3] = this.buffer[i - 1], i -= 4, s > 4 && (s -= 4)
			}
			this.buffer[i] = O, this.buffer[i + 1] = t, this.buffer[i + 2] = e, this.buffer[i + 3] = s
		} else this.buffer.push(O, t, e, s)
	}
	shift(O, t, e, s) {
		if(131072 & O) this.pushState(65535 & O, this.pos);
		else if(262144 & O) this.pos = s, this.shiftContext(t, e), t <= this.p.parser.maxNode && this.buffer.push(t, e, s, 4);
		else {
			let i = O,
				{
					parser: n
				} = this.p;
			this.pos = s, !n.stateFlag(i, 1) && (s > e || t <= n.maxNode) && (this.reducePos = s), this.pushState(i, Math.min(e, this.reducePos)), this.shiftContext(t, e), t <= n.maxNode && this.buffer.push(t, e, s, 4)
		}
	}
	apply(O, t, e, s) {
		65536 & O ? this.reduce(O) : this.shift(O, t, e, s)
	}
	useNode(O, t) {
		let e = this.p.reused.length - 1;
		(e < 0 || this.p.reused[e] != O) && (this.p.reused.push(O), e++);
		let s = this.pos;
		this.reducePos = this.pos = s + O.length, this.pushState(t, s), this.buffer.push(e, s, this.reducePos, -1), this.curContext && this.updateContext(this.curContext.tracker.reuse(this.curContext.context, O, this, this.p.stream.reset(this.pos - O.length)))
	}
	split() {
		let O = this,
			t = O.buffer.length;
		...
		let e = O.buffer.slice(t),
			s = O.bufferBase + t;
		for(; O && s == O.bufferBase;) O = O.parent;
		return new i(this.p, this.stack.slice(), this.state, this.reducePos, this.pos, this.score, e, s, this.curContext, this.lookAhead, O)
	}
	recoverByDelete(O, t) {
	...
	recoverByInsert(O) {
		...
		if(t.length > 8 || this.stack.length >= 120) {
			let e = [];
			for(let s, i = 0; i < t.length; i += 2)(s = t[i + 1]) != this.state && this.p.parser.hasAction(s, O) && e.push(t[i], s);
			if(this.stack.length < 120)
				for(let O = 0; e.length < 8 && O < t.length; O += 2) {
					let s = t[O + 1];
					e.some((O, t) => 1 & t && O == s) || e.push(t[O], s)
				}
			t = e
		}
		let e = [];
		for(let O = 0; O < t.length && e.length < 4; O += 2) {
			...
			let i = this.split();
			i.pushState(s, this.pos), i.storeNode(0, i.pos, i.pos, 4, !0), i.shiftContext(t[O], this.pos), i.reducePos = this.pos, i.score -= 200, e.push(i)
		}
		return e
	}
	forceReduce() {
		...
		if(!O.validAction(this.state, t)) {
			...
			if(i < 0 || O.getGoto(this.stack[i], s, !1) < 0) {
	...
	findForcedReduction() {
		let {
			...
			if(!t.includes(s)) return t.push(s), O.allActions(s, t => {
				if(393216 & t);
				else if(65536 & t) {
					let e = (t >> 19) - i;
					if(e > 1) {
						let s = 65535 & t,
							i = this.stack.length - 3 * e;
						if(i >= 0 && O.getGoto(this.stack[i], s, !1) >= 0) return e << 19 | 65536 | s
					}
				} else {
					let O = e(t, i + 1);
					if(null != O) return O
...
function o(O, t = Uint16Array) {
	if("string" != typeof O) return O;
	let e = null;
	for(let s = 0, i = 0; s < O.length;) {
		let n = 0;
		for(;;) {
			let t = O.charCodeAt(s++),
				e = !1;
			if(126 == t) {
				n = 65535;
				break
...
class l {
	constructor(O, t) {
		this.input = O, this.ranges = t, this.chunk = "", this.chunkOff = 0, this.chunk2 = "", this.chunk2Pos = 0, this.next = -1, this.token = h, this.rangeIndex = 0, this.pos = this.chunkPos = t[0].from, this.range = t[0], this.end = t[t.length - 1].to, this.readNext()
	}
	resolveOffset(O, t) {
		let e = this.range,
			s = this.rangeIndex,
	...
	peek(O) {
		let t, e, s = this.chunkOff + O;
		if(s >= 0 && s < this.chunk.length) t = this.pos + O, e = this.chunk.charCodeAt(s);
		else {
			let s = this.resolveOffset(O, 1);
			if(null == s) return -1;
			if(t = s, t >= this.chunk2Pos && t < this.chunk2Pos + this.chunk2.length) e = this.chunk2.charCodeAt(t - this.chunk2Pos);
			else {
				let O = this.rangeIndex,
					s = this.range;
				for(; s.to <= t;) s = this.ranges[++O];
				this.chunk2 = this.input.chunk(this.chunk2Pos = t), t + this.chunk2.length > s.to && (this.chunk2 = this.chunk2.slice(0, s.to - t)), e = this.chunk2.charCodeAt(0)
			}
		}
	...
	reset(O, t) {
		if(t ? (this.token = t, t.start = O, t.lookAhead = O + 1, t.value = t.extended = -1) : this.token = h, this.pos != O) {
	...
	read(O, t) {
		...
		for(let s of this.ranges) {
			if(s.from >= t) break;
			s.to > O && (e += this.input.read(Math.max(s.from, O), Math.min(s.to, t)))
		}
		return e
	}
}
...
c.prototype.contextual = c.prototype.fallback = c.prototype.extend = !1;
class p {
	constructor(O, t, e) {
		this.precTable = t, this.elseToken = e, this.data = "string" == typeof O ? o(O) : O
	}
	token(O, t) {
		let e = O.pos,
			s = 0;
		...
		s && (O.reset(e, O.token), O.acceptToken(this.elseToken, s))
	}
}
p.prototype.contextual = c.prototype.fallback = c.prototype.extend = !1;

function u(O, t, e, s, i, n) {
	let r = 0,
		a = 1 << s,
		{
			dialect: o
	...
	O: for(; 0 != (a & O[r]);) {
		let e = O[r + 1];
		for(let s = r + 3; s < e; s += 2)
			if((O[s + 1] & a) > 0) {
				let e = O[s];
				if(o.allows(e) && (-1 == t.token.value || t.token.value == e || P(e, t.token.value, i, n))) {
					t.acceptToken(e);
					break
				}
			} let s = t.next,
			Q = 0,
			h = O[r + 2];
		if(!(t.next < 0 && h > Q && 65535 == O[e + 3 * h - 3])) {
			for(; Q < h;) {
				let i = Q + h >> 1,
					n = e + i + (i << 1),
					a = O[n],
...
function P(O, t, e, s) {
	let i = f(e, s, t);
	return i < 0 || f(e, s, O) < i
}
const d = "undefined" != typeof process && process.env && /\bparse\b/.test(process.env.LOG);
let X = null;
...
class g {
	...
	nextFragment() {
		...
		if(O) {
			for(this.safeFrom = O.openStart ? k(O.tree, O.from + O.offset, 1) - O.offset : O.from, this.safeTo = O.openEnd ? k(O.tree, O.to + O.offset, -1) - O.offset : O.to; this.trees.length;) this.trees.pop(), this.start.pop(), this.index.pop();
	...
	nodeAt(t) {
		...
		for(;;) {
			...
			if(n instanceof O.Tree) {
				if(r == t) {
					if(r < this.safeFrom) return null;
					let t = r + n.length;
					if(t <= this.safeTo) {
						let e = n.prop(O.NodeProp.lookAhead);
						if(!e || t + e < this.fragment.to) return n
					}
				}
				this.index[e]++, r + n.length >= Math.max(this.safeFrom, t) && (this.trees.push(n), this.start.push(r), this.index.push(0))
...
class m {
	constructor(O, t) {
		this.stream = t, this.tokens = [], this.mainToken = null, this.actions = [], this.tokens = O.tokenizers.map(O => new Q)
	}
	getActions(O) {
		let t = 0,
			e = null,
			...
			n = s.stateSlot(O.state, 3),
			r = O.curContext ? O.curContext.hash : 0,
			a = 0;
		for(let s = 0; s < i.length; s++) {
			if(!(1 << s & n)) continue;
			let o = i[s],
				Q = this.tokens[s];
			if((!e || o.fallback) && ((o.contextual || Q.start != O.pos || Q.mask != n || Q.context != r) && (this.updateCachedToken(Q, o, O), Q.mask = n, Q.context = r), Q.lookAhead > Q.end + 25 && (a = Math.max(Q.lookAhead, a)), 0 != Q.value)) {
				let s = t;
				if(Q.extended > -1 && (t = this.addActions(O, Q.extended, Q.end, t)), t = this.addActions(O, Q.value, Q.end, t), !o.extend && (e = Q, t > s)) break
			}
		}
		for(; this.actions.length > t;) this.actions.pop();
		return a && O.setLookAhead(a), e || O.pos != this.stream.end || (e = new Q, e.value = O.p.parser.eofTerm, e.start = e.end = O.pos, t = this.addActions(O, e.value, e.end, t)), this.mainToken = e, this.actions
	}
	getMainToken(O) {
		...
		return t.start = e, t.end = Math.min(e + 1, s.stream.end), t.value = e == s.stream.end ? s.parser.eofTerm : 0, t
	}
	updateCachedToken(O, t, e) {
		let s = this.stream.clipPos(e.pos);
		if(t.token(this.stream.reset(s, O), e), O.value > -1) {
			let {
				parser: t
			} = e.p;
			for(let s = 0; s < t.specialized.length; s++)
				if(t.specialized[s] == O.value) {
					let i = t.specializers[s](this.stream.read(O.start, O.end), e);
					if(i >= 0 && e.p.parser.dialect.allows(i >> 1)) {
						1 & i ? O.extended = i >> 1 : O.value = i >> 1;
						break
					}
				}
		} else O.value = 0, O.end = this.stream.clipPos(s + 1)
	}
	putAction(O, t, e, s) {
		for(let t = 0; t < s; t += 3)
			if(this.actions[t] == O) return s;
		return this.actions[s++] = O, this.actions[s++] = t, this.actions[s++] = e, s
	}
	addActions(O, t, e, s) {
		let {
			state: i
		...
		for(let O = 0; O < 2; O++)
			for(let a = n.stateSlot(i, O ? 2 : 1);; a += 3) {
				if(65535 == r[a]) {
					if(1 != r[a + 1]) {
						0 == s && 2 == r[a + 1] && (s = this.putAction(b(r, a + 2), t, e, s));
						break
					}
					a = b(r, a + 2)
				}
				r[a] == t && (s = this.putAction(b(r, a + 1), t, e, s))
			}
		return s
	}
}
class S {
	constructor(O, t, e, s) {
		this.parser = O, this.input = t, this.ranges = s, this.recovering = 0, this.nextStackID = 9812, this.minStackPos = 0, this.reused = [], this.stoppedAt = null, this.lastBigReductionStart = -1, this.lastBigReductionSize = 0, this.bigReductionCount = 0, this.stream = new l(t, s), this.tokens = new m(O, this.stream), this.topTerm = O.top[1];
		let {
			from: n
		} = s[0];
		this.stacks = [i.start(this, O.top[0], n)], this.fragments = e.length && this.stream.end - n > 4 * O.bufferLength ? new g(e, O.nodeSet) : null
	}
	get parsedPos() {
	...
	advance() {
		...
		for(let n = 0; n < e.length; n++) {
			let r = e[n];
			for(;;) {
				if(this.tokens.mainToken = null, r.pos > s) i.push(r);
				else {
					if(this.advanceStack(r, i, e)) continue;
					{
						O || (O = [], t = []), O.push(r);
		...
		if(!i.length) {
			...
			if(t) return d && console.log("Finish with " + this.stackID(t)), this.stackToTree(t);
			if(this.parser.strict) throw d && O && console.log("Stuck with token " + (this.tokens.mainToken ? this.parser.getName(this.tokens.mainToken.value) : "none")), new SyntaxError("No parse at " + s);
			this.recovering || (this.recovering = 5)
		}
		if(this.recovering && O) {
			let e = null != this.stoppedAt && O[0].pos > this.stoppedAt ? O[0] : this.runRecovery(O, t, i);
			if(e) return d && console.log("Force-finish " + this.stackID(e)), this.stackToTree(e.forceAll())
		}
		if(this.recovering) {
			let O = 1 == this.recovering ? 1 : 3 * this.recovering;
			if(i.length > O)
				for(i.sort((O, t) => t.score - O.score); i.length > O;) i.pop();
			i.some(O => O.reducePos > s) && this.recovering--
		} else if(i.length > 1) {
	...
	advanceStack(t, e, s) {
		...
		if(this.fragments) {
			...
			for(let a = this.fragments.nodeAt(i); a;) {
				let i = this.parser.nodeSet.types[a.type.id] == a.type ? n.getGoto(t.state, a.type.id) : -1;
				if(i > -1 && a.length && (!e || (a.prop(O.NodeProp.contextHash) || 0) == s)) return t.useNode(a, i), d && console.log(r + this.stackID(t) + ` (via reuse of ${n.getName(a.type.id)})`), !0;
				if(!(a instanceof O.Tree) || 0 == a.children.length || a.positions[0] > 0) break;
				let o = a.children[0];
				if(!(o instanceof O.Tree && 0 == a.positions[0])) break;
				a = o
			}
		}
		let a = n.stateSlot(t.state, 4);
		if(a > 0) return t.reduce(a), d && console.log(r + this.stackID(t) + ` (via always-reduce ${n.getName(65535&a)})`), !0;
		...
		for(let O = 0; O < o.length;) {
			...
			if(c.apply(a, Q, p ? p.start : c.pos, h), d && console.log(r + this.stackID(c) + ` (via ${65536&a?`reduce of ${n.getName(65535&a)}`:"shift"} for ${n.getName(Q)} @ ${i}${c==t?"":", split"})`), l) return !0;
	...
	runRecovery(O, t, e) {
		...
		for(let n = 0; n < O.length; n++) {
			...
			if(r.deadEnd) {
				if(i) continue;
				if(i = !0, r.restart(), d && console.log(Q + this.stackID(r) + " (restarted)"), this.advanceFully(r, e)) continue
			}
			let h = r.split(),
				l = Q;
			for(let O = 0; O < 10 && h.forceReduce(); O++) {
				if(d && console.log(l + this.stackID(h) + " (via force-reduce)"), this.advanceFully(h, e)) break;
				d && (l = this.stackID(h) + " -> ")
			}
			for(let O of r.recoverByInsert(a)) d && console.log(Q + this.stackID(O) + " (via recover-insert)"), this.advanceFully(O, e);
			this.stream.end > r.pos ? (o == r.pos && (o++, a = 0), r.recoverByDelete(a, o), d && console.log(Q + this.stackID(r) + ` (via recover-delete ${this.parser.getName(a)})`), $(r, e)) : (!s || s.score < h.score) && (s = h)
		}
		return s
	}
	stackToTree(t) {
	...
	stackID(O) {
		let t = (X || (X = new WeakMap)).get(O);
		return t || X.set(O, t = String.fromCodePoint(this.nextStackID++)), t + O
	}
}
...
function $(O, t) {
	for(let e = 0; e < t.length; e++) {
		let s = t[e];
		if(s.pos == O.pos && s.sameState(O)) return void(t[e].score < O.score && (t[e] = O))
	}
	t.push(O)
}
class x {
...
class R extends O.Parser {
	constructor(t) {
		...
		let s = Object.keys(t.topRules).map(O => t.topRules[O][1]),
			i = [];
			...
			for(let e of t.nodeProps) {
				...
				for(let O = 1; O < e.length;) {
					...
					if(s >= 0) n(s, t, e[O++]);
						...
						let i = e[O + -s];
						for(let r = -s; r > 0; r--) n(e[O++], t, i);
						O++
					}
				}
			}
		this.nodeSet = new O.NodeSet(e.map((e, n) => O.NodeType.define({
		...
		this.specializers = this.specializerSpecs.map(W), this.states = o(t.states, Uint32Array), this.data = o(t.stateData), this.goto = o(t.goto), this.maxTerm = t.maxTerm, this.tokenizers = t.tokenizers.map(O => "number" == typeof O ? new c(r, O) : O), this.topRules = t.topRules, this.dialects = t.dialects || {}, this.dynamicPrecedences = t.dynamicPrecedences || null, this.tokenPrecTable = t.tokenPrec, this.termNames = t.termNames || null, this.maxNode = this.nodeSet.types.length - 1, this.dialect = this.parseDialect(), this.top = this.topRules[Object.keys(this.topRules)[0]]
	}
	createParse(O, t, e) {
		let s = new S(this, O, t, e);
		for(let i of this.wrappers) s = i(s, O, t, e);
		return s
	}
	getGoto(O, t, e = !1) {
		let s = this.goto;
	...
	hasAction(O, t) {
		let e = this.data;
		for(let s = 0; s < 2; s++)
			for(let i, n = this.stateSlot(O, s ? 2 : 1);; n += 3) {
				if(65535 == (i = e[n])) {
					if(1 != e[n + 1]) {
						if(2 == e[n + 1]) return b(e, n + 2);
						break
					}
					i = e[n = b(e, n + 2)]
				}
				if(i == t || 0 == i) return b(e, n + 1)
			}
		return 0
	}
	stateSlot(O, t) {
	...
	allActions(O, t) {
		let e = this.stateSlot(O, 4),
			s = e ? t(e) : void 0;
		for(let e = this.stateSlot(O, 1); null == s; e += 3) {
	...
	configure(O) {
		...
		return O.tokenizers && (t.tokenizers = this.tokenizers.map(t => {
			...
			return e ? e.to : t
		})), O.specializers && (t.specializers = this.specializers.slice(), t.specializerSpecs = this.specializerSpecs.map((e, s) => {
			let i = O.specializers.find(O => O.from == e.external);
			if(!i) return e;
			let n = Object.assign(Object.assign({}, e), {
				external: i.to
			});
			return t.specializers[s] = W(n), n
	...
	parseDialect(O) {
		...
		for(let O = 0; O < t.length; O++)
			if(!e[O])
				for(let e, i = this.dialects[t[O]]; 65535 != (e = this.data[i++]);)(s || (s = new Uint8Array(this.maxTerm + 1)))[e] = 1;
		return new x(O, e, s)
	}
...
const w = new class {
	...
	for(let e = 0, s = O.next;
		(t.context && (s < 0 || 10 == s || 13 == s || 47 == s && 47 == O.peek(e + 1)) || 41 == s || 125 == s) && O.acceptToken(177), 32 == s || 9 == s;) s = O.peek(++e)
}, {
	contextual: !0
});
let V = new Set([95, 184, 20, 12, 17, 144, 145, 142, 148, 13, 53, 25]);
const Y = new class {
		constructor(O) {
			this.start = O.start, this.shift = O.shift || T, this.reduce = O.reduce || T, this.reuse = O.reuse || T, this.hash = O.hash || (() => 0), this.strict = !1 !== O.strict
		}
	}({
		start: !1,
		shift: (O, t) => 179 == t ? O : V.has(t)
	}),
	y = t.styleTags({
		"func interface struct chan map const type var": t.tags.definitionKeyword,
		"import package": t.tags.moduleKeyword,
		"switch for go select return break continue goto fallthrough case if else defer": t.tags.controlKeyword,
		range: t.tags.keyword,
		Bool: t.tags.bool,
		String: t.tags.string,
		Rune: t.tags.character,
		Number: t.tags.number,
		Nil: t.tags.null,
		VariableName: t.tags.variableName,
		DefName: t.tags.definition(t.tags.variableName),
		TypeName: t.tags.typeName,
		LabelName: t.tags.labelName,
		FieldName: t.tags.propertyName,
		"FunctionDecl/DefName": t.tags.function(t.tags.definition(t.tags.variableName)),
		"TypeSpec/DefName": t.tags.definition(t.tags.typeName),
		"CallExpr/VariableName": t.tags.function(t.tags.variableName),
		...
		"DerefOp .": t.tags.derefOperator,
		"UpdateOp IncDecOp": t.tags.updateOperator,
		CompareOp: t.tags.compareOperator,
		"= :=": t.tags.definitionOperator,
		"<-": t.tags.operator,
		'~ "*"': t.tags.modifier,
		"; ,": t.tags.separator,
		"... :": t.tags.punctuation,
	...
	G = R.deserialize({
		...
		tokenizers: [w, 1, 2, new p("j~RQYZXz{^~^O$|~~aP!P!Qd~iO$}~~", 25, 181)],
		topRules: {
			SourceFile: [0, 3]
		},
		dynamicPrecedences: {
	...
	U = [s.snippetCompletion("func ${name}(${params}) ${type} {\n\t${}\n}", {
		label: "func",
		detail: "declaration",
		type: "keyword"
	}), s.snippetCompletion("func (${receiver}) ${name}(${params}) ${type} {\n\t${}\n}", {
		label: "func",
		detail: "method declaration",
		type: "keyword"
	}), s.snippetCompletion("var ${name} = ${value}", {
		label: "var",
		detail: "declaration",
		type: "keyword"
	}), s.snippetCompletion("type ${name} ${type}", {
		label: "type",
		detail: "declaration",
		type: "keyword"
	}), s.snippetCompletion("const ${name} = ${value}", {
		label: "const",
		detail: "declaration",
		type: "keyword"
	}), s.snippetCompletion("type ${name} = ${type}", {
		...
		type: "keyword"
	}), s.snippetCompletion("for ${init}; ${test}; ${update} {\n\t${}\n}", {
		label: "for",
		detail: "loop",
		type: "keyword"
	}), s.snippetCompletion("for ${i} := range ${value} {\n\t${}\n}", {
		label: "for",
		detail: "range",
		type: "keyword"
	}), s.snippetCompletion("select {\n\t${}\n}", {
		label: "select",
		detail: "statement",
		type: "keyword"
	}), s.snippetCompletion("case ${}:\n${}", {
		label: "case",
		type: "keyword"
	}), s.snippetCompletion("switch ${} {\n\t${}\n}", {
		label: "switch",
		detail: "statement",
		type: "keyword"
	}), s.snippetCompletion("switch ${}.(${type}) {\n\t${}\n}", {
		label: "switch",
		detail: "type statement",
		type: "keyword"
	}), s.snippetCompletion("if ${} {\n\t${}\n}", {
		label: "if",
		detail: "block",
		type: "keyword"
	}), s.snippetCompletion("if ${} {\n\t${}\n} else {\n\t${}\n}", {
		...
		detail: "/ else block",
		type: "keyword"
	}), s.snippetCompletion('import ${name} "${module}"\n${}', {
		label: "import",
		detail: "declaration",
		type: "keyword"
	})],
	v = new O.NodeWeakMap,
	Z = new Set(["SourceFile", "Block", "FunctionDecl", "MethodDecl", "FunctionLiteral", "ForStatement", "SwitchStatement", "TypeSwitchStatement", "IfStatement"]);

function q(O, t) {
	return (e, s) => {
		O: for(let i = e.node.firstChild, n = 0, r = null;;) {
			for(; !i;) {
				if(!n) break O;
				n--, i = r.nextSibling, r = r.parent
			}
			t && i.name == t || "SpecList" == i.name ? (n++, r = i, i = i.firstChild) : ("DefName" == i.name && s(i, O), i = i.nextSibling)
		}
		return !0
	}
}
const _ = {
	FunctionDecl: q("function"),
	VarDecl: q("var", "VarSpec"),
	ConstDecl: q("constant", "ConstSpec"),
	TypeDecl: q("type", "TypeSpec"),
	ImportDecl: q("constant", "ImportSpec"),
	Parameter: q("var"),
	__proto__: null
};
...
function C(t, e) {
	...
	return e.cursor(O.IterMode.IncludeAnonymous).iterate(O => {
		if(n) n = !1;
			...
			let t = _[O.name];
			if(t && t(O, r) || Z.has(O.name)) return !1
		} else if(O.to - O.from > 8192) {
			for(let e of C(t, O.node)) i.push(e);
			return !1
		}
	}), v.set(e, i), i
}
const E = /^[\w$\xa1-\uffff][\w$\d\xa1-\uffff]*$/,
	z = ["String", "LineComment", "BlockComment", "DefName", "LabelName", "FieldName", ".", "?."],
	D = O => {
		let t = e.syntaxTree(O.state).resolveInner(O.pos, -1);
		if(z.indexOf(t.name) > -1) return null;
		let s = "VariableName" == t.name || t.to - t.from < 20 && E.test(O.state.sliceDoc(t.from, t.to));
		if(!s && !O.explicit) return null;
		let i = [];
		for(let e = t; e; e = e.parent) Z.has(e.name) && (i = i.concat(C(O.state.doc, e)));
		return {
			options: i,
			from: s ? t.from : O.pos,
			validFor: E
	...
	F = e.LRLanguage.define({
		...
		parser: G.configure({
			props: [e.indentNodeProp.add({
				...
				"SwitchBlock SelectBlock": O => {
					let t = O.textAfter,
						e = /^\s*\}/.test(t),
						s = /^\s*(case|default)\b/.test(t);
					return O.baseIndent + (e || s ? 0 : O.unit)
				},
				...
				"Block SwitchBlock SelectBlock LiteralValue InterfaceType StructType SpecList": e.foldInside,
				BlockComment: O => ({
					from: O.from + 2,
					to: O.to - 2
				})
...
const A = "interface struct chan map package go return break continue goto fallthrough else defer range true false nil".split(" ").map(N);
exports.go = function() {
	let O = U.concat(A);
	return new e.LanguageSupport(F, [F.data.of({
		autocomplete: s.ifNotIn(z, s.completeFromList(O))
	}), F.data.of({
		autocomplete: D
	})])
}, exports.goLanguage = F, exports.localCompletionSource = D, exports.snippets = U;
