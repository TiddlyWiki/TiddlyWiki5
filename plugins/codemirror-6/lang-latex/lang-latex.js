/*\
title: $:/plugins/tiddlywiki/codemirror-6/plugins/lang-latex/lang-latex.js
	...
	z = a.StateField.define({
		...
		update(t, e) {
			if(e.docChanged && t.diagnostics.size) {
				...
				}!O.size && a && e.state.facet(B).autoPanel && (a = null), t = new v(O, a, i)
			}
			for(let O of e.effects)
				if(O.is(V)) {
					let i = e.state.facet(B).autoPanel ? O.value.length ? M.open : null : t.panel;
...
function A(t, e, O) {
	...
	let r = t.state.facet(B).tooltipFilter;
...
const N = t => {
	...
	W = i.ViewPlugin.fromClass(class {
		...
		update(t) {
			let e = t.state.facet(B);
			(t.docChanged || e != t.startState.facet(B) || e.needsRefresh && e.needsRefresh(t)) && (this.lintTime = Date.now() + e.delay, this.set || (this.set = !0, this.timeout = setTimeout(this.run, e.delay)))
		}
		force() {
...
const Ot = [z, i.EditorView.decorations.compute([z], t => {
	...
	hideOn: function(t, e) {
		let O = e.pos,
			i = e.end || O,
			a = t.state.facet(B).hideOn(t, O, i);
		if(null != a) return a;
...
const gt = pt("\\"),
	...
	Bt = (t, e) => _t.has(t) ? 77 : At.has(t) ? 78 : Zt.has(t) ? 79 : Nt.has(t) ? 80 : Wt[t] || -1,
	Lt = {
		"\\(": 85,
		"\\)": 86,
		"\\[": 87,
