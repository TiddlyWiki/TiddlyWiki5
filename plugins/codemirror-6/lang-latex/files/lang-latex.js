"use strict";
var t = require("$:/plugins/tiddlywiki/codemirror-6/lib/lezer-common.js"),
	e = require("$:/plugins/tiddlywiki/codemirror-6/lib/codemirror-language.js"),
	O = require("$:/plugins/tiddlywiki/codemirror-6/lib/lezer-highlight.js"),
	i = require("$:/plugins/tiddlywiki/codemirror-6/lib/codemirror-view.js"),
	a = require("$:/plugins/tiddlywiki/codemirror-6/lib/codemirror-state.js"),
	n = require("$:/plugins/tiddlywiki/codemirror-6/lib/codemirror-autocomplete.js");
class s {
	constructor(t, e, O, i, a, n, s, r, o, l = 0, d) {
		this.p = t, this.stack = e, this.state = O, this.reducePos = i, this.pos = a, this.score = n, this.buffer = s, this.bufferBase = r, this.curContext = o, this.lookAhead = l, this.parent = d
	}
	toString() {
		return `[${this.stack.filter((t,e)=>e%3==0).concat(this.state)}]@${this.pos}${this.score?"!"+this.score:""}`
	}
	static start(t, e, O = 0) {
		let i = t.parser.context;
		return new s(t, [], e, O, O, 0, [], 0, i ? new r(i, i.start) : null, 0, null)
	}
	get context() {
	...
	reduce(t) {
		...
		let O = t >> 19,
			...
			n = this.reducePos < this.pos - 25 && this.setLookAhead(this.pos),
			s = a.dynamicPrecedence(i);
		if(s && (this.score += s), 0 == O) return this.pushState(a.getGoto(this.state, i, !0), this.reducePos), i < a.minRepeatTerm && this.storeNode(i, this.reducePos, this.reducePos, n ? 8 : 4, !0), void this.reduceContext(i, this.reducePos);
		let r = this.stack.length - 3 * (O - 1) - (262144 & t ? 6 : 0),
			o = r ? this.stack[r - 2] : this.p.ranges[0].from,
			l = this.reducePos - o;
		l >= 2e3 && !(null === (e = this.p.parser.nodeSet.types[i]) || void 0 === e ? void 0 : e.isAnonymous) && (o == this.p.lastBigReductionStart ? (this.p.bigReductionCount++, this.p.lastBigReductionSize = l) : this.p.lastBigReductionSize < l && (this.p.bigReductionCount = 1, this.p.lastBigReductionStart = o, this.p.lastBigReductionSize = l));
		let d = r ? this.stack[r - 1] : 0,
			c = this.bufferBase + this.buffer.length - d;
		if(i < a.minRepeatTerm || 131072 & t) {
			let t = a.stateFlag(this.state, 1) ? this.pos : this.reducePos;
			this.storeNode(i, o, t, c + 4, !0)
		}
		if(262144 & t) this.state = this.stack[r];
		else {
			let t = this.stack[r - 3];
			this.state = a.getGoto(t, i, !0)
		}
		for(; this.stack.length > r;) this.stack.pop();
		this.reduceContext(i, o)
	}
	storeNode(t, e, O, i = 4, a = !1) {
		if(0 == t && (!this.stack.length || this.stack[this.stack.length - 1] < this.buffer.length + this.bufferBase)) {
			...
			if(0 == i && t.parent && (i = t.bufferBase - t.parent.bufferBase, t = t.parent), i > 0 && 0 == t.buffer[i - 4] && t.buffer[i - 1] > -1) {
				if(e == O) return;
				if(t.buffer[i - 2] >= e) return void(t.buffer[i - 2] = O)
			}
		}
		if(a && this.pos != O) {
			let a = this.buffer.length;
			if(a > 0 && (0 != this.buffer[a - 4] || this.buffer[a - 1] < 0)) {
				let t = !1;
				for(let e = a; e > 0 && this.buffer[e - 2] > O; e -= 4)
					if(this.buffer[e - 1] >= 0) {
						t = !0;
						break
					} if(t)
					for(; a > 0 && this.buffer[a - 2] > O;) this.buffer[a] = this.buffer[a - 4], this.buffer[a + 1] = this.buffer[a - 3], this.buffer[a + 2] = this.buffer[a - 2], this.buffer[a + 3] = this.buffer[a - 1], a -= 4, i > 4 && (i -= 4)
			}
			this.buffer[a] = t, this.buffer[a + 1] = e, this.buffer[a + 2] = O, this.buffer[a + 3] = i
		} else this.buffer.push(t, e, O, i)
	}
	shift(t, e, O, i) {
		if(131072 & t) this.pushState(65535 & t, this.pos);
		else if(262144 & t) this.pos = i, this.shiftContext(e, O), e <= this.p.parser.maxNode && this.buffer.push(e, O, i, 4);
		else {
			let a = t,
				{
					parser: n
				} = this.p;
			this.pos = i, !n.stateFlag(a, 1) && (i > O || e <= n.maxNode) && (this.reducePos = i), this.pushState(a, Math.min(O, this.reducePos)), this.shiftContext(e, O), e <= n.maxNode && this.buffer.push(e, O, i, 4)
		}
	}
	apply(t, e, O, i) {
		65536 & t ? this.reduce(t) : this.shift(t, e, O, i)
	}
	useNode(t, e) {
		let O = this.p.reused.length - 1;
		(O < 0 || this.p.reused[O] != t) && (this.p.reused.push(t), O++);
		let i = this.pos;
		this.reducePos = this.pos = i + t.length, this.pushState(e, i), this.buffer.push(O, i, this.reducePos, -1), this.curContext && this.updateContext(this.curContext.tracker.reuse(this.curContext.context, t, this, this.p.stream.reset(this.pos - t.length)))
	}
	split() {
		let t = this,
			e = t.buffer.length;
		for(; e > 0 && t.buffer[e - 2] > t.reducePos;) e -= 4;
		let O = t.buffer.slice(e),
			i = t.bufferBase + e;
		for(; t && i == t.bufferBase;) t = t.parent;
		return new s(this.p, this.stack.slice(), this.state, this.reducePos, this.pos, this.score, O, i, this.curContext, this.lookAhead, t)
	}
	recoverByDelete(t, e) {
		let O = t <= this.p.parser.maxNode;
		O && this.storeNode(t, this.pos, e, 4), this.storeNode(0, this.pos, e, O ? 8 : 4), this.pos = this.reducePos = e, this.score -= 190
	}
	canShift(t) {
		for(let e = new o(this);;) {
			let O = this.p.parser.stateSlot(e.state, 4) || this.p.parser.hasAction(e.state, t);
			if(0 == O) return !1;
			if(!(65536 & O)) return !0;
			e.reduce(O)
		}
	...
	recoverByInsert(t) {
		...
		if(e.length > 8 || this.stack.length >= 120) {
			let O = [];
			for(let i, a = 0; a < e.length; a += 2)(i = e[a + 1]) != this.state && this.p.parser.hasAction(i, t) && O.push(e[a], i);
			if(this.stack.length < 120)
				for(let t = 0; O.length < 8 && t < e.length; t += 2) {
					let i = e[t + 1];
					O.some((t, e) => 1 & e && t == i) || O.push(e[t], i)
				}
			e = O
		}
		let O = [];
		for(let t = 0; t < e.length && O.length < 4; t += 2) {
			let i = e[t + 1];
			if(i == this.state) continue;
			let a = this.split();
			a.pushState(i, this.pos), a.storeNode(0, a.pos, a.pos, 4, !0), a.shiftContext(e[t], this.pos), a.reducePos = this.pos, a.score -= 200, O.push(a)
		}
		return O
	}
	forceReduce() {
		...
		if(!t.validAction(this.state, e)) {
			let O = e >> 19,
				i = 65535 & e,
				a = this.stack.length - 3 * O;
			if(a < 0 || t.getGoto(this.stack[a], i, !1) < 0) {
	...
	findForcedReduction() {
		let {
			...
			if(!e.includes(i)) return e.push(i), t.allActions(i, e => {
				if(393216 & e);
				else if(65536 & e) {
					let O = (e >> 19) - a;
					if(O > 1) {
						let i = 65535 & e,
							a = this.stack.length - 3 * O;
						if(a >= 0 && t.getGoto(this.stack[a], i, !1) >= 0) return O << 19 | 65536 | i
					}
				} else {
					let t = O(e, a + 1);
					if(null != t) return t
...
class o {
	...
	reduce(t) {
		let e = 65535 & t,
			O = t >> 19;
		0 == O ? (this.stack == this.start.stack && (this.stack = this.stack.slice()), this.stack.push(this.state, 0, 0), this.base += 3) : this.base -= 3 * (O - 1);
...
class l {
	constructor(t, e, O) {
		this.stack = t, this.pos = e, this.index = O, this.buffer = t.buffer, 0 == this.index && this.maybeNext()
	}
...
class h {
	constructor(t, e) {
		this.input = t, this.ranges = e, this.chunk = "", this.chunkOff = 0, this.chunk2 = "", this.chunk2Pos = 0, this.next = -1, this.token = Q, this.rangeIndex = 0, this.pos = this.chunkPos = e[0].from, this.range = e[0], this.end = e[e.length - 1].to, this.readNext()
	}
	resolveOffset(t, e) {
		let O = this.range,
			i = this.rangeIndex,
	...
	peek(t) {
		let e, O, i = this.chunkOff + t;
		if(i >= 0 && i < this.chunk.length) e = this.pos + t, O = this.chunk.charCodeAt(i);
		else {
			let i = this.resolveOffset(t, 1);
			if(null == i) return -1;
			if(e = i, e >= this.chunk2Pos && e < this.chunk2Pos + this.chunk2.length) O = this.chunk2.charCodeAt(e - this.chunk2Pos);
			else {
				let t = this.rangeIndex,
					i = this.range;
				for(; i.to <= e;) i = this.ranges[++t];
				this.chunk2 = this.input.chunk(this.chunk2Pos = e), e + this.chunk2.length > i.to && (this.chunk2 = this.chunk2.slice(0, i.to - e)), O = this.chunk2.charCodeAt(0)
			}
		}
		return e >= this.token.lookAhead && (this.token.lookAhead = e + 1), O
	}
	acceptToken(t, e = 0) {
		let O = e ? this.resolveOffset(e, -1) : this.pos;
		if(null == O || O < this.token.start) throw new RangeError("Token end out of bounds");
		this.token.value = t, this.token.end = O
	}
	acceptTokenTo(t, e) {
	...
	reset(t, e) {
		if(e ? (this.token = e, e.start = t, e.lookAhead = t + 1, e.value = e.extended = -1) : this.token = Q, this.pos != t) {
	...
	read(t, e) {
		...
		for(let i of this.ranges) {
			if(i.from >= e) break;
			i.to > t && (O += this.input.read(Math.max(i.from, t), Math.min(i.to, e)))
		}
		return O
	}
}
class f {
	...
	token(t, e) {
		...
		! function(t, e, O, i, a, n) {
			...
			t: for(; 0 != (r & t[s]);) {
				let O = t[s + 1];
				for(let i = s + 3; i < O; i += 2)
					if((t[i + 1] & r) > 0) {
						let O = t[i];
						if(o.allows(O) && (-1 == e.token.value || e.token.value == O || p(O, e.token.value, a, n))) {
							e.acceptToken(O);
							break
						}
					} let i = e.next,
					l = 0,
					d = t[s + 2];
				if(!(e.next < 0 && d > l && 65535 == t[O + 3 * d - 3])) {
					for(; l < d;) {
						let a = l + d >> 1,
							n = O + a + (a << 1),
							r = t[n],
		...
		}(this.data, t, e, this.id, O.data, O.tokenPrecTable)
	}
}
f.prototype.contextual = f.prototype.fallback = f.prototype.extend = !1, f.prototype.fallback = f.prototype.extend = !1;
class u {
	constructor(t, e = {}) {
...
function p(t, e, O, i) {
	let a = m(O, i, e);
	return a < 0 || m(O, i, t) < a
}
const g = "undefined" != typeof process && process.env && /\bparse\b/.test(process.env.LOG);
let X = null;

function q(e, O, i) {
	let a = e.cursor(t.IterMode.IncludeAnonymous);
	for(a.moveTo(O);;)
		if(!(i < 0 ? a.childBefore(O) : a.childAfter(O)))
			for(;;) {
				if((i < 0 ? a.to < O : a.from > O) && !a.type.isError) return i < 0 ? Math.max(0, Math.min(a.to - 1, O - 25)) : Math.min(e.length, Math.max(a.from + 1, O + 25));
				if(i < 0 ? a.prevSibling() : a.nextSibling()) break;
				if(!a.parent()) return i < 0 ? 0 : e.length
			}
}
class b {
	constructor(t, e) {
	...
	nextFragment() {
		...
		if(t) {
			for(this.safeFrom = t.openStart ? q(t.tree, t.from + t.offset, 1) - t.offset : t.from, this.safeTo = t.openEnd ? q(t.tree, t.to + t.offset, -1) - t.offset : t.to; this.trees.length;) this.trees.pop(), this.start.pop(), this.index.pop();
...
class $ {
	...
	getActions(t) {
		let e = 0,
			...
			n = i.stateSlot(t.state, 3),
			s = t.curContext ? t.curContext.hash : 0,
			r = 0;
		for(let i = 0; i < a.length; i++) {
			if(!(1 << i & n)) continue;
			let o = a[i],
				l = this.tokens[i];
			if((!O || o.fallback) && ((o.contextual || l.start != t.pos || l.mask != n || l.context != s) && (this.updateCachedToken(l, o, t), l.mask = n, l.context = s), l.lookAhead > l.end + 25 && (r = Math.max(l.lookAhead, r)), 0 != l.value)) {
				let i = e;
				if(l.extended > -1 && (e = this.addActions(t, l.extended, l.end, e)), e = this.addActions(t, l.value, l.end, e), !o.extend && (O = l, e > i)) break
			}
		}
		for(; this.actions.length > e;) this.actions.pop();
		return r && t.setLookAhead(r), O || t.pos != this.stream.end || (O = new c, O.value = t.p.parser.eofTerm, O.start = O.end = t.pos, e = this.addActions(t, O.value, O.end, e)), this.mainToken = O, this.actions
	}
	getMainToken(t) {
		...
		return e.start = O, e.end = Math.min(O + 1, i.stream.end), e.value = O == i.stream.end ? i.parser.eofTerm : 0, e
	}
	updateCachedToken(t, e, O) {
		let i = this.stream.clipPos(O.pos);
		if(e.token(this.stream.reset(i, t), O), t.value > -1) {
			let {
				parser: e
			} = O.p;
			for(let i = 0; i < e.specialized.length; i++)
				if(e.specialized[i] == t.value) {
					let a = e.specializers[i](this.stream.read(t.start, t.end), O);
					if(a >= 0 && O.p.parser.dialect.allows(a >> 1)) {
						1 & a ? t.extended = a >> 1 : t.value = a >> 1;
						break
					}
				}
	...
	putAction(t, e, O, i) {
		for(let e = 0; e < i; e += 3)
			if(this.actions[e] == t) return i;
		return this.actions[i++] = t, this.actions[i++] = e, this.actions[i++] = O, i
	}
	addActions(t, e, O, i) {
		let {
			state: a
		...
		for(let t = 0; t < 2; t++)
			for(let r = n.stateSlot(a, t ? 2 : 1);; r += 3) {
				if(65535 == s[r]) {
					if(1 != s[r + 1]) {
						0 == i && 2 == s[r + 1] && (i = this.putAction(U(s, r + 2), e, O, i));
						break
					}
					r = U(s, r + 2)
				}
				s[r] == e && (i = this.putAction(U(s, r + 1), e, O, i))
			}
		return i
	}
}
class S {
	constructor(t, e, O, i) {
		this.parser = t, this.input = e, this.ranges = i, this.recovering = 0, this.nextStackID = 9812, this.minStackPos = 0, this.reused = [], this.stoppedAt = null, this.lastBigReductionStart = -1, this.lastBigReductionSize = 0, this.bigReductionCount = 0, this.stream = new h(e, i), this.tokens = new $(t, this.stream), this.topTerm = t.top[1];
		let {
			from: a
		} = i[0];
		this.stacks = [s.start(this, t.top[0], a)], this.fragments = O.length && this.stream.end - a > 4 * t.bufferLength ? new b(O, t.nodeSet) : null
	}
	get parsedPos() {
		return this.minStackPos
	}
	advance() {
		let t, e, O = this.stacks,
			i = this.minStackPos,
			a = this.stacks = [];
		...
		for(let n = 0; n < O.length; n++) {
			let s = O[n];
			for(;;) {
				if(this.tokens.mainToken = null, s.pos > i) a.push(s);
				else {
					if(this.advanceStack(s, a, O)) continue;
					{
						t || (t = [], e = []), t.push(s);
						let O = this.tokens.getMainToken(s);
						e.push(O.value, O.end)
					}
				}
				break
			}
		...
		if(!a.length) {
			let e = t && function(t) {
				...
				for(let O of t) {
					let t = O.p.stoppedAt;
					(O.pos == O.p.stream.end || null != t && O.pos > t) && O.p.parser.stateFlag(O.state, 2) && (!e || e.score < O.score) && (e = O)
				}
				return e
			}(t);
			if(e) return g && console.log("Finish with " + this.stackID(e)), this.stackToTree(e);
		...
		if(this.recovering && t) {
			let O = null != this.stoppedAt && t[0].pos > this.stoppedAt ? t[0] : this.runRecovery(t, e, a);
			if(O) return g && console.log("Force-finish " + this.stackID(O)), this.stackToTree(O.forceAll())
		}
		if(this.recovering) {
			let t = 1 == this.recovering ? 1 : 3 * this.recovering;
			if(a.length > t)
				for(a.sort((t, e) => e.score - t.score); a.length > t;) a.pop();
			a.some(t => t.reducePos > i) && this.recovering--
		} else if(a.length > 1) {
			t: for(let t = 0; t < a.length - 1; t++) {
				let e = a[t];
			...
			a.length > 12 && (a.sort((t, e) => e.score - t.score), a.splice(12, a.length - 12))
		}
		this.minStackPos = a[0].pos;
		for(let t = 1; t < a.length; t++) a[t].pos < this.minStackPos && (this.minStackPos = a[t].pos);
		return null
	}
	stopAt(t) {
	...
	advanceStack(e, O, i) {
		let a = e.pos,
			...
			s = g ? this.stackID(e) + " -> " : "";
		if(null != this.stoppedAt && a > this.stoppedAt) return e.forceReduce() ? e : null;
		if(this.fragments) {
			let O = e.curContext && e.curContext.tracker.strict,
				i = O ? e.curContext.hash : 0;
			for(let r = this.fragments.nodeAt(a); r;) {
				let a = this.parser.nodeSet.types[r.type.id] == r.type ? n.getGoto(e.state, r.type.id) : -1;
				if(a > -1 && r.length && (!O || (r.prop(t.NodeProp.contextHash) || 0) == i)) return e.useNode(r, a), g && console.log(s + this.stackID(e) + ` (via reuse of ${n.getName(r.type.id)})`), !0;
				if(!(r instanceof t.Tree) || 0 == r.children.length || r.positions[0] > 0) break;
				let o = r.children[0];
				if(!(o instanceof t.Tree && 0 == r.positions[0])) break;
				r = o
			}
		}
		let r = n.stateSlot(e.state, 4);
		if(r > 0) return e.reduce(r), g && console.log(s + this.stackID(e) + ` (via always-reduce ${n.getName(65535&r)})`), !0;
		...
		for(let t = 0; t < o.length;) {
			...
			if(Q.apply(r, l, h ? h.start : Q.pos, d), g && console.log(s + this.stackID(Q) + ` (via ${65536&r?`reduce of ${n.getName(65535&r)}`:"shift"} for ${n.getName(l)} @ ${a}${Q==e?"":", split"})`), c) return !0;
			Q.pos > a ? O.push(Q) : i.push(Q)
		}
		return !1
	}
	advanceFully(t, e) {
	...
	runRecovery(t, e, O) {
		...
		for(let n = 0; n < t.length; n++) {
			...
			if(s.deadEnd) {
				if(a) continue;
				if(a = !0, s.restart(), g && console.log(l + this.stackID(s) + " (restarted)"), this.advanceFully(s, O)) continue
			}
			let d = s.split(),
				c = l;
			for(let t = 0; t < 10 && d.forceReduce(); t++) {
				if(g && console.log(c + this.stackID(d) + " (via force-reduce)"), this.advanceFully(d, O)) break;
				g && (c = this.stackID(d) + " -> ")
			}
			for(let t of s.recoverByInsert(r)) g && console.log(l + this.stackID(t) + " (via recover-insert)"), this.advanceFully(t, O);
			this.stream.end > s.pos ? (o == s.pos && (o++, r = 0), s.recoverByDelete(r, o), g && console.log(l + this.stackID(s) + ` (via recover-delete ${this.parser.getName(r)})`), P(s, O)) : (!i || i.score < d.score) && (i = d)
		}
		return i
	}
	stackToTree(e) {
		return e.close(), t.Tree.build({
			buffer: l.create(e),
	...
	stackID(t) {
		let e = (X || (X = new WeakMap)).get(t);
		return e || X.set(t, e = String.fromCodePoint(this.nextStackID++)), e + t
	}
}
...
function P(t, e) {
	for(let O = 0; O < e.length; O++) {
		let i = e[O];
		if(i.pos == t.pos && i.sameState(t)) return void(e[O].score < t.score && (e[O] = t))
	}
	e.push(t)
}
class k {
	constructor(t, e, O) {
		this.source = t, this.flags = e, this.disabled = O
	}
	allows(t) {
...
class x extends t.Parser {
	constructor(e) {
		...
		for(let t = 0; t < e.repeatNodeCount; t++) O.push("");
		let i = Object.keys(e.topRules).map(t => e.topRules[t][1]),
			a = [];
		for(let t = 0; t < O.length; t++) a.push([]);

		function n(t, e, O) {
			a[t].push([e, e.deserialize(String(O))])
		}
		if(e.nodeProps)
			for(let O of e.nodeProps) {
				let e = O[0];
				"string" == typeof e && (e = t.NodeProp[e]);
				for(let t = 1; t < O.length;) {
					let i = O[t++];
					if(i >= 0) n(i, e, O[t++]);
					else {
						let a = O[t + -i];
						for(let s = -i; s > 0; s--) n(O[t++], e, a);
						t++
					}
				}
			}
		this.nodeSet = new t.NodeSet(O.map((O, n) => t.NodeType.define({
			name: n >= this.minRepeatTerm ? void 0 : O,
			id: n,
			props: a[n],
			top: i.indexOf(n) > -1,
			error: 0 == n,
			skipped: e.skippedNodes && e.skippedNodes.indexOf(n) > -1
		}))), e.propSources && (this.nodeSet = this.nodeSet.extend(...e.propSources)), this.strict = !1, this.bufferLength = t.DefaultBufferLength;
		let s = d(e.tokenData);
		this.context = e.context, this.specializerSpecs = e.specialized || [], this.specialized = new Uint16Array(this.specializerSpecs.length);
		for(let t = 0; t < this.specializerSpecs.length; t++) this.specialized[t] = this.specializerSpecs[t].term;
		this.specializers = this.specializerSpecs.map(T), this.states = d(e.states, Uint32Array), this.data = d(e.stateData), this.goto = d(e.goto), this.maxTerm = e.maxTerm, this.tokenizers = e.tokenizers.map(t => "number" == typeof t ? new f(s, t) : t), this.topRules = e.topRules, this.dialects = e.dialects || {}, this.dynamicPrecedences = e.dynamicPrecedences || null, this.tokenPrecTable = e.tokenPrec, this.termNames = e.termNames || null, this.maxNode = this.nodeSet.types.length - 1, this.dialect = this.parseDialect(), this.top = this.topRules[Object.keys(this.topRules)[0]]
	}
	createParse(t, e, O) {
		let i = new S(this, t, e, O);
		for(let a of this.wrappers) i = a(i, t, e, O);
		return i
	}
	getGoto(t, e, O = !1) {
		let i = this.goto;
	...
	hasAction(t, e) {
		let O = this.data;
		for(let i = 0; i < 2; i++)
			for(let a, n = this.stateSlot(t, i ? 2 : 1);; n += 3) {
				if(65535 == (a = O[n])) {
					if(1 != O[n + 1]) {
						if(2 == O[n + 1]) return U(O, n + 2);
						break
					}
					a = O[n = U(O, n + 2)]
				}
				if(a == e || 0 == a) return U(O, n + 1)
			}
		return 0
	}
	stateSlot(t, e) {
	...
	allActions(t, e) {
		let O = this.stateSlot(t, 4),
			i = O ? e(O) : void 0;
		for(let O = this.stateSlot(t, 1); null == i; O += 3) {
	...
	nextStates(t) {
		let e = [];
		for(let O = this.stateSlot(t, 1);; O += 3) {
			if(65535 == this.data[O]) {
				if(1 != this.data[O + 1]) break;
				O = U(this.data, O + 2)
			}
			if(!(1 & this.data[O + 2])) {
				let t = this.data[O + 1];
				e.some((e, O) => 1 & O && e == t) || e.push(this.data[O], t)
			}
		}
		return e
	}
	configure(t) {
		let e = Object.assign(Object.create(x.prototype), this);
		if(t.props && (e.nodeSet = this.nodeSet.extend(...t.props)), t.top) {
			let O = this.topRules[t.top];
			if(!O) throw new RangeError(`Invalid top rule name ${t.top}`);
			e.top = O
		}
		return t.tokenizers && (e.tokenizers = this.tokenizers.map(e => {
			let O = t.tokenizers.find(t => t.from == e);
			return O ? O.to : e
		})), t.specializers && (e.specializers = this.specializers.slice(), e.specializerSpecs = this.specializerSpecs.map((O, i) => {
			let a = t.specializers.find(t => t.from == O.external);
			if(!a) return O;
			let n = Object.assign(Object.assign({}, O), {
				external: a.to
			});
			return e.specializers[i] = T(n), n
	...
	parseDialect(t) {
		...
		for(let t = 0; t < e.length; t++)
			if(!O[t])
				for(let O, a = this.dialects[e[t]]; 65535 != (O = this.data[a++]);)(i || (i = new Uint8Array(this.maxTerm + 1)))[O] = 1;
		return new k(t, O, i)
	}
	...
	if(O && "object" == typeof O && null == O.nodeType && !Array.isArray(O)) {
		for(var i in O)
			if(Object.prototype.hasOwnProperty.call(O, i)) {
				var a = O[i];
				"string" == typeof a ? t.setAttribute(i, a) : null != a && (t[i] = a)
			} e++
	}
	for(; e < arguments.length; e++) y(t, arguments[e]);
	return t
}
...
function y(t, e) {
	if("string" == typeof e) t.appendChild(document.createTextNode(e));
		...
		for(var O = 0; O < e.length; O++) y(t, e[O])
	}
}
class G {
	constructor(t, e, O) {
		this.from = t, this.to = e, this.diagnostic = O
	}
}
class v {
	constructor(t, e, O) {
		this.diagnostics = t, this.panel = e, this.selected = O
	}
	static init(t, e, O) {
		let n = O.facet(L).markerFilter;
		n && (t = n(t, O));
		let s = t.slice().sort((t, e) => t.from - e.from || t.to - e.to),
			r = new a.RangeSetBuilder,
			o = [],
			l = 0,
			d = O.doc.iter(),
			c = 0,
			Q = O.doc.length;
		for(let t = 0;;) {
			let e, O, a = t == s.length ? null : s[t];
			if(!a && !o.length) break;
			if(o.length) e = l, O = o.reduce((t, e) => Math.min(t, e.to), a && a.from > e ? a.from : 1e8);
			else {
				if(e = a.from, e > Q) break;
				O = a.to, o.push(a), t++
			}
			for(; t < s.length;) {
				let i = s[t];
				...
				o.push(i), t++, O = Math.min(i.to, O)
			}
			O = Math.min(O, Q);
			let n = !1;
			if(o.some(t => t.from == e && (t.to == O || O == Q)) && (n = e == O, !n && O - e < 10)) {
				let t = e - (c + d.value.length);
				t > 0 && (d.next(t), c = e);
				for(let t = e;;) {
					if(t >= O) {
						n = !0;
					...
					if(!d.lineBreak && c + d.value.length > t) break;
					t = c + d.value.length, c += d.value.length, d.next()
				}
			}
			let h = et(o);
			if(n) r.add(e, e, i.Decoration.widget({
				widget: new D(h),
				diagnostics: o.slice()
			}));
			else {
				let t = o.reduce((t, e) => e.markClass ? t + " " + e.markClass : t, "");
				r.add(e, O, i.Decoration.mark({
					class: "cm-lintRange cm-lintRange-" + h + t,
					diagnostics: o.slice(),
					inclusiveEnd: o.some(t => t.to > O)
				}))
			}
			if(l = O, l == Q) break;
			for(let t = 0; t < o.length; t++) o[t].to <= l && o.splice(t--, 1)
		}
		let h = r.finish();
		return new v(h, e, R(h))
	}
}

function R(t, e = null, O = 0) {
	let i = null;
	return t.between(O, 1e9, (t, O, {
		spec: a
	}) => {
		if(!(e && a.diagnostics.indexOf(e) < 0))
			if(i) {
				if(a.diagnostics.indexOf(i.diagnostic) < 0) return !1;
				i = new G(i.from, O, i.diagnostic)
			} else i = new G(t, O, e || a.diagnostics[0])
	}), i
}

function j(t, e) {
	return t.field(z, !1) ? e : e.concat(a.StateEffect.appendConfig.of(Ot))
}
const V = a.StateEffect.define(),
	C = a.StateEffect.define(),
	Y = a.StateEffect.define(),
	z = a.StateField.define({
		create: () => new v(i.Decoration.none, null, null),
		update(t, e) {
			if(e.docChanged && t.diagnostics.size) {
				let O = t.diagnostics.map(e.changes),
					i = null,
					a = t.panel;
				if(t.selected) {
					let a = e.changes.mapPos(t.selected.from, 1);
					i = R(O, t.selected.diagnostic, a) || R(O, null, a)
				}!O.size && a && e.state.facet(L).autoPanel && (a = null), t = new v(O, a, i)
			}
			for(let O of e.effects)
				if(O.is(V)) {
					let i = e.state.facet(L).autoPanel ? O.value.length ? M.open : null : t.panel;
					t = v.init(O.value, i, e.state)
				} else O.is(C) ? t = new v(t.diagnostics, O.value ? M.open : null, t.selected) : O.is(Y) && (t = new v(t.diagnostics, t.panel, O.value));
			return t
		},
		provide: t => [i.showPanel.from(t, t => t.panel), i.EditorView.decorations.from(t, t => t.diagnostics)]
	}),
	_ = i.Decoration.mark({
...
function A(t, e, O) {
	let i, {
		...
		n = -1,
		s = -1;
	a.between(e - (O < 0 ? 1 : 0), e + (O > 0 ? 1 : 0), (t, a, {
		spec: r
	}) => {
		if(e >= t && e <= a && (t == a || (e > t || O > 0) && (e < a || O < 0))) return i = r.diagnostics, n = t, s = a, !1
	});
	let r = t.state.facet(L).tooltipFilter;
	return i && r && (i = r(i, t.state)), i ? {
		pos: n,
		end: s,
...
const N = t => {
		let e = t.state.field(z, !1);
		return !(!e || !e.panel) && (t.dispatch({
			effects: C.of(!1)
		}), !0)
	},
	W = i.ViewPlugin.fromClass(class {
		constructor(t) {
		...
		run() {
			...
			if(t < this.lintTime - 10) this.timeout = setTimeout(this.run, this.lintTime - t);
				...
				e.length && function(t, e, O) {
					let i = [],
						a = -1;
					for(let n of t) n.then(O => {
						i.push(O), clearTimeout(a), i.length == t.length ? e(i) : a = setTimeout(() => e(i), 200)
					}, O)
				}(e.map(t => Promise.resolve(t(this.view))), e => {
					this.view.state.doc == t.doc && this.view.dispatch(function(t, e) {
						return {
							effects: j(t, [V.of(e)])
						}
					}(this.view.state, e.reduce((t, e) => t.concat(e))))
				}, t => {
					i.logException(this.view.state, t)
				})
			}
		}
		update(t) {
			let e = t.state.facet(L);
			(t.docChanged || e != t.startState.facet(L) || e.needsRefresh && e.needsRefresh(t)) && (this.lintTime = Date.now() + e.delay, this.set || (this.set = !0, this.timeout = setTimeout(this.run, e.delay)))
		}
		force() {
...
const L = a.Facet.define({
	combine: t => ({
		sources: t.map(t => t.source).filter(t => null != t),
		...a.combineConfig(t.map(t => t.config), {
			delay: 750,
			markerFilter: null,
			tooltipFilter: null,
			needsRefresh: null,
			...
			markerFilter: B,
			tooltipFilter: B,
			needsRefresh: (t, e) => t ? e ? O => t(O) || e(O) : t : e,
			hideOn: (t, e) => t ? e ? (O, i, a) => t(O, i, a) || e(O, i, a) : t : e,
			autoPanel: (t, e) => t || e
		})
	})
});
...
function K(t) {
	...
	if(t) t: for(let {
		...
		for(let t = 0; t < O.length; t++) {
			let i = O[t];
			if(/[a-zA-Z]/.test(i) && !e.some(t => t.toLowerCase() == i.toLowerCase())) {
				e.push(i);
				continue t
			}
		}
...
function I(t, e, O) {
	...
	}, e.renderMessage ? e.renderMessage(t) : e.message), null === (i = e.actions) || void 0 === i ? void 0 : i.map((O, i) => {
		let n = !1,
			s = i => {
				if(i.preventDefault(), n) return;
				n = !0;
				let a = R(t.state.field(z).diagnostics, e);
				a && O.apply(t, a.from, a.to)
			},
			{
				name: r
			} = O,
			o = a[i] ? r.indexOf(a[i]) : -1,
			l = o < 0 ? r : [r.slice(0, o), w("u", r.slice(o, o + 1)), r.slice(o + 1)];
		return w("button", {
			type: "button",
			class: "cm-diagnosticAction" + (O.markClass ? " " + O.markClass : ""),
			onclick: s,
			onmousedown: s,
			"aria-label": ` Action: ${r}${o<0?"":` (access key "${a[i]})"`}.`
		}, l)
	}), e.source && w("div", {
		class: "cm-diagnosticSource"
	}, e.source))
}
class D extends i.WidgetType {
	constructor(t) {
		super(), this.sev = t
	}
	eq(t) {
...
class F {
	constructor(t, e) {
		this.diagnostic = e, this.id = "item_" + Math.floor(4294967295 * Math.random()).toString(16), this.dom = I(t, e, !0), this.dom.id = this.id, this.dom.setAttribute("role", "option")
	}
}
class M {
	constructor(t) {
		...
		this.list = w("ul", {
			...
			onkeydown: e => {
				if(27 == e.keyCode) N(this.view), this.view.focus();
					...
					if(!(e.keyCode >= 65 && e.keyCode <= 90 && this.selectedIndex >= 0)) return;
					{
						let {
							diagnostic: O
						} = this.items[this.selectedIndex], i = K(O.actions);
						for(let a = 0; a < i.length; a++)
							if(i[a].toUpperCase().charCodeAt(0) == e.keyCode) {
								let e = R(this.view.state.field(z).diagnostics, O);
								e && O.actions[a].apply(t, e.from, e.to)
							}
					}
				}
				e.preventDefault()
	...
	get selectedIndex() {
		let t = this.view.state.field(z).selected;
		if(!t) return -1;
	...
	update() {
		let {
			diagnostics: t,
			selected: e
		} = this.view.state.field(z), O = 0, i = !1, a = null, n = new Set;
		for(t.between(0, this.view.state.doc.length, (t, s, {
				spec: r
			}) => {
				for(let t of r.diagnostics) {
					if(n.has(t)) continue;
					n.add(t);
					let s, r = -1;
					for(let e = O; e < this.items.length; e++)
						if(this.items[e].diagnostic == t) {
							r = e;
							break
						} r < 0 ? (s = new F(this.view, t), this.items.splice(O, 0, s), i = !0) : (s = this.items[r], r > O && (this.items.splice(O, r - O), i = !0)), e && s.diagnostic == e.diagnostic ? s.dom.hasAttribute("aria-selected") || (s.dom.setAttribute("aria-selected", "true"), a = s) : s.dom.hasAttribute("aria-selected") && s.dom.removeAttribute("aria-selected"), O++
				}
			}); O < this.items.length && !(1 == this.items.length && this.items[0].diagnostic.from < 0);) i = !0, this.items.pop();
		0 == this.items.length && (this.items.push(new F(this.view, {
			from: -1,
			to: -1,
			severity: "info",
			...
			write: ({
				...
				let O = e.height / this.list.offsetHeight;
				t.top < e.top ? this.list.scrollTop -= (e.top - t.top) / O : t.bottom > e.bottom && (this.list.scrollTop += (t.bottom - e.bottom) / O)
			}
	...
	sync() {
		...
		for(let O of this.items)
			if(O.dom.parentNode == this.list) {
				for(; t != O.dom;) e();
				t = O.dom.nextSibling
			} else this.list.insertBefore(O.dom, t);
		for(; t;) e()
	}
	moveSelection(t) {
		if(this.selectedIndex < 0) return;
		let e = R(this.view.state.field(z).diagnostics, this.items[t].diagnostic);
...
const H = i.EditorView.baseTheme({
	".cm-diagnostic": {
	...
	".cm-lintRange-error": {
		backgroundImage: J("#d11")
	},
	".cm-lintRange-warning": {
		backgroundImage: J("orange")
	},
	".cm-lintRange-info": {
		backgroundImage: J("#999")
	},
	".cm-lintRange-hint": {
		backgroundImage: J("#66d")
	},
	".cm-lintRange-active": {
...
function et(t) {
	...
	for(let i of t) {
		let t = tt(i.severity);
		t > O && (O = t, e = i.severity)
	}
	return e
}
const Ot = [z, i.EditorView.decorations.compute([z], t => {
	let {
		selected: e,
		panel: O
	} = t.field(z);
	return e && O && e.from != e.to ? i.Decoration.set([_.range(e.from, e.to)]) : i.Decoration.none
}), i.hoverTooltip(A, {
	hideOn: function(t, e) {
		let O = e.pos,
			i = e.end || O,
			a = t.state.facet(L).hideOn(t, O, i);
		if(null != a) return a;
		let n = t.startState.doc.lineAt(e.pos);
		return !(!t.effects.some(t => t.is(V)) && !t.changes.touchesRange(n.from, Math.max(n.to, i)))
	}
}), H];

function it(t) {
	return t >= 65 && t <= 90 || t >= 97 && t <= 122 || t >= 48 && t <= 57 || 42 === t || 43 === t || 64 === t
}
...
function ot(t, e) {
	const O = t.pos + e;
	if(st === t && rt === O) return nt;
	if(t.peek(e) !== "{".charCodeAt(0)) return;
	e++;
	let i = "";
	for(;;) {
		const O = t.peek(e);
...
const dt = new class {
		constructor(t) {
			this.start = t.start, this.shift = t.shift || E, this.reduce = t.reduce || E, this.reuse = t.reuse || E, this.hash = t.hash || (() => 0), this.strict = !1 !== t.strict
		}
	}({
		start: null,
		shift: (t, e, O, i) => 9 === e ? new lt(ot(i, 6) || "", t) : t,
		reduce: (t, e) => 94 === e && t ? t.parent : t,
		reuse: (t, e, O, i) => 9 === e.type.id ? new lt(ot(i, 0) || "", t) : t,
		hash: t => t ? t.hash : 0,
		strict: !1
	}),
	ct = new u((t, e) => {
		t.next === "*".charCodeAt(0) && t.advance();
		const O = t.next;
		if(-1 !== O && !/\s|\*/.test(String.fromCharCode(O))) {
			for(t.advance();;) {
				const e = t.next;
				if(-1 === e || e === Pt) return;
				if(t.advance(), e === O) break
			}
	...
	Qt = new u((t, e) => {
		let O = t.next;
		if(-1 !== O && !/\s/.test(String.fromCharCode(O))) {
			for(O === Xt && (O = bt), t.advance();;) {
				const e = t.next;
				if(-1 === e || e === Pt) return;
				if(t.advance(), e === O) break
			}
			return t.acceptToken(2)
		}
	}, {
		contextual: !1
	}),
	ht = (t, e, O = 0) => {
		for(let i = 0; i < e.length; i++)
			if(String.fromCharCode(t.peek(O + i)) !== e[i]) return !1;
		return !0
	},
	ft = new u((t, e) => {
		const O = "\\end{" + e.context.name + "}";
		for(let e = 0;; e++) {
			if(-1 === t.peek(e) || ht(t, O, e)) return t.acceptToken(5, e)
		}
	}, {
		contextual: !1
	}),
	ut = new u(t => {
		for(let e = 0;; e++) {
			const O = t.peek(e);
			if(-1 === O || O === bt) return t.acceptToken(3, e)
		}
	}, {
		contextual: !1
	}),
	mt = new u(t => {
		for(let e = 0;; e++) {
			const O = t.peek(e);
			if(-1 === O || O === St || O === Pt) return t.acceptToken(4, e)
		}
	}, {
		contextual: !1
	});
...
const gt = pt("\\"),
	...
	kt = t => new u(e => {
		for(let O = 0; O < 100; ++O) {
			const i = e.peek(O);
			if(i === St || i === $t) continue;
			const a = t(i);
			if(a) return void e.acceptToken(a)
		}
	}, {
		contextual: !1,
		fallback: !0
	}),
	Et = kt(t => t === Xt ? 376 : 377),
	xt = kt(t => t === Xt || t === qt ? 378 : 379),
	Ut = pt("@"),
	Tt = new u((t, e) => {
		let O = 0,
			i = -1;
		...
		if(-1 !== a && a === gt) {
			...
			if(-1 !== i) return t.acceptToken(6, i + 1)
		}
	}),
	wt = "\\end{document}".split("").reverse(),
	yt = new u((t, e) => {
		if(-1 !== t.next) {
			for(let e = 1; e < wt.length + 1; e++)
				if(String.fromCharCode(t.peek(-e)) !== wt[e - 1]) return;
			for(; t.next === St || t.next === Pt;) {
				if(-1 === t.advance()) return t.acceptToken(7)
			}
			for(; - 1 !== t.advance(););
			return t.acceptToken(8)
		}
	}),
	Gt = new Set(["\\fullref", "\\Vref", "\\autopageref", "\\autoref", "\\eqref", "\\labelcpageref", "\\labelcref", "\\lcnamecref", "\\lcnamecrefs", "\\namecref", "\\nameCref", "\\namecrefs", "\\nameCrefs", "\\thnameref", "\\thref", "\\titleref", "\\vrefrange", "\\Crefrange", "\\Crefrang", "\\fref", "\\pref", "\\tref", "\\Aref", "\\Bref", "\\Pref", "\\Sref", "\\vref", "\\nameref"]),
	vt = new Set(["\\vpageref", "\\vref", "\\zcpageref", "\\zcref", "\\zfullref", "\\zref", "\\zvpageref", "\\zvref", "\\cref", "\\Cref", "\\pageref", "\\ref", "\\Ref", "\\subref", "\\zpageref", "\\ztitleref", "\\vpagerefrange", "\\zvpagerefrange", "\\zvrefrange", "\\crefrange"]),
	Rt = new Set(["\\autocites", "\\Autocites", "\\Cite", "\\citeA", "\\citealp", "\\Citealp", "\\citealt", "\\Citealt", "\\citeauthorNP", "\\citeauthorp", "\\Citeauthorp", "\\citeauthort", "\\Citeauthort", "\\citeNP", "\\citenum", "\\citen", "\\citeonline", "\\cites", "\\Cites", "\\citeurl", "\\citeyearpar", "\\defcitealias", "\\fnotecite", "\\footcite", "\\footcitetext", "\\footfullcite", "\\footnotecites", "\\Footnotecites", "\\fullcite", "\\fullciteA", "\\fullciteauthor", "\\fullciteauthorNP", "\\maskcite", "\\maskciteA", "\\maskcitealp", "\\maskCitealp", "\\maskcitealt", "\\maskCitealt", "\\maskciteauthor", "\\maskciteauthorNP", "\\maskciteauthorp", "\\maskCiteauthorp", "\\maskciteauthort", "\\maskCiteauthort", "\\maskciteNP", "\\maskcitenum", "\\maskcitep", "\\maskCitep", "\\maskcitepalias", "\\maskcitet", "\\maskCitet", "\\maskcitetalias", "\\maskciteyear", "\\maskciteyearNP", "\\maskciteyearpar", "\\maskfullcite", "\\maskfullciteA", "\\maskfullciteauthor", "\\maskfullciteauthorNP", "\\masknocite", "\\maskshortcite", "\\maskshortciteA", "\\maskshortciteauthor", "\\maskshortciteauthorNP", "\\maskshortciteNP", "\\mautocite", "\\Mautocite", "\\mcite", "\\Mcite", "\\mfootcite", "\\mfootcitetext", "\\mparencite", "\\Mparencite", "\\msupercite", "\\mtextcite", "\\Mtextcite", "\\nocite", "\\nocitemeta", "\\notecite", "\\Parencite", "\\parencites", "\\Parencites", "\\pnotecite", "\\shortcite", "\\shortciteA", "\\shortciteauthor", "\\shortciteauthorNP", "\\shortciteNP", "\\smartcite", "\\Smartcite", "\\smartcites", "\\Smartcites", "\\supercite", "\\supercites", "\\textcite", "\\Textcite", "\\textcites", "\\Textcites"]),
	jt = new Set(["\\cite", "\\citeauthor", "\\Citeauthor", "\\citedate", "\\citep", "\\citepalias", "\\Citep", "\\citetitle", "\\citeyear", "\\parencite", "\\citet", "\\citetalias", "\\autocite", "\\Autocite"]),
	Vt = new Set(["\\label", "\\thlabel", "\\zlabel"]),
	Ct = new Set(["\\text", "\\tag", "\\textrm", "\\intertext"]),
	Yt = {
		"\\hbox": 17,
		"\\title": 18,
		"\\author": 51,
	...
	zt = (t, e) => "\\begin" === t ? 9 : "\\end" === t ? 10 : Gt.has(t) ? 11 : vt.has(t) ? 12 : Rt.has(t) ? 13 : jt.has(t) ? 14 : Vt.has(t) ? 15 : Ct.has(t) ? 16 : Yt[t] || -1,
	_t = new Set(["tabular", "xltabular", "tabularx", "longtable"]),
	At = new Set(["equation", "equation*", "displaymath", "displaymath*", "math", "math*", "multline", "multline*", "matrix", "tikzcd"]),
	Zt = new Set(["array", "eqnarray", "eqnarray*", "align", "align*", "alignat", "alignat*", "flalign", "flalign*", "gather", "gather*", "pmatrix", "pmatrix*", "bmatrix", "bmatrix*", "Bmatrix", "Bmatrix*", "vmatrix", "vmatrix*", "Vmatrix", "Vmatrix*", "smallmatrix", "smallmatrix*", "split", "split*", "gathered", "gathered*", "aligned", "aligned*", "alignedat", "alignedat*", "cases", "cases*", "dcases", "dcases*", "rcases", "rcases*", "IEEEeqnarray", "IEEEeqnarray*"]),
	Nt = new Set(["verbatim", "boxedverbatim", "lstlisting", "minted", "Verbatim", "lstlisting", "tcblisting", "codeexample", "comment"]),
	Wt = {
		document: 76,
		tikzpicture: 81,
		figure: 82,
	...
	Lt = (t, e) => _t.has(t) ? 77 : At.has(t) ? 78 : Zt.has(t) ? 79 : Nt.has(t) ? 80 : Wt[t] || -1,
	Bt = {
		"\\(": 85,
		"\\)": 86,
		"\\[": 87,
	...
	It = x.deserialize({
		...
		tokenizers: [ct, Qt, ut, mt, ft, Tt, yt, 0, 1, 2, 3, Et, xt],
		topRules: {
			LaTeX: [0, 91]
		},
		specialized: [{
...
const Dt = ["document", "abstract", "appendix", "frontmatter", "mainmatter", "backmatter", "figure", "figure*", "table", "table*", "wrapfigure", "subfigure", "center", "flushleft", "flushright", "quote", "quotation", "verse", "itemize", "enumerate", "description", "list", "verbatim", "verbatim*", "lstlisting", "minted", "Verbatim", "comment", "math", "displaymath", "equation", "equation*", "align", "align*", "gather", "gather*", "multline", "multline*", "flalign", "flalign*", "alignat", "alignat*", "array", "cases", "split", "tabular", "tabular*", "tabularx", "longtable", "xltabular", "theorem", "lemma", "corollary", "proposition", "definition", "example", "proof", "tikzpicture", "scope", "minipage"],
	Ft = ["\\documentclass", "\\usepackage", "\\title", "\\author", "\\date", "\\maketitle", "\\tableofcontents", "\\appendix", "\\bibliography", "\\bibliographystyle", "\\part", "\\chapter", "\\section", "\\subsection", "\\subsubsection", "\\paragraph", "\\subparagraph", "\\begin", "\\end", "\\label", "\\ref", "\\pageref", "\\cite", "\\nocite", "\\bibitem", "\\textbf", "\\textit", "\\texttt", "\\textsf", "\\textrm", "\\textsc", "\\emph", "\\underline", "\\textcolor", "\\colorbox", "\\item", "\\itemize", "\\enumerate", "\\includegraphics", "\\caption", "\\figure", "\\frac", "\\sqrt", "\\sum", "\\int", "\\prod", "\\lim", "\\infty", "\\partial", "\\alpha", "\\beta", "\\gamma", "\\delta", "\\epsilon", "\\varepsilon", "\\zeta", "\\eta", "\\theta", "\\iota", "\\kappa", "\\lambda", "\\mu", "\\nu", "\\xi", "\\pi", "\\rho", "\\sigma", "\\tau", "\\upsilon", "\\phi", "\\varphi", "\\chi", "\\psi", "\\omega", "\\Gamma", "\\Delta", "\\Theta", "\\Lambda", "\\Xi", "\\Pi", "\\Sigma", "\\Upsilon", "\\Phi", "\\Psi", "\\Omega", "\\&", "\\%", "\\$", "\\#", "\\_", "\\{", "\\}", "\\\\", "\\quad", "\\qquad", "\\hspace", "\\vspace", "\\hline", "\\cline", "\\multicolumn", "\\multirow", "\\toprule", "\\midrule", "\\bottomrule", "\\newcommand", "\\renewcommand", "\\newenvironment", "\\renewenvironment", "\\def", "\\let", "\\input", "\\include", "\\includeonly"],
	Mt = ["\\sin", "\\cos", "\\tan", "\\arcsin", "\\arccos", "\\arctan", "\\sinh", "\\cosh", "\\tanh", "\\log", "\\ln", "\\exp", "\\min", "\\max", "\\sup", "\\inf", "\\lim", "\\limsup", "\\liminf", "\\det", "\\dim", "\\mod", "\\gcd", "\\lcm", "\\mathop", "\\rightarrow", "\\leftarrow", "\\Rightarrow", "\\Leftarrow", "\\mapsto", "\\approx", "\\sim", "\\simeq", "\\cong", "\\equiv", "\\prec", "\\succ", "\\neq", "\\geq", "\\leq", "\\ll", "\\gg", "\\subset", "\\subseteq", "\\in", "\\notin", "\\cap", "\\cup", "\\setminus", "\\emptyset", "\\varnothing", "\\forall", "\\exists", "\\nexists", "\\mathbb{R}", "\\mathbb{Z}", "\\mathbb{N}", "\\mathbb{Q}", "\\mathbb{C}", "\\hat", "\\tilde", "\\bar", "\\vec", "\\dot", "\\ddot", "\\underline", "\\overline", "\\begin{equation}", "\\begin{align}", "\\begin{align*}", "\\begin{gather}", "\\begin{array}", "\\begin{cases}", "\\begin{matrix}", "\\begin{pmatrix}", "\\begin{bmatrix}", "\\begin{vmatrix}"],
	Jt = ["amsmath", "amssymb", "amsfonts", "amsthm", "mathtools", "graphicx", "xcolor", "hyperref", "url", "geometry", "fancyhdr", "lastpage", "booktabs", "tabularx", "longtable", "multirow", "tikz", "pgfplots", "pgf", "babel", "inputenc", "fontenc", "natbib", "biblatex", "cite", "algorithm", "algorithmic", "listings", "minted", "enumitem", "cleveref", "microtype"];
	...
	if(t.startsWith("\\begin{")) {
		...
		if(O) {
			...
			return {
				...
				type: "function",
				apply: (O, a, n, s) => {
					const r = O.state.doc.lineAt(n).text.match(/^(\s*)/),
						o = r ? r[1] : "",
						l = o + "  ";
					let d = `\\begin{${i}}`,
						c = t.length;
					e && (d += `\n${l}\n${o}\\end{${i}}`, c += 1 + l.length), O.dispatch({
						changes: {
							from: n,
							to: s,
							insert: d
...
const te = [{
	...
	apply: (t, e, O, i) => {
		const a = t.state.doc.lineAt(O).text.match(/^(\s*)/),
			n = a ? a[1] : "",
			s = n + "  ",
			r = `\\begin{figure}[htbp]\n${s}\\centering\n${s}\\includegraphics[width=0.8\\textwidth]{}\n${s}\\caption{}\n${s}\\label{fig:}\n${n}\\end{figure}`;
		t.dispatch({
			changes: {
				from: O,
				to: i,
...
function ee(t) {
	return function(e) {
		if(!e.explicit) {
			const t = e.matchBefore(/\\[a-zA-Z]*$|\\(begin|end)\{[a-zA-Z]*$/);
			if(!t || t.from === t.to) return null
		}
		if(function(t) {
				const e = t.state.sliceDoc(Math.max(0, t.pos - 20), t.pos);
				return /\\(begin|end)\{[^}]*$/.test(e)
			}(e)) {
			const O = e.matchBefore(/\\(begin|end)\{([a-zA-Z]*)$/);
			if(O) {
				const e = Dt.map(e => {
					return i = t, {
						label: O = e,
						type: "class",
						apply: (t, e, a, n) => {
							const s = t.state.doc.lineAt(a),
								r = t.state.sliceDoc(s.from, a);
							if(/\\begin\{[^}]*$/.test(r)) {
								const e = s.text.match(/^(\s*)/),
									r = e ? e[1] : "",
									o = r + "  ";
								let l = `${O}}`,
									d = 1;
								i && (l += `\n${o}\n${r}\\end{${O}}`, d += o.length + 1), t.dispatch({
									changes: {
										from: a,
										to: n,
										insert: l
				...
				return {
					from: O.from + O.text.lastIndexOf("{") + 1,
					options: e,
					validFor: /^[a-zA-Z*]*$/
				}
			}
		...
		if(function(t) {
				const e = t.state.sliceDoc(Math.max(0, t.pos - 30), t.pos);
				return /\\[a-zA-Z]*$/.test(e)
			}(e)) {
			const O = e.matchBefore(/\\([a-zA-Z]*)$/);
			if(O) {
				let i = Ft.map(e => Ht(e, t));
				return function(t) {
					const e = t.state.sliceDoc(0, t.pos).match(/\$/g);
					return !!e && e.length % 2 == 1
				}(e) && (i = [...i, ...Mt.map(e => Ht(e, t))]), i = [...i, ...te], {
					from: O.from,
					options: i,
					validFor: /^\\?[a-zA-Z]*$/
				}
		...
		const O = e.matchBefore(/\\usepackage(\[\S*\])?\{([a-zA-Z,]*)$/);
		return O ? {
			from: O.from + O.text.lastIndexOf("{") + 1,
			options: Jt.map(t => ({
				label: t,
...
const Oe = a.StateEffect.define();

function ie(t, e) {
	const O = t.state.doc.lineAt(e).text.match(/^(\s*)/);
	return O ? O[1] : ""
}
const ae = [a.StateField.define({
	create: () => ({
		active: !1,
		lastEnv: null
	}),
	...
	run: t => {
		const e = function(t) {
			...
			const i = e.doc.lineAt(O.from).text,
				a = /\\begin\{([^}]+)\}[ \t]*$/.exec(i);
			return a ? {
				name: a[1],
				pos: O.from
	...
	run: t => {
		...
		const i = e.doc.lineAt(O.from).from,
			a = e.sliceDoc(i, O.from),
			n = /\\begin\{([^}]*)$/.exec(a);
		if(n) {
			const e = n[1],
				i = ie(t, O.from),
...
}]), a.EditorState.transactionFilter.of(t => {
	if(!t.isUserEvent("input")) return t;
	const e = t.startState,
		O = t.changes;
	let i = [],
		n = t.selection,
		s = !1;
	return O.iterChanges((O, r, o, l, d) => {
		const c = d.sliceString(0),
			Q = /\\begin\{([^}]+)\}$/.exec(c);
		if(Q && t.isUserEvent("input.complete")) {
			const t = Q[1],
				r = e.sliceDoc();
			if(!new RegExp(`\\\\end\\{${t}\\}`).test(r)) {
				const r = e.doc.lineAt(O).text.match(/^(\s*)/),
					o = r ? r[1] : "",
					d = o + "  ";
				i.push({
					from: l,
					insert: `\n${d}\n${o}\\end{${t}}`
				}), n = a.EditorSelection.single(l + d.length + 1), s = !0
			}
		}
	}), s ? [t, {
		changes: i,
...
function ne(t = {}) {
	...
	return t => {
		const i = [],
			a = e.syntaxTree(t.state),
			n = t.state.doc;
		if(O.checkMissingDocumentEnv) {
			let t = !1;
			a.cursor().iterate(e => {
				if("DocumentEnvironment" === e.name || "DocumentEnvName" === e.name) return t = !0, !1
			}), !t && n.length > 100 && i.push({
				from: 0,
				to: Math.min(n.length, 200),
				severity: "warning",
				message: "Missing document environment. LaTeX documents should be enclosed in \\begin{document}...\\end{document}",
				source: "LaTeX"
			})
		}
		if(O.checkUnmatchedEnvironments) {
			const t = new Map,
				e = new Map;
			a.cursor().iterate(O => {
				var i, a;
				if("BeginEnv" === O.name) {
					const e = se(O);
					if(e) {
						const a = n.sliceString(e.from, e.to);
						t.has(a) || t.set(a, []), null === (i = t.get(a)) || void 0 === i || i.push(O.from)
					}
				} else if("EndEnv" === O.name) {
					const t = se(O);
					if(t) {
						const i = n.sliceString(t.from, t.to);
						e.has(i) || e.set(i, []), null === (a = e.get(i)) || void 0 === a || a.push(O.from)
					}
				}
			});
			for(const [O, a] of t.entries()) {
				...
				if(a.length > t.length)
					for(let e = t.length; e < a.length; e++) i.push({
						from: a[e],
						to: a[e] + 6 + O.length + 1,
						severity: "error",
						message: `Missing \\end{${O}}`,
						source: "LaTeX"
					})
			}
			for(const [O, a] of e.entries()) {
				...
				if(a.length > e.length)
					for(let t = e.length; t < a.length; t++) i.push({
						from: a[t],
						to: a[t] + 4 + O.length + 1,
						severity: "error",
						message: `Missing \\begin{${O}}`,
						source: "LaTeX"
					})
			}
		}
		if(O.checkMissingReferences) {
			const t = new Set,
				e = new Map;
			a.cursor().iterate(O => {
				var i;
				if("LabelCtrlSeq" === O.name) {
					const e = O.node.nextSibling;
					if(e && "LabelArgument" === e.name) {
						const O = n.sliceString(e.from + 1, e.to - 1);
						t.add(O)
					}
				} else if("RefCtrlSeq" === O.name || "RefStarrableCtrlSeq" === O.name) {
					const t = null === (i = O.node.nextSibling) || void 0 === i ? void 0 : i.nextSibling;
					if(t && "RefArgument" === t.name) {
						const i = n.sliceString(t.from + 1, t.to - 1);
						e.set(i, O.from)
					}
				}
			});
			for(const [O, a] of e.entries()) t.has(O) || i.push({
				from: a,
				to: a + O.length + 6,
				severity: "warning",
				message: `Reference to undefined label: ${O}`,
				source: "LaTeX"
			})
		}
		return i
...
function se(t) {
	let e = null;
	for(let O = t.node.firstChild; O; O = O.nextSibling)
		if("EnvNameGroup" === O.name) {
			e = O;
			break
		} if(e)
		for(let t = e.firstChild; t; t = t.nextSibling)
			if("EnvName" === t.name || "DocumentEnvName" === t.name || "TabularEnvName" === t.name || "EquationEnvName" === t.name || "EquationArrayEnvName" === t.name || "VerbatimEnvName" === t.name || "TikzPictureEnvName" === t.name || "FigureEnvName" === t.name || "ListEnvName" === t.name || "TableEnvName" === t.name) return t;
	return null
}
const re = {
		"\\documentclass": {
	...
	le = i.hoverTooltip((t, O, i) => {
		const a = e.syntaxTree(t.state).resolve(O);
		if("CtrlSeq" === a.name || a.name.endsWith("CtrlSeq") || "Begin" === a.name || "End" === a.name) {
			const e = t.state.sliceDoc(a.from, a.to),
				O = re[e];
			if(O) return de(O, a.from, a.to)
		}
		if("EnvName" === a.name || a.name.endsWith("EnvName")) {
			const e = t.state.sliceDoc(a.from, a.to),
				O = oe[e];
			if(O) return de(O, a.from, a.to)
		}
		const n = a.parent;
		if(n && "EnvNameGroup" === n.name) {
			const e = function(t) {
				if(!t) return null;
				for(let e = t.firstChild; e; e = e.nextSibling)
					if("EnvName" === e.name || "DocumentEnvName" === e.name || "TabularEnvName" === e.name || "EquationEnvName" === e.name || "EquationArrayEnvName" === e.name || "VerbatimEnvName" === e.name || "TikzPictureEnvName" === e.name || "FigureEnvName" === e.name || "ListEnvName" === e.name || "TableEnvName" === e.name) return e;
				return null
			}(n);
			if(e) {
				const O = t.state.sliceDoc(e.from, e.to),
					i = oe[O];
				if(i) return de(i, e.from, e.to)
			}
		}
		return null
	});
...
function de(t, e, O) {
	...
	a.textContent = t.description, a.className = "cm-latex-tooltip-description", i.appendChild(a);
	let n = document.createElement("div");
	if(n.textContent = "Syntax: " + t.syntax, n.className = "cm-latex-tooltip-syntax", i.appendChild(n), t.example) {
		let e = document.createElement("div");
		e.textContent = "Example: " + t.example, e.className = "cm-latex-tooltip-example", i.appendChild(e)
	}
	if(t.package) {
		let e = document.createElement("div");
		e.textContent = "Package: " + t.package, e.className = "cm-latex-tooltip-package", i.appendChild(e)
	}
	return {
		pos: e,
		end: O,
...
function Qe(t, e, O) {
	...
	if(a.text.startsWith("% {"))
		for(let t = a.number + 1; t <= i.lines; t++) {
			const e = i.line(t);
...
const he = e.LRLanguage.define({
		parser: It.configure({
			props: [e.indentNodeProp.add({
				Environment: t => t.baseIndent + t.unit,
				KnownEnvironment: t => t.baseIndent + t.unit,
				Group: t => t.baseIndent + t.unit,
				BeginEnv: t => t.baseIndent + t.unit,
				"Content TextArgument LongArg": t => t.baseIndent + t.unit
			}), e.foldNodeProp.add({
				Environment: e.foldInside,
				KnownEnvironment: e.foldInside,
				Group: e.foldInside,
				DocumentEnvironment: e.foldInside,
				TabularEnvironment: e.foldInside,
				EquationEnvironment: e.foldInside,
				EquationArrayEnvironment: e.foldInside,
				VerbatimEnvironment: e.foldInside,
				TikzPictureEnvironment: e.foldInside,
				FigureEnvironment: e.foldInside,
				ListEnvironment: e.foldInside,
				TableEnvironment: e.foldInside,
				Book: e.foldInside,
				Part: e.foldInside,
				Chapter: e.foldInside,
				Section: e.foldInside,
				SubSection: e.foldInside,
				SubSubSection: e.foldInside,
				Paragraph: e.foldInside,
				SubParagraph: e.foldInside
			}), O.styleTags({
				CtrlSeq: O.tags.keyword,
				CtrlSym: O.tags.operator,
				Csname: O.tags.keyword,
				Dollar: O.tags.processingInstruction,
				MathSpecialChar: O.tags.operator,
				MathChar: O.tags.variableName,
				MathOpening: O.tags.bracket,
				MathClosing: O.tags.bracket,
				EnvName: O.tags.className,
				DocumentEnvName: O.tags.className,
				TabularEnvName: O.tags.className,
				EquationEnvName: O.tags.className,
				EquationArrayEnvName: O.tags.className,
				VerbatimEnvName: O.tags.className,
				TikzPictureEnvName: O.tags.className,
				FigureEnvName: O.tags.className,
				ListEnvName: O.tags.className,
				TableEnvName: O.tags.className,
				BookCtrlSeq: O.tags.heading,
				PartCtrlSeq: O.tags.heading,
				ChapterCtrlSeq: O.tags.heading,
				SectionCtrlSeq: O.tags.heading,
				SubSectionCtrlSeq: O.tags.heading,
				SubSubSectionCtrlSeq: O.tags.heading,
				ParagraphCtrlSeq: O.tags.heading,
				SubParagraphCtrlSeq: O.tags.heading,
				Comment: O.tags.comment,
				VerbContent: O.tags.meta,
				VerbatimContent: O.tags.meta,
				LstInlineContent: O.tags.meta,
				LiteralArgContent: O.tags.string,
				SpaceDelimitedLiteralArgContent: O.tags.string,
				OpenBrace: O.tags.bracket,
				CloseBrace: O.tags.bracket,
				OpenBracket: O.tags.bracket,
				CloseBracket: O.tags.bracket,
				Begin: O.tags.keyword,
				End: O.tags.keyword,
				TextBoldCtrlSeq: O.tags.strong,
				TextItalicCtrlSeq: O.tags.emphasis,
				TextSmallCapsCtrlSeq: O.tags.className,
				TextTeletypeCtrlSeq: O.tags.monospace,
				EmphasisCtrlSeq: O.tags.emphasis,
				UnderlineCtrlSeq: O.tags.emphasis,
				TitleCtrlSeq: O.tags.heading,
				AuthorCtrlSeq: O.tags.heading,
				DateCtrlSeq: O.tags.heading,
				Number: O.tags.number,
				Normal: O.tags.content,
				Ampersand: O.tags.operator,
				Tilde: O.tags.operator,
				TrailingContent: O.tags.invalid,
				DocumentClassCtrlSeq: O.tags.definitionKeyword,
				UsePackageCtrlSeq: O.tags.keyword,
				LabelCtrlSeq: O.tags.labelName,
				RefCtrlSeq: O.tags.labelName,
				RefStarrableCtrlSeq: O.tags.labelName,
				CiteCtrlSeq: O.tags.quote,
				CiteStarrableCtrlSeq: O.tags.quote,
				BibliographyCtrlSeq: O.tags.heading,
				BibliographyStyleCtrlSeq: O.tags.heading
			})]
		}),
		languageData: {
			commentTokens: {
...
function me(t) {
	...
	for(let O = t.firstChild; O; O = O.nextSibling)
		...
		for(let t = e.firstChild; t; t = t.nextSibling)
			if("EnvName" === t.name || "DocumentEnvName" === t.name || "TabularEnvName" === t.name || "EquationEnvName" === t.name || "EquationArrayEnvName" === t.name || "VerbatimEnvName" === t.name || "TikzPictureEnvName" === t.name || "FigureEnvName" === t.name || "ListEnvName" === t.name || "TableEnvName" === t.name) return t.name;
	return null
}
Object.defineProperty(exports, "autocompletion", {
	...
	enumerable: !0,
	get: function() {
		return n.completionKeymap
	}
}), exports.autoCloseTags = ae, exports.findEnvironmentName = me, exports.findMatchingEnvironment = function(t, e, O) {
	var i, a;
	const n = O.resolve(e);
	if("Begin" === n.name || "BeginEnv" === (null === (i = n.parent) || void 0 === i ? void 0 : i.name)) {
		const t = "Begin" === n.name ? n : n.parent;
		if(!t) return null;
		if(!me(t)) return null;
		let e = t.parent;
		for(; e && !e.name.endsWith("Environment") && "Environment" !== e.name;) e = e.parent;
		if(!e) return null;
		let O = null;
		for(let t = e.firstChild; t; t = t.nextSibling)
			if("EndEnv" === t.name) {
	...
	if("End" === n.name || "EndEnv" === (null === (a = n.parent) || void 0 === a ? void 0 : a.name)) {
		const t = "End" === n.name ? n : n.parent;
		if(!t) return null;
		if(!me(t)) return null;
		let e = t.parent;
		for(; e && !e.name.endsWith("Environment") && "Environment" !== e.name;) e = e.parent;
	...
	return null
}, exports.findSectionBoundaries = function(t) {
	if(!t) return null;
	if("Book" === t.name || "Part" === t.name || "Chapter" === t.name || "Section" === t.name || "SubSection" === t.name || "SubSubSection" === t.name || "Paragraph" === t.name || "SubParagraph" === t.name) {
		return t.firstChild ? {
			from: t.from,
			to: t.to
		} : null
	...
	return null
}, exports.getIndentationLevel = function(t) {
	if(!t) return 0;
	if("Environment" === t.name || t.name.endsWith("Environment")) return 1;
	if("Group" === t.name) return 1;
	let e = t.parent,
		O = 0;
	for(; e;)("Environment" === e.name || e.name.endsWith("Environment") || "Group" === e.name) && O++, e = e.parent;
	return O
}, exports.latex = function(t = {}) {
	var O, a, s, r, o;
	const l = {
			...t,
			autoCloseTags: null === (O = t.autoCloseTags) || void 0 === O || O,
			enableLinting: null === (a = t.enableLinting) || void 0 === a || a,
			enableTooltips: null === (s = t.enableTooltips) || void 0 === s || s,
			enableAutocomplete: null === (r = t.enableAutocomplete) || void 0 === r || r,
			autoCloseBrackets: null === (o = t.autoCloseBrackets) || void 0 === o || o
		},
		d = [];
	return d.push(he.data.of({
		autocomplete: ee(l.autoCloseTags)
	})), d.push(e.foldService.of(Qe)), l.enableAutocomplete && (d.push(n.autocompletion({
		override: [ee(l.autoCloseTags)],
		defaultKeymap: !0,
		activateOnTyping: !0,
		icons: !0
	})), d.push(i.keymap.of(n.completionKeymap))), d.push(ce), l.autoCloseBrackets && d.push(n.closeBrackets()), l.autoCloseTags && d.push(...ae), l.enableLinting && d.push(function(t, e = {}) {
		return [L.of({
			source: t,
			config: e
		}), W, Ot]
	}(ne())), l.enableTooltips && d.push(le), new e.LanguageSupport(he, d)
}, exports.latexBracketMatching = ce, exports.latexCompletionSource = ee, exports.latexCompletions = ue, exports.latexExtensions = fe, exports.latexHoverTooltip = le, exports.latexLanguage = he, exports.latexLinter = ne, exports.parser = It, exports.snippets = te;
