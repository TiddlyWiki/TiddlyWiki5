...
GeomapWidget.prototype.renderMap = function () {
	...
	this.map.on("moveend zoomend", function (event) {
		if (self.hasAttribute("state")) {
			...
			if (
				!tiddler ||
				tiddler.fields.lat !== lat ||
				tiddler.fields.long !== long ||
				tiddler.fields.zoom !== zoom
			) {
...
GeomapWidget.prototype.refreshMap = function () {
	...
	var myIcon = new $tw.Leaflet.Icon({
		iconUrl: $tw.utils.makeDataUri(
			this.wiki.getTiddlerText("$:/plugins/tiddlywiki/geospatial/images/markers/pin"),
			"image/svg+xml"
		),
	...
	this.layerControl = $tw.Leaflet.control
...
GeomapWidget.prototype.setMapView = function () {
	...
	if (stateTiddler) {
		this.map.setView(
			[$tw.utils.parseNumber(stateTiddler.fields.lat, 0), $tw.utils.parseNumber(stateTiddler.fields.long, 0)],
			$tw.utils.parseNumber(stateTiddler.fields.zoom, 0)
		);
		return true;
	}
	return false;
...
GeomapWidget.prototype.refresh = function (changedTiddlers) {
	...
	if (result) {
		...
		if (
			...
			(this.hasAttribute("state") && changedTiddlers[this.getAttribute("state")]) ||
			changedAttributes.maxZoom
		) {
			this.setMapView();
		}
	}
