...
constructor(options) {
  options = options || {};
  /** @type {typeof import("./sql-engine").SqlEngine} */
...
async hasRecipePermission(userId, recipeName, permissionName) {
  ...
  `, {
    ...
    if (recipe && !!recipe.owner_id && recipe.owner_id === userId) {
      return true;
    } else {
      var permission = await this.checkACLPermission(userId, "recipe", recipeName, permissionName, recipe && recipe.owner_id);
      return permission;
    }
...
`;
    ...
    const result = await this.engine.runStatementGet(checkPermissionQuery, {
      ...
      $entity_type: entityType,
      $entity_name: entityName,
      $permission_id: aclRecord && aclRecord.permission_id
    });
