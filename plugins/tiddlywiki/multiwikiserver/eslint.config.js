  ...
  create(context) {
    ...
    return {
      ":expression"(node) {
        ...
        if (checks.has(node.parent.type)) return;
        const tsNode = services.esTreeNodeToTSNodeMap.get(node);
        if (isSometimesThenable(checker, tsNode)) {
          context.report({ node: node, messageId: 'expression' });
        }
      },
    };
    function isSometimesThenable(checker, tsNode) {
      const type = checker.getTypeAtLocation(tsNode);
      for (const subType of tsutils.unionTypeParts(checker.getApparentType(type))) {
        if (tsutils.isThenableType(checker, tsNode, subType)) {
          return true;
        }
      }
      return false;
