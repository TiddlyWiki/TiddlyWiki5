...
InnerWikiWidget.prototype.render = function (parent, nextSibling) {
	...
	if (!this.document.isTiddlyWikiFakeDom) {
		...
		this.domIFrame.style.transform =
			"translate(" + -this.clipLeft + "px," + -this.clipTop + "px) scale(" + this.scale + ")";
		...
		var anchorWidth = 40,
			...
			domAnchor = dm("img", {
				...
				attributes: {
					...
					src:
						"data:image/svg+xml," +
						encodeURIComponent(self.wiki.getTiddlerText("$:/plugins/tiddlywiki/innerwiki/crosshairs.svg"))
				}
			});
