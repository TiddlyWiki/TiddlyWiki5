...
const r = require("$:/plugins/tiddlywiki/markdown-legacy/remarkable.js");
...
let md = new Remarkable(remarkableOpts);
...
if($tw.modules.titles["$:/plugins/tiddlywiki/katex/katex.min.js"]) {
	const rk = require("$:/plugins/tiddlywiki/markdown-legacy/remarkable-katex.js");
	md = md.use(rk);
}
...
function findTagWithType(nodes,startPoint,type,level) {
	for(let i = startPoint;i < nodes.length;i++) {
...
function convertNodes(remarkableTree,isStartOfInline) {
	...
	function withChildren(currentIndex,currentLevel,closingType,nodes,callback) {
		const j = findTagWithType(nodes,currentIndex + 1,closingType,currentLevel);
		if(j === false) {
			console.error(`Failed to find a ${closingType} node after position ${currentIndex}`);
			console.log(nodes);
			return currentIndex + 1;
		}
		const children = convertNodes(nodes.slice(currentIndex + 1,j));
		callback(children);
		return j;
	}
	function wrappedElement(elementTag,currentIndex,currentLevel,closingType,nodes) {
		return withChildren(currentIndex,currentLevel,closingType,nodes,(children) => {
			out.push({
				type: "element",
				tag: elementTag,
				children
	...
	for(let i = 0;i < remarkableTree.length;i++) {
		var currentNode = remarkableTree[i];
		switch(currentNode.type) {
			case "paragraph_open": {
				// If the paragraph is a "tight" layout paragraph, don't wrap children in a <p> tag.
				if(currentNode.tight) {
					i = withChildren(i,currentNode.level,"paragraph_close",remarkableTree,(children) => {
			...
			case "link_open": {
				i = withChildren(i,currentNode.level,"link_close",remarkableTree,(children) => {
			...
			case "td_open": {
				...
				i = withChildren(i,currentNode.level,`${elementTag}_close`,remarkableTree,(children) => {
					const attributes = {};
			...
			case "text": {
				...
				accumulatedText += currentNode.content;
				break;
			}
			...
			case "katex": {
				...
				if(pluginOpts.renderWikiText) {
					...
					const displayModeSuffix = currentNode.block ? "\n" : "";
			...
			default: {
				if(currentNode.type.substr(currentNode.type.length - 5) === "_open") {
					const tagName = currentNode.type.substr(0,currentNode.type.length - 5);
		...
		if(accumulatedText
			...
			if(!pluginOpts.renderWikiText || accumulatedText.match(/^\s*$/)) {
				...
				const parseAsInline = !(isStartOfInline && i === 0);
				let textToParse = accumulatedText;
				if(pluginOpts.renderWikiTextPragma !== "") {
					textToParse = `${pluginOpts.renderWikiTextPragma}\n${textToParse}`;
				}
				const wikiParser = $tw.wiki.parseText("text/vnd.tiddlywiki",textToParse,{
					parseAsInline
				});
				let rs = wikiParser.tree;
