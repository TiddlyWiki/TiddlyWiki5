...
CecilyStoryView.prototype.insert = function (widget) {
	...
	$tw.utils.setStyle(domNode, [{position: "absolute"}, {transition: ""}, {opacity: "0.0"}]);
	// Position it
	var title = widget.parseTreeNode.itemTitle;
	this.positionTiddler(title, domNode);
	$tw.utils.forceLayout(domNode);
	// Animate it in
	$tw.utils.setStyle(domNode, [{transition: "opacity " + duration + "ms ease-out"}, {opacity: "1.0"}]);
};
...
CecilyStoryView.prototype.remove = function (widget) {
	...
	// Animate the closure
	$tw.utils.setStyle(targetElement, [{transition: "none"}, {opacity: "1.0"}]);
