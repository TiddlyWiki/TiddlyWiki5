		...
		$tw.utils.each(matches, function (match, index) {
			...
			if (!merged) {
				container.appendChild(
					$tw.utils.domMaker("span", {
						text:
							(match.startPos < contextLength ? "" : ellipsis) +
			...
			if (merged) {
				...
				container.appendChild(
					$tw.utils.domMaker("span", {
						text:
							textMap.string.slice(match.endPos, match.endPos + contextLength) +
...
DynannotateWidget.prototype.refresh = function (changedTiddlers) {
	...
	if (
		changedAttributes.selection ||
		changedAttributes.selectionPrefix ||
		changedAttributes.selectionSuffix ||
	...
	var wrapperWidth = this.domWrapper.offsetWidth,
		hasResized =
			wrapperWidth !== this.wrapperWidth || changedTiddlers["$:/state/DynaView/ViewportDimensions/ResizeCount"],
