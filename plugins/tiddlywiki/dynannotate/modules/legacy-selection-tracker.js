		...
		if (selection && (selection.type === "Range" || (self.allowBlankSelectionPopup && !self.selectionPopupTitle))) {
			...
			if (!!anchorContainer || !!focusContainer) {
				...
				if (
					...
					self.selectionPrefixSaveTitle = anchorContainer.getAttribute(
						"data-annotation-selection-prefix-save"
					);
					self.selectionSuffixSaveTitle = anchorContainer.getAttribute(
						"data-annotation-selection-suffix-save"
					);
					...
					if (self.selectionSaveTitle) {
						...
						var context = textMap.extractContext(
						...
						if (context) {
							if (self.selectionPrefixSaveTitle) {
								self.wiki.addTiddler(
									new $tw.Tiddler({title: self.selectionPrefixSaveTitle, text: context.prefix})
								);
							}
							if (self.selectionSuffixSaveTitle) {
								self.wiki.addTiddler(
									new $tw.Tiddler({title: self.selectionSuffixSaveTitle, text: context.suffix})
								);
							}
						}
					}
