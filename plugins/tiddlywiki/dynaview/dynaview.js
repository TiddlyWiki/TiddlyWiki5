...
exports.startup = function () {
	...
	$tw.hooks.addHook("th-page-refreshed", function () {
		if (lastScrollY === window.scrollY) {
			// Don't do scroll anchoring if the scroll position got changed
...
function saveScrollPosition() {
	if (!localStorageHasFailed) {
		if (
			...
			$tw.wiki.getTiddlerText("$:/config/DynaView/RestoreScrollPositionAtStartup") === "yes"
		) {
			var top = findTopmostTiddler();
			if (top.element) {
				try {
...
function saveViewportDimensions() {
	...
	if (document.querySelector(".tc-dynaview-request-refresh-on-resize")) {
		if (previousViewportWidth !== viewportWidth || previousViewportHeight !== viewportHeight) {
			var count =
				parseInt($tw.wiki.getTiddlerText("$:/state/DynaView/ViewportDimensions/ResizeCount", "0"), 10) || 0;
			$tw.wiki.addTiddler(
