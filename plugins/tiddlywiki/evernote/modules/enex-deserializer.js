...
exports["application/enex+xml"] = function(text,fields) {
	...
	const enex = doc.querySelector("en-export");
	if(!enex) {
	...
	$tw.utils.each(noteNodes,(noteNode) => {
		const noteTitle = getTextContent(noteNode,"title");
		// get real note content node
		let contentNode = noteNode.querySelector("content");
		const contentText = (contentNode.textContent || "").replace(/&nbsp;/g,' ').trim();
		if(contentText) {
		...
		$tw.utils.each(noteNode.querySelectorAll("resource"),(resourceNode) => {
			// hash generated by applying https://github.com/vzhd1701/evernote-backup/pull/54
			const hash = resourceNode.querySelector("data").getAttribute("hash");
			const text = getTextContent(resourceNode,"data");
			const mimeType = getTextContent(resourceNode,"mime");
			const contentTypeInfo = $tw.config.contentTypeInfo[mimeType] || {extension: ""};
			let title = getTextContent(resourceNode,"resource-attributes>file-name");
		...
		$tw.utils.each(noteNode.querySelectorAll("tag"),(tagNode) => {
			noteResult.tags.push(tagNode.textContent);
		});
		// If there's an update date, set modifiy date accordingly
		const update = getTextContent(noteNode,"updated");
		if(update) {
			noteResult.modified = convertDate(update);
		}
		$tw.utils.each(noteNode.querySelectorAll("note-attributes>*"),(attrNode) => {
...
function fixAttachmentReference(contentNode,md5Hash,mimeType,name) {
	...
	const mediaNode = contentNode.querySelector(`en-media[hash="${md5Hash}"]`);
	if(!name) {
