...
exports.startup = function() {
	...
	$tw.rootWidget.addEventListener("tm-confetti-launch",(event) => {
		const paramObject = event.paramObject || {};
		const options = {};
		const extractNumericParameter = function(name) {
			if(paramObject[name]) {
				options[name] = $tw.utils.parseNumber(paramObject[name]);
			}
		};
		const extractListParameter = function(name) {
			if(paramObject[name]) {
				options[name] = $tw.utils.parseStringArray(paramObject[name]);
			}
		};
		const extractBooleanParameter = function(name) {
			if(paramObject[name]) {
				options[name] = paramObject[name] === "yes";
			}
		};
		$tw.utils.each("particleCount angle spread startVelocity decay gravity drift ticks scalar zIndex".split(" "),(name) => {
			extractNumericParameter(name);
		});
		$tw.utils.each("colors shapes".split(" "),(name) => {
		...
		const delay = paramObject.delay ? $tw.utils.parseNumber(paramObject.delay) : 0;
		$tw.confettiManager.launch(delay,options);
	});
	$tw.rootWidget.addEventListener("tm-confetti-reset",(event) => {
