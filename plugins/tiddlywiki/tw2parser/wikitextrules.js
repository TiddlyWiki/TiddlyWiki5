...
var rules = [
	{
		...
		handler: function (w) {
			var table = {
				type: "element",
				tag: "table",
				attributes: {class: {type: "string", value: "table"}},
				children: []
			};
		...
		rowHandler: function (w, e, prevColumns) {
			...
			while (cellMatch && cellMatch.index == w.nextMatch) {
				if (cellMatch[1] == "~") {
					...
					else if (spaceLeft) $tw.utils.addAttributeToParseTreeNode(cell, "align", "right");
					w.nextMatch--;
				}
				col++;
				this.cellRegExp.lastIndex = w.nextMatch;
		...
		handler: function (w) {
			...
			while (lookaheadMatch && lookaheadMatch.index == w.nextMatch) {
				...
				if (listLevel > currLevel) {
					...
					for (t = currLevel; t > listLevel; t--) stack.pop();
		...
		handler: function (w) {
			...
			do {
				if (newLevel > currLevel) {
					...
					for (t = currLevel; t > newLevel; t--) stack.pop();
				}
				currLevel = newLevel;
...

	{
		...
		handler: function (w) {
			this.lookaheadRegExp.lastIndex = w.matchStart;
			var lookaheadMatch = this.lookaheadRegExp.exec(w.source);
			if (lookaheadMatch && lookaheadMatch.index == w.matchStart) w.nextMatch = this.lookaheadRegExp.lastIndex;
		}
	},
		...
		handler: function (w) {
			...
			switch (w.matchText) {
				...
				case "__":
					...
					w.output.push(e);
					w.subWikifyTerm(e.children, /(__)/gm);
					break;
				case "^^":
		...
		handler: function (w) {
			switch (w.matchText) {
				case "@@":
					...
					w.output.push(e);
					var styles = inlineCssHelper(w);
					if (styles.length === 0) setAttr(e, "class", "marked");
					else applyCssHelper(e, styles);
					w.subWikifyTerm(e.children, /(@@)/gm);
					break;
				case "{{":
