...
describe("Utility tests", function () {
	...
	it("should handle base64 encoding emojis", function () {
		...
		expect($tw.utils.base64Encode(booksEmoji)).not.toBe(
			"7aC97bOa",
			"if base64 is 7aC97bOa then surrogate pairs were incorrectly treated as codepoints"
		);
		expect($tw.utils.base64Encode(booksEmoji)).toBe(
			"8J+Tmg==",
			"if surrogate pairs are correctly treated as a single code unit then base64 should be 8J+Tmg=="
		);
		expect($tw.utils.base64Decode("8J+Tmg==")).toBe(booksEmoji);
		expect($tw.utils.base64Decode($tw.utils.base64Encode(booksEmoji))).toBe(
			booksEmoji,
	...
	it("should handle base64 encoding emojis in URL-safe variant", function () {
		var booksEmoji = "📚";
		expect($tw.utils.base64Encode(booksEmoji, false, true)).toBe(
			"8J-Tmg==",
			"if surrogate pairs are correctly treated as a single code unit then base64 should be 8J+Tmg=="
		);
		expect($tw.utils.base64Decode("8J-Tmg==", false, true)).toBe(booksEmoji);
		expect($tw.utils.base64Decode($tw.utils.base64Encode(booksEmoji, false, true), false, true)).toBe(
			booksEmoji,
			"should round-trip correctly"
		);
	});
	...
	it("should parse text references", function () {
		...
		expect(ptr("title")).toEqual({title: "title"});
		expect(ptr("ti#tle")).toEqual({title: "ti#tle"});
		expect(ptr("ti!tle")).toEqual({title: "ti!tle"});
		expect(ptr("ti#tle##index")).toEqual({title: "ti#tle", index: "index"});
		expect(ptr("ti!tle!!field")).toEqual({title: "ti!tle", field: "field"});
		expect(ptr("title##index!!field")).toEqual({title: "title##index", field: "field"});
		expect(ptr("title!!field##index")).toEqual({title: "title", field: "field##index"});
	});
