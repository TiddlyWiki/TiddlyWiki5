...
describe("'reduce' and 'intersection' filter prefix tests", function () {
	...
	it("should handle the :cascade filter prefix", function () {
		expect(
			wiki
				.filterTiddlers("[[Rice Pudding]] :cascade[all[shadows+tiddlers]tag[$:/tags/Filter]get[text]]")
				.join(",")
		).toBe("It is not customary");
		expect(
			wiki
				.filterTiddlers("[[chocolate cake]] :cascade[all[shadows+tiddlers]tag[$:/tags/Filter]get[text]]")
				.join(",")
		).toBe("It is customary");
		expect(
			wiki
				.filterTiddlers("[[Sparkling water]] :cascade[all[shadows+tiddlers]tag[$:/tags/Filter]get[text]]")
				.join(",")
		).toBe("Just a default");
		expect(
			wiki
				.filterTiddlers("[[Rice Pudding]] :cascade[all[shadows+tiddlers]tag[$:/tags/SecondFilter]get[text]]")
				.join(",")
		).toBe("It is not customary");
		expect(
			wiki
				.filterTiddlers("[[chocolate cake]] :cascade[all[shadows+tiddlers]tag[$:/tags/SecondFilter]get[text]]")
				.join(",")
		).toBe("It is customary");
		expect(
			wiki
				.filterTiddlers("[[Sparkling water]] :cascade[all[shadows+tiddlers]tag[$:/tags/SecondFilter]get[text]]")
				.join(",")
		).toBe("");
	});
	...
	it("should handle the :reduce filter prefix", function () {
		...
		expect(
			wiki.filterTiddlers("[tag[shopping]] :reduce[get[price]multiply{!!quantity}add<accumulator>]").join(",")
		).toBe("27.75");
		expect(
			wiki
				.filterTiddlers(
		...
		expect(
			wiki.filterTiddlers("[tag[non-existent]] :reduce[get[price]multiply{!!quantity}add<accumulator>]").length
		).toBe(0);
		expect(
			wiki
				.filterTiddlers(
		...
		expect(
			wiki
				.filterTiddlers(
					"[tag[non-existent]] :reduce:11,22[get[price]multiply{!!quantity}add<accumulator>] :else[[0]]"
				)
				.join(",")
		).toBe("0");
		...
		expect(
			wiki
				.filterTiddlers(
					"[tag[non-existent]] :reduce:11[get[price]multiply{!!quantity}add<accumulator>] :else[[0]]"
				)
				.join(",")
		).toBe("0");
	});
	...
	it("should handle the standard-deviation operator", function () {
		expect(parseFloat(wiki.filterTiddlers("[tag[shopping]get[price]standard-deviation[]]").join(","))).toBeCloseTo(
			1.71
		);
		expect(parseFloat(wiki.filterTiddlers("[tag[food]get[price]standard-deviation[]]").join(","))).toBeCloseTo(
			1.835
		);
	...
	it("should handle the :filter prefix and filter operator", function () {
		...
		expect(
			wiki
				.filterTiddlers("[tag[textexample]] :filter[get[text]length[]compare:integer:gteq[18]]", anchorWidget)
				.join(",")
		...
		expect(
			wiki.filterTiddlers("[tag[textexample]filter<larger-than-18-macro-param>]", anchorWidget).join(",")
		).toBe("Cheesecake,Chocolate Cake,Red wine");
	});
	...
	it("should handle the :sort prefix", function () {
		...
		expect(
			wiki.filterTiddlers("a1 a10 a2 a3 b10 b3 b1 c9 c11 c1 :sort:alphanumeric:reverse[{!!title}]").join(",")
		).toBe("c11,c9,c1,b10,b3,b1,a10,a3,a2,a1");
	...
	it("should handle the :map prefix", function () {
		...
		expect(wiki.filterTiddlers("[tag[shopping]] :map[tags[]]").join(",")).toBe(
		...
		expect(
			wiki
				.filterTiddlers(
					"[tag[shopping]] :map[get[title]addprefix[-]addprefix<length>addprefix[of]addprefix<index>]"
				)
				.join(",")
	...
	it("should handle the :then prefix", function () {
		...
		expect(wiki.filterTiddlers("[[one]] :then[tag[shopping]]").join(",")).toBe(
			"Brownies,Chick Peas,Milk,Rice Pudding"
		);
	...
	it("should handle macro parameters for filter run prefixes", function () {
		...
		expect(
			wiki.filterTiddlers('[tag[shopping]] :map[<echo "$(index)$ $(currentTiddler)$">]', anchorWidget).join(",")
		...
		expect(
			wiki
				.filterTiddlers(
					'[tag[shopping]] :reduce[<echo "$(accumulator)$ $(index)$ $(currentTiddler)$,">]',
					anchorWidget
				)
				.join(",")
		).toBe(" 0 Brownies, 1 Chick Peas, 2 Milk, 3 Rice Pudding,");
