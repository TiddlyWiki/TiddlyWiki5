...
describe("Checkbox widget", function () {
	...
	var fieldModeTests = [
		...
		},
		{
			...
			widgetText:
				"<$checkbox tiddler='TiddlerOne' field='expand' indeterminate='yes' checked='yes' unchecked='no' />",
		...
		// true -> indeterminate cannot happen in field mode
		{
			...
			tiddlers: [{title: "TiddlerOne", text: "Jolly Old World", expand: "some other value"}],
			widgetText:
				"<$checkbox tiddler='TiddlerOne' field='expand' indeterminate='' checked='yes' unchecked='no' />",
			startsOutChecked: false,
			expectedChange: {TiddlerOne: {expand: "yes"}}
		}
	];
	...
	var listModeTests = [
		...
		},
		{
			...
			widgetText:
				"\\define checkActions() <$action-listops $tiddler='ExampleTiddler' $field='someField' $filter='yes'/>\n" +
		...
		{
			testName:
				"list mode neither checked nor unchecked specified, means field value is treated as empty=false, nonempty=true",
			tiddlers: [{title: "ExampleTiddler", someField: "yes"}],
			widgetText:
				"\\define checkActions() <$action-listops $tiddler='ExampleTiddler' $field='someField' $filter='yes -no'/>\n" +
		...
		},
		{
			...
			widgetText:
				"<$checkbox tiddler='Colors' listField='colors' indeterminate='yes' unchecked='red' checked='green' />",
			startsOutChecked: undefined,
	...
	var listModeTestsWithListField = listModeTests
	...
	var listModeTestsWithTagsField = listModeTests
		.filter((data) => data.widgetText.includes("listField='colors'"))
		.map((data) => {
			var newData = {
				...data,
	...
	var indexListModeTests = listModeTests.map((data) => {
		...
		var newWidgetText = data.widgetText
			.replace("listField='colors'", "listIndex='colors'")
			.replace(/\$field/g, "$index")
			.replace("listField='someField'", "listIndex='someField'");
		var newChange = {};
		for (var key of Object.keys(data.expectedChange)) {
			var oldChange = data.expectedChange[key];
			if (oldChange.colors) {
	...
	var filterModeTests = [
		{
			...
			tiddlers: [{title: "Colors", colors: "red orange yellow"}],
			widgetText:
				"\\define checkActions() <$action-listops $tiddler='Colors' $field='colors' $subfilter='-red green'/>\n" +
		...
		},
		{
			...
			widgetText:
				"\\define checkActions() <$action-listops $tiddler='Colors' $field='colors' $subfilter='green'/>\n" +
