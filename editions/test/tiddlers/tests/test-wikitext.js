...
describe("WikiText tests", function() {
	...
	it("should render plain text tiddlers as a paragraph", function() {
		expect(wiki.renderTiddler("text/html","TiddlerOne")).toBe("<p dir=\"auto\">The quick brown fox</p>");
	});
	it("should preserve single new lines", function() {
		expect(wiki.renderTiddler("text/html","TiddlerTwo")).toBe("<p dir=\"auto\">The rain in Spain\nfalls mainly on the plain</p>");
	});
	it("should use double new lines to create paragraphs", function() {
		expect(wiki.renderTiddler("text/html","TiddlerThree")).toBe("<p dir=\"auto\">The speed of sound</p><p dir=\"auto\">The light of speed</p>");
	});
	it("should support attributes specified as macro invocations", function() {
		expect(wiki.renderTiddler("text/html","TiddlerFour")).toBe("<p dir=\"auto\"><a class=\"tc-tiddlylink tc-tiddlylink-missing\" href=\"#This%20is%20my%20%27%27amazingly%27%27%20groovy%20macro%21\">This is a link</a></p>");
	});
	...
	it("handles inline style wikitext notation", function() {
		expect(wiki.renderText("text/html","text/vnd-tiddlywiki",
			"some @@highlighted@@ text")).toBe('<p dir="auto">some <span class="tc-inline-style">highlighted</span> text</p>');
		expect(wiki.renderText("text/html","text/vnd-tiddlywiki",
			"some @@color:green;.tc-inline-style 1 style and 1 class@@ text")).toBe('<p dir="auto">some <span class=" tc-inline-style " style="color:green;">1 style and 1 class</span> text</p>');
		expect(wiki.renderText("text/html","text/vnd-tiddlywiki",
			"some @@background-color:red;red@@ text")).toBe('<p dir="auto">some <span style="background-color:red;">red</span> text</p>');
		expect(wiki.renderText("text/html","text/vnd-tiddlywiki",
			"some @@.myClass class@@ text")).toBe('<p dir="auto">some <span class=" myClass ">class</span> text</p>');
		expect(wiki.renderText("text/html","text/vnd-tiddlywiki",
			"some @@.myClass.secondClass 2 classes@@ text")).toBe('<p dir="auto">some <span class=" myClass secondClass ">2 classes</span> text</p>');
		expect(wiki.renderText("text/html","text/vnd-tiddlywiki",
			"some @@background:red;.myClass style and class@@ text")).toBe('<p dir="auto">some <span class=" myClass " style="background:red;">style and class</span> text</p>');
		expect(wiki.renderText("text/html","text/vnd-tiddlywiki",
			"some @@background:red;color:white;.myClass 2 style and 1 class@@ text")).toBe('<p dir="auto">some <span class=" myClass " style="background:red;color:white;">2 style and 1 class</span> text</p>');
	});
});
