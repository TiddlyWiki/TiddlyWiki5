...
describe("Widget module", function () {
	...
	it("should make sure that getVariableInfo returns all expected parameters", function () {
		...
		expect(childNode.getVariableInfo("macro", {allowSelfAssigned: true}).params).toEqual([
			{name: "a", value: "aa"}
		]);
		...
		expect(childNode.getVariableInfo("proc", {allowSelfAssigned: true}).params).toEqual([
			{name: "p", default: "pp"}
		]);
		expect(childNode.getVariableInfo("$my.widget", {allowSelfAssigned: true}).params).toEqual([
			{name: "w", default: "ww"}
		]);
