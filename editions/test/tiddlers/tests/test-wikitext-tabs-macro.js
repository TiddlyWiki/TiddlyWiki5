...
describe("Tabs-macro HTML tests", function () {
	...
	$tw.wiki.addTiddler(
		new $tw.Tiddler({title: "TabOne", text: "Text tab 1", caption: "t 1"}, $tw.wiki.getModificationFields())
	);
	$tw.wiki.addTiddler(
		new $tw.Tiddler({title: "TabTwo", text: "Text tab 2", caption: "t 2"}, $tw.wiki.getModificationFields())
	);
	// TabThree shows description used in button-template instead of caption
	$tw.wiki.addTiddler(
		new $tw.Tiddler(
			{title: "TabThree", text: "Text tab 3", caption: "t 3", description: "desc"},
	...
	$tw.wiki.addTiddler(
		new $tw.Tiddler(
			{title: "tabs-macro-definition", "code-body": "yes", text: coreTabsTiddler.fields.text},
			$tw.wiki.getModificationFields()
		)
	);
	...
	it("should render 'horizontal' tabs from v5.2.2 and up with whitespace trim", function () {
		expect($tw.wiki.renderTiddler("text/html", "test-tabs-horizontal")).toBe(
			expected.fields.text.replace(/\n/g, "")
		);
	});

	it("should render all 'horizontal' tabs from v5.2.2 and up with whitespace trim", function () {
		expect($tw.wiki.renderTiddler("text/html", "test-tabs-horizontal-all")).toBe(
	...
	it("should render 'vertical' tabs from v5.2.2 and up with whitespace trim", function () {
		expect($tw.wiki.renderTiddler("text/html", "test-tabs-vertical")).toBe(
