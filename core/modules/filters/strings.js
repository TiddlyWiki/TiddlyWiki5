...
exports.length = makeStringBinaryOperator(function (a) {
	return ["" + ("" + a).length];
});

exports.uppercase = makeStringBinaryOperator(function (a) {
	return [("" + a).toUpperCase()];
});

exports.lowercase = makeStringBinaryOperator(function (a) {
	return [("" + a).toLowerCase()];
});

exports.sentencecase = makeStringBinaryOperator(function (a) {
	return [$tw.utils.toSentenceCase(a)];
});

exports.titlecase = makeStringBinaryOperator(function (a) {
...
exports.split = makeStringBinaryOperator(function (a, b) {
	return ("" + a).split(b);
});

exports["enlist-input"] = makeStringBinaryOperator(function (a, o, s) {
	return $tw.utils.parseStringArray("" + a, s === "raw");
});

exports.join = makeStringReducingOperator(function (accumulator, value, operand) {
	if (accumulator === null) {
		return value;
	} else {
...
exports.levenshtein = makeStringBinaryOperator(function (a, b) {
...
exports.applypatches = makeStringBinaryOperator(function (a, b) {
	var dmpObject = new dmp.diff_match_patch(),
		patches;
	try {
...
exports["search-replace"] = function (source, operator, options) {
	var results = [],
		...
		flags =
			(flagSuffix.indexOf("g") !== -1 ? "g" : "") +
			(flagSuffix.indexOf("i") !== -1 ? "i" : "") +
	...
	source(function (tiddler, title) {
		if (title && operator.operands.length > 1) {
			results.push(title.replace(regExp, replacement));
			regExp.lastIndex = 0;
		} else {
			results.push(title);
		}
