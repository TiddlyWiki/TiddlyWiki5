...
class KeyboardManager {
	...
	*/
	parseKeyDescriptor(keyDescriptor, options) {
		var components = keyDescriptor.split(/\+|\-/), info = {
			keyCode: 0,
			shiftKey: false,
			altKey: false,
			ctrlKey: false,
		...
		for (var t = 0; t < components.length; t++) {
			var s = components[t].toLowerCase(), c = s.charCodeAt(0);
	...
	*/
		...
		$tw.utils.each(keyDescriptors, function (keyDescriptor) {
			...
			if (keyDescriptor.substr(0, 2) === "((" && keyDescriptor.substr(-2, 2) === "))") {
				if (options.stack.indexOf(keyDescriptor) === -1) {
					...
					var name = keyDescriptor.substring(2, keyDescriptor.length - 2), lookupName = function (configName) {
	...
	updateShortcutLists(tiddlerList) {
		...
		for (var i = 0; i < tiddlerList.length; i++) {
			var title = tiddlerList[i], tiddlerFields = $tw.wiki.getTiddler(title).fields;
	...
	*/
	...
	detectNewShortcuts(changedTiddlers) {
		var shortcutConfigTiddlers = [], handled = false;
