...
exports.init = function (parser) {
	this.parser = parser;
	// Regexp to match
	this.matchRegExp = /\<\%\s*if\s+/gm;
	this.terminateIfRegExp = /\%\>/gm;
};
...
exports.parse = function () {
	...
	var filterCondition = this.parser.source.substring(
		this.match.index + this.match[0].length,
		this.terminateIfMatch.index
	);
...
exports.parseIfClause = function (filterCondition) {
	...
	var hasLineBreak = !!$tw.utils.parseTokenRegExp(
		this.parser.source,
		this.parser.pos,
		/([^\S\n\r]*\r?\n(?:[^\S\n\r]*\r?\n|$))/g
	);
	...
	if (ex.match) {
		if (ex.match[1] === "endif") {
			...
			// Check for an immediately following double linebreak
			hasLineBreak = !!$tw.utils.parseTokenRegExp(
				this.parser.source,
				this.parser.pos,
				/([^\S\n\r]*\r?\n(?:[^\S\n\r]*\r?\n|$))/g
			);
