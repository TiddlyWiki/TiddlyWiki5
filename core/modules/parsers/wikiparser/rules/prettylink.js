...
exports.parse = function() {
	// Move past the match
	const start = this.parser.pos + 2;
	this.parser.pos = this.matchRegExp.lastIndex;
	// Process the link
	const text = this.match[1];
	const link = this.match[2] || text;
	let textEndPos = this.parser.source.indexOf("|",start);
	if(textEndPos < 0 || textEndPos > this.matchRegExp.lastIndex) {
		textEndPos = this.matchRegExp.lastIndex - 2;
	}
	const linkStart = this.match[2] ? (start + this.match[1].length + 1) : start;
	...
	if($tw.utils.isLinkExternal(link)) {
		return [{
			...
			children: [{
				type: "text",text,start,end: textEndPos
			}]
		}];
	} else {
		return [{
