	...
	while(true) {
		...
		for(var t=0; t<match[0].length; t++) {
			...
			if(quoteStack.length <= t) {
				...
				// Link this quote element into the last child item of the parent quote item
				if(t) {
		...
		// Process the body of the quote item into the last quote item
		...
		// Consume any whitespace following the quote item
		this.parser.skipWhitespace();
	}
	// Return the root element of the quote
	return [quoteStack[0]];
};
