...
exports.parse = function() {
	...
	const paramString = this.match[2];
	const params = [];
	if(paramString !== "") {
		...
		while(paramMatch) {
			...
			const defaultValue = paramMatch[2] || paramMatch[3] || paramMatch[4] || paramMatch[5] || paramMatch[6];
			if(defaultValue) {
	...
	if(this.match[3]) {
		...
		reEnd = new RegExp(String.raw`((?:^|\r?\n)[^\S\n\r]*\\end[^\S\n\r]*(?:` + $tw.utils.escapeRegExp(this.match[1]) + String.raw`)?\s*?(?:$|\r?\n))`,"mg");
	} else {
