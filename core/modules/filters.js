...
exports.parseFilter = function (filterString) {
	...
	var results = [], // Array of arrays of operator nodes {operator:,operand:}
		...
		match;
	var whitespaceRegExp = /(\s+)/gm,
		operandRegExp = /((?:\+|\-|~|=|\:(\w+)(?:\:([\w\:, ]*))?)?)(?:(\[)|(?:"([^"]*)")|(?:'([^']*)')|([^\s\[\]]+))/gm;
	while (p < filterString.length) {
		...
		if (p < filterString.length) {
			...
			if (match[5] || match[6] || match[7]) {
				// Double quoted string, single quoted string or unquoted title
				operation.operators.push({operator: "title", operands: [{text: match[5] || match[6] || match[7]}]});
			}
			...
			$tw.utils.each(operation.operators, function (operator) {
				...
				$tw.utils.each(operator.operands, function (operand) {
					if (operand.indirect) {
						...
						operand.value =
							widgetClass.evaluateVariable(widget, varTree.name, {
