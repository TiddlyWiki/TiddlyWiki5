...
TranscludeWidget.prototype.getTransclusionTarget = function () {
	...
	if (this.hasAttribute("$variable")) {
		if (this.transcludeVariable) {
			...
			var variableInfo = this.getVariableInfo(this.transcludeVariable, {
				params: this.getOrderedTransclusionParameters()
			});
			this.transcludeVariableIsFunction =
				variableInfo.srcVariable && variableInfo.srcVariable.isFunctionDefinition;
	...
	if (this.hasAttribute("$variable")) {
		if (this.transcludeVariable) {
			...
			if (srcVariable && srcVariable.isFunctionDefinition) {
				...
				this.transcludeFunctionResult =
					(variableInfo.resultList ? variableInfo.resultList[0] : variableInfo.text) || "";
			}
				...
				if (srcVariable && srcVariable.isFunctionDefinition) {
					...
					if (variableInfo.isCacheable && srcVariable[cacheKey]) {
						...
						parser = this.wiki.parseText(this.transcludeType, variableInfo.text || "", {
							...
							configTrimWhiteSpace: srcVariable && srcVariable.configTrimWhiteSpace
						});
		...
		// Transcluding a text reference
		parser = this.wiki.parseTextReference(this.transcludeTitle, this.transcludeField, this.transcludeIndex, {
			parseAsInline: parseAsInline,
			subTiddler: this.transcludeSubTiddler,
			defaultType: this.transcludeType
		});
...
TranscludeWidget.prototype.parserNeedsRefresh = function () {
	...
	return (
		...
		parserInfo._canonical_uri !== this._canonical_uri
	);
};
...
TranscludeWidget.prototype.refresh = function (changedTiddlers) {
	...
	if (
		...
		(this.transcludeVariableIsFunction && this.functionNeedsRefresh()) ||
