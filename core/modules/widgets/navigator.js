...
NavigatorWidget.prototype.saveStoryList = function (storyList) {
	if (this.storyTitle) {
		...
		this.wiki.addTiddler(new $tw.Tiddler({title: this.storyTitle}, storyTiddler, {list: storyList}));
	}
};
...
NavigatorWidget.prototype.handleDeleteTiddlerEvent = function (event) {
	...
	if (
		...
		!win.confirm($tw.language.getString("ConfirmDeleteTiddler", {variables: {title: confirmationTitle}}))
	) {
		return false;
	}
	...
	if (tiddler) {
		...
		if (draftTitle) {
			...
			if (isRename && this.wiki.tiddlerExists(draftTitle)) {
				isConfirmed = win.confirm(
					$tw.language.getString("ConfirmOverwriteTiddler", {variables: {title: draftTitle}})
				);
			}
			if (isConfirmed) {
...
NavigatorWidget.prototype.handleCancelTiddlerEvent = function (event) {
	...
	if (draftTiddler && originalTitle) {
		...
		if (this.wiki.isDraftModified(draftTitle)) {
			isConfirmed = win.confirm($tw.language.getString("ConfirmCancelTiddler", {variables: {title: draftTitle}}));
		}
		// Remove the draft tiddler
		if (isConfirmed) {
...
NavigatorWidget.prototype.handleImportTiddlersEvent = function (event) {
	...
	var autoOpenOnImport =
		event.autoOpenOnImport || paramObject.autoOpenOnImport || this.getVariable("tv-auto-open-on-import");
