...
class DropZoneWidget extends Widget {
	...
	*/
	...
	filterByContentTypes(tiddlerFieldsArray) {
		var filteredTypes, filtered = [], types = [];
	...
	handleDropEvent(event) {
		var self = this, readFileCallback = function (tiddlerFieldsArray) {
		...
		var self = this, dataTransfer = event.dataTransfer;
		// Remove highlighting
		this.resetState();
	...
	handlePasteEvent(event) {
		...
		if (["TEXTAREA", "INPUT"].indexOf(event.target.tagName) == -1 && !event.target.isContentEditable && !event.twEditor) {
			var self = this, items = event.clipboardData.items;
