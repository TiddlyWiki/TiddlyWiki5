			...
			for (var t = 0; t < dataTransfer.items.length; t++) {
				...
				if (item.kind === "string") {
					item.getAsString(function (str) {
						var tiddlerFields = self.wiki.deserializeTiddlers(
							null,
							str,
							{title: fallbackTitle},
...
DropZoneWidget.prototype.handlePasteEvent = function (event) {
	...
	var getItem = function (type) {
		...
		return function (str) {
			...
			if (self.dropzoneDeserializer) {
				tiddlerFields = self.wiki.deserializeTiddlers(
					null,
					str,
	...
	if (
		["TEXTAREA", "INPUT"].indexOf(event.target.tagName) == -1 &&
		...
		for (var t = 0; t < items.length; t++) {
			...
			if (item.kind === "file") {
				...
				item.kind === "string" &&
				!["text/html", "text/plain", "Text"].includes(item.type) &&
