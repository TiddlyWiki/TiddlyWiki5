...
const DEBOUNCE_INTERVAL = 100; // Delay after last scroll event before updating the bound tiddler

const Widget = require("$:/core/modules/widgets/widget.js").widget;

const ScrollableWidget = function(parseTreeNode,options) {
...
ScrollableWidget.prototype.scrollIntoView = function(element,callback,options) {
	...
	const scrollableBounds = this.outerDomNode.getBoundingClientRect();
	const clientTargetBounds = element.getBoundingClientRect();
	const bounds = {
	...
	const endX = getEndPos(bounds.left,bounds.width,scrollPosition.x,this.outerDomNode.offsetWidth);
	const endY = getEndPos(bounds.top,bounds.height,scrollPosition.y,this.outerDomNode.offsetHeight);
...
ScrollableWidget.prototype.listenerFunction = function(event) {
	...
	this.timeout = setTimeout(() => {
		const existingTiddler = self.wiki.getTiddler(self.scrollableBind);
		const newTiddlerFields = {
...
ScrollableWidget.prototype.updateScrollPositionFromBoundTiddler = function() {
	...
	const tiddler = this.wiki.getTiddler(this.scrollableBind);
	if(tiddler) {
		let scrollLeftTo = this.outerDomNode.scrollLeft;
		...
		let scrollTopTo = this.outerDomNode.scrollTop;
...
ScrollableWidget.prototype.refresh = function(changedTiddlers) {
	...
	// Refresh children
	const result = this.refreshChildren(changedTiddlers);
