		...
		if (parser) {
			...
			while (parseTreeNode && ["setvariable", "set", "parameters"].indexOf(parseTreeNode.type) !== -1) {
				...
				if (parseTreeNode.type === "set" || parseTreeNode.type === "setvariable") {
					if (
						parseTreeNode.isMacroDefinition ||
						parseTreeNode.isProcedureDefinition ||
						parseTreeNode.isWidgetDefinition ||
...
ImportVariablesWidget.prototype.refresh = function (changedTiddlers) {
	...
	if (
		...
		!$tw.utils.isArrayEqual(this.tiddlerList, tiddlerList) ||
		haveListedTiddlersChanged()
	) {
		// Compute the filter
