...
ListWidget.prototype.refresh = function (changedTiddlers) {
	...
	if (
		...
		changedAttributes.variable ||
		changedAttributes.counter ||
		changedAttributes.template ||
		changedAttributes.editTemplate ||
		changedAttributes.join ||
...
ListWidget.prototype.handleHistoryChanges = function () {
	...
	var entry = 0;
	while (
		entry < newHistory.length &&
		entry < this.history.length &&
		newHistory[entry].title === this.history[entry].title
	) {
		entry++;
	}
