...
const ListWidget = function(parseTreeNode,options) {
...
ListWidget.prototype.render = function(parent,nextSibling) {
	...
	let StoryView = this.storyViews[this.storyViewName];
...
ListWidget.prototype.execute = function(changedAttributes) {
	...
	if(this.list.length === 0) {
		...
		$tw.utils.each(this.list,(title,index) => {
...
ListWidget.prototype.findExplicitTemplates = function() {
	...
	const searchChildren = function(childNodes) {
		let foundInlineTemplate = false;
		$tw.utils.each(childNodes,(node) => {
...
ListWidget.prototype.getTiddlerList = function() {
	const limit = $tw.utils.getInt(this.getAttribute("limit",""),undefined);
	const defaultFilter = "[!is[system]sort[title]]";
	let results = this.wiki.filterTiddlers(this.getAttribute("filter",defaultFilter),this);
	if(limit !== undefined) {
		if(limit >= 0) {
...
ListWidget.prototype.getEmptyMessage = function() {
	let parser;
	const emptyMessage = this.getAttribute("emptyMessage");
...
ListWidget.prototype.makeItemTemplate = function(title,index) {
	...
	const isDraft = tiddler && tiddler.hasField("draft.of");
	let {template} = this;
	const {join} = this;
	let templateTree;
	...
	if(template) {
		...
		if(!templateTree || templateTree.length === 0) {
			...
			templateTree = [{
				type: "element",tag: this.parseTreeNode.isBlock ? "div" : "span",children: [{
					type: "link",attributes: {to: {type: "string",value: title}},children: [
	...
	const parseTreeNode = {type: "listitem",itemTitle: title,variableName: this.variableName,children: templateTree,join};
...
ListWidget.prototype.handleHistoryChanges = function() {
	...
	const newHistory = this.wiki.getTiddlerDataCached(this.historyTitle,[]);
...
ListWidget.prototype.handleListChanges = function(changedTiddlers) {
	...
	if(this.list.length === 0) {
		...
		let hasRefreshed = false; var t;
		if(this.counterName) {
			let mustRefreshOldLast = false;
			...
			let mustRecreateLastItem = false;
			...
			let isLast = false; let wasLast = false;
			for(t = 0;t < this.list.length;t++) {
				isLast = t === this.list.length - 1;
				const index = this.findListItem(t,this.list[t]);
...
ListWidget.prototype.insertListItem = function(index,title) {
	...
	const widget = this.makeChildWidget(this.makeItemTemplate(title,index));
	widget.parentDomNode = this.parentDomNode; // Hack to enable findNextSiblingDomNode() to work
	this.children.splice(index,0,widget);
	const nextSibling = widget.findNextSiblingDomNode();
...
ListWidget.prototype.removeListItem = function(index) {
	const widget = this.children[index];
...
const ListItemWidget = function(parseTreeNode,options) {
	this.initialise(parseTreeNode,options);
};
...
ListItemWidget.prototype.execute = function() {
	...
	// Add join if needed
	let {children} = this.parseTreeNode;
	const {join} = this.parseTreeNode;
	if(join && join.length && !this.parseTreeNode.isLast) {
		children = [...children];
		$tw.utils.each(join,(joinNode) => {
...
*/
const ListTemplateWidget = function(parseTreeNode,options) {
	// Main initialisation inherited from widget.js
	this.initialise(parseTreeNode,options);
};
...
const ListEmptyWidget = function(parseTreeNode,options) {
	// Main initialisation inherited from widget.js
	this.initialise(parseTreeNode,options);
};
...
const ListJoinWidget = function(parseTreeNode,options) {
	// Main initialisation inherited from widget.js
	this.initialise(parseTreeNode,options);
};
