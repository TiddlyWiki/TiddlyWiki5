...
const SetMultipleVariablesWidget = function(parseTreeNode,options) {
...
SetMultipleVariablesWidget.prototype.setVariables = function() {
	...
	if(filterNames && filterValues) {
		...
		$tw.utils.each(this.variableNames,(varname,index) => {
...
SetMultipleVariablesWidget.prototype.refresh = function(changedTiddlers) {
	const filterNames = this.getAttribute("$names","");
	const filterValues = this.getAttribute("$values","");
	const variableNames = this.wiki.filterTiddlers(filterNames,this);
	const variableValues = this.wiki.filterTiddlers(filterValues,this);
