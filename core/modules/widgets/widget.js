			...
			if (module.baseClass) {
				...
				if (!baseClass) {
					throw (
						"Module '" +
						title +
						"' is attemping to extend a non-existent base class '" +
						module.baseClass +
						"'"
					);
				}
...
Widget.prototype.makeChildWidget = function (parseTreeNode, options) {
	...
	if (this.variables[variableDefinitionName]) {
		var isOverrideable = function () {
			...
			return (
				parseTreeNode.type.indexOf(".") !== -1 || (!!self.widgetClasses[parseTreeNode.type] && !$tw.safeMode)
			);
		};
		if (!parseTreeNode.isNotRemappable && isOverrideable()) {
			...
			if (
				...
				variableInfo.srcVariable.value &&
