...
class DataWidget extends Widget {
	...
	*/
	computeDataTiddlerValues() {
		...
		var tiddlers = [], compoundTiddlers, title;
	...
	*/
	extractCompoundTiddler(title) {
		var tiddler = this.wiki.getTiddler(title);
		if (tiddler && tiddler.fields.type === "text/vnd.tiddlywiki-multiple") {
			var text = tiddler.fields.text || "", rawTiddlers = text.split(/\r?\n\+\r?\n/), tiddlers = [];
			$tw.utils.each(rawTiddlers, function (rawTiddler) {
				var fields = Object.create(null), split = rawTiddler.split(/\r?\n\r?\n/mg);
				if (split.length >= 1) {
