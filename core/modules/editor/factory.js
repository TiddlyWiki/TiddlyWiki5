...
function editTextWidgetFactory(toolbarEngine,nonToolbarEngine) {
	...
	class EditTextWidget extends Widget {
		...
		render(parent, nextSibling) {
			...
			// Create our element
			var editInfo = this.getEditInfo(), Engine = this.editShowToolbar ? toolbarEngine : nonToolbarEngine;
		...
		getEditInfo() {
			// Get the edit value
			var self = this, value, type = "text/plain", update;
			if (this.editIndex) {
				...
				update = function (value) {
					var tiddler = self.wiki.getTiddler(self.editTitle), updateFields = {
		...
		/*
			...
			*/
		handleEditTextOperationMessage(event) {
		...
		handleKeydownEvent(event) {
			...
			if (this.toolbarNode) {
				...
				for (var index = 0; index < shortcutElements.length; index++) {
					var el = shortcutElements[index], shortcutData = el.getAttribute("data-tw-keyboard-shortcut"), keyInfoArray = $tw.keyboardManager.parseKeyDescriptors(shortcutData, {
						wiki: this.wiki
					});
		...
		/*
			...
			*/
		propogateKeydownEvent(event) {
			var newEvent = this.cloneEvent(event, ["keyCode", "code", "which", "key", "metaKey", "ctrlKey", "altKey", "shiftKey"]);
			return !this.parentDomNode.dispatchEvent(newEvent);
		}
		cloneEvent(event, propertiesToCopy) {
			var propertiesToCopy = propertiesToCopy || [], newEvent = this.document.createEventObject ? this.document.createEventObject() : this.document.createEvent("Events");
