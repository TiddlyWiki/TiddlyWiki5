...
exports.getAllPlugins = function (options) {
	...
	$tw.utils.each(
		$tw.getLibraryItemSearchPaths(
			...
			options.ignoreEnvironmentVariables ? undefined : $tw.config.pluginsEnvVar
		),
	...
	$tw.utils.each(
		$tw.getLibraryItemSearchPaths(
			...
			options.ignoreEnvironmentVariables ? undefined : $tw.config.themesEnvVar
		),
		collectPublisherPlugins
	);
	$tw.utils.each(
		$tw.getLibraryItemSearchPaths(
			...
			options.ignoreEnvironmentVariables ? undefined : $tw.config.languagesEnvVar
		),
		collectPlugins
	);
	return tiddlers;
