...
(function (name, definition, context, dependencies) {
	if (typeof context["module"] !== "undefined" && context["module"]["exports"]) {
		...
		typeof context["define"] !== "undefined" &&
				...
				while (i < string.length) {
					...
					if (charCode < 128) {
						...
						output += String.fromCharCode(
							((charCode & 15) << 12) |
								((string.charCodeAt(i + 1) & 63) << 6) |
								(string.charCodeAt(i + 2) & 63)
						);
						i += 3;
					} else {
						var codePoint =
							((charCode & 7) << 18) |
							((string.charCodeAt(i + 1) & 63) << 12) |
							((string.charCodeAt(i + 2) & 63) << 6) |
							(string.charCodeAt(i + 3) & 63);
						// output += String.fromCodePoint(codePoint);  // Can't do this because Internet Explorer doesn't have String.fromCodePoint
						output +=
							String.fromCharCode(((codePoint - 65536) >> 10) + 55296) +
							String.fromCharCode(((codePoint - 65536) & 1023) + 56320); // So we do this instead
						i += 4;
					}
				}
