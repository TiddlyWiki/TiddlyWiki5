...
exports.generateTiddlerFilepath = function(title,options) {
	...
	if(encode) {
		writePath = path.resolve(directory,encodeURIComponent(writePath));
	}
...
exports.saveTiddlerToFile = function(tiddler,fileInfo,callback) {
	var err = $tw.utils.createDirectory(path.dirname(fileInfo.filepath));
	if(err) {
		return callback(err);
	}
	if(fileInfo.hasMetaFile) {
		...
		fs.writeFile(
			fileInfo.filepath,
			fileInfo.type === "application/x-tiddler"? 
				tiddler.getFieldStringBlock({exclude: ["text","bag"]}) + (!!tiddler.fields.text ? "\n\n" + tiddler.fields.text : ""): 
...
exports.deleteTiddlerFile = function(fileInfo,callback) {
	...
	if(!fileInfo || !fileInfo.filepath || !fs.existsSync(fileInfo.filepath)) {
		...
		var err = new Error("Invalid filepath `" + (fileInfo && fileInfo.filepath ? fileInfo.filepath : "undefined") + "`");
		err.code = "EACCES";
		err.syscall = "unlink";
		return callback(err);
	}
