...
exports.generateTiddlerFileInfo = function (tiddler, options) {
	...
	$tw.utils.each(tiddler.getFieldStrings(), function (value, fieldName) {
		if (fieldName !== "text") {
			hasUnsafeFields = hasUnsafeFields || /[\x00-\x1F]/gm.test(value);
			hasUnsafeFields = hasUnsafeFields || $tw.utils.trim(value) !== value;
		}
		hasUnsafeFields = hasUnsafeFields || /:|#/gm.test(fieldName);
	});
	...
	if (hasUnsafeFields) {
		...
		if (
			...
			tiddlerType === "text/vnd.tiddlywiki-multiple" ||
			tiddler.hasField("_canonical_uri")
		) {
			// Save as a .tid file
