...
exports.formatDateString = function (date, template) {
	...
	if (t.indexOf("[UTC]") == 0) {
		if (t == "[UTC]YYYY0MM0DD0hh0mm0ssXXX") return $tw.utils.stringifyDate(date || new Date());
...
exports.getRelativeDate = function (delta) {
	...
	return {
		delta: delta,
		description: $tw.language.getString("RelativeDate/" + (futurep ? "Future" : "Past") + "/Second", {
			variables: {period: "1"}
		}),
		updatePeriod: 1000
	};
...
exports.htmlTextEncode = function (s) {
	if (s) {
		return s.toString().replace(/&/gm, "&amp;").replace(/</gm, "&lt;").replace(/>/gm, "&gt;");
	} else {
		return "";
	}
};
...
exports.tagToCssSelector = function (tagName) {
	return (
		"tc-tagged-" +
		encodeURIComponent(tagName).replace(/[!"#$%&'()*+,\-./:;<=>?@[\\\]^`{\|}~,]/gm, function (c) {
			return "\\" + c;
		})
	);
};
