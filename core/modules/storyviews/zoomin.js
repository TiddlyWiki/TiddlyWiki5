...
const ZoominListView = function(listWidget) {
	const self = this;
	...
	const history = this.listWidget.wiki.getTiddlerDataCached(this.listWidget.historyTitle,[]);
	let targetTiddler;
	if(history.length > 0) {
...
ZoominListView.prototype.navigateTo = function(historyInfo) {
	...
	const sourceBounds = historyInfo.fromPageRect || {
	...
	const titleDomNode = findTitleDomNode(listItemWidget) || listItemWidget.findFirstDomNode();
	const zoomBounds = titleDomNode.getBoundingClientRect();
	// Compute the transform for the target tiddler to make the title lie over the source rectange
	const targetBounds = targetElement.getBoundingClientRect();
	let scale = sourceBounds.width / zoomBounds.width;
	let x = sourceBounds.left - targetBounds.left - (zoomBounds.left - targetBounds.left) * scale;
	let y = sourceBounds.top - targetBounds.top - (zoomBounds.top - targetBounds.top) * scale;
	...
	const self = this;
	const prevCurrentTiddler = this.currentTiddlerDomNode;
...
ZoominListView.prototype.remove = function(widget) {
	...
	let toWidget = widget.previousSibling();
	if(!toWidget) {
		toWidget = widget.nextSibling();
	}
	let toWidgetDomNode = toWidget && toWidget.findFirstDomNode();
