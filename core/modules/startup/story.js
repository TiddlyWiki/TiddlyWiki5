...
exports.startup = function () {
	...
	if ($tw.browser) {
		...
		$tw.rootWidget.addEventListener("tm-home", function (event) {
			...
			$tw.wiki.addTiddler(
				{title: DEFAULT_STORY_TITLE, text: "", list: storyList},
		...
		$tw.rootWidget.addEventListener("tm-permalink", function (event) {
			updateLocationHash({
				updateAddressBar:
					$tw.wiki.getTiddlerText(CONFIG_PERMALINKVIEW_UPDATE_ADDRESS_BAR, "yes").trim() === "yes"
						? "permalink"
						: "none",
				updateHistory: $tw.wiki.getTiddlerText(CONFIG_UPDATE_HISTORY, "no").trim(),
				targetTiddler: event.param || event.tiddlerTitle,
				copyToClipboard:
					$tw.wiki.getTiddlerText(CONFIG_PERMALINKVIEW_COPY_TO_CLIPBOARD, "yes").trim() === "yes"
						? "permalink"
						: "none",
...
function updateLocationHash(options) {
	...
	switch (options.updateAddressBar) {
		...
		case "permaview":
			$tw.locationHash =
				"#" + encodeURIComponent(targetTiddler) + ":" + encodeURIComponent($tw.utils.stringifyList(storyList));
			break;
	}
	...
	switch (options.copyToClipboard) {
		...
		case "permaview":
			url =
				...
				"#" +
				encodeURIComponent(targetTiddler) +
				":" +
				encodeURIComponent($tw.utils.stringifyList(storyList));
			break;
	}
	if (url) {
		$tw.utils.copyToClipboard(url, {
			successNotification: options.successNotification,
			failureNotification: options.failureNotification
		});
	}
