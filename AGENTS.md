...
- CSS targets the same era (roughly 2018). You SHOULD NOT use newer features such as `:has()`, container queries, `@layer`, `@scope`, or subgrid. If one would be far simpler than the alternative, you MAY propose it with trade-offs for the maintainer to decide; you MUST NOT adopt it silently
...
- When auditing your own work, you MUST be honest about sloppy fixes; distinguish legitimate test adjustments from weakening a test until it passes
...
- Each test MUST be reproducible by maintainers from its body and comments
...
- Then commit with `git commit -F commit-msg.md` and delete the file. You MUST NOT stage or commit `commit-msg.md` itself
...
- You MUST prefix any part that needs a decision from the user with a lightning icon
